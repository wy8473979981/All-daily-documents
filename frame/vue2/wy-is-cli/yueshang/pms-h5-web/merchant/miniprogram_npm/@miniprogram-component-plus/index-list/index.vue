<template>
<view>
<scroll-view class="page page-select-index" :style="'height: ' + windowHeight + 'px;'" enable-back-to-top :scroll-into-view="intoView" scroll-y @scroll="onScroll">
  <view>
    <slot></slot>
  </view>
  <view v-for="(item, index) in list" :key="index" class="index_list_item" :id="item.alpha">
    ...
    <view class="index-group__content">
      <view class="index-group__list">
        <block v-for="(subItem, index2) in item.subItems" :key="index2">
          <view class="index-group__item thin-border-bottom" hover-class="bg-highlight" :data-item="subItem" @tap="choose">
              {{subItem.name}}
          </view>
        </block>
      </view>
