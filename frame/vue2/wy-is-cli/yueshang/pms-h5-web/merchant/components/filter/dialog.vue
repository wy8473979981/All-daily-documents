      ...
      <block v-if="component.groups.length">
        <view v-for="(group, groupi) in component.groups" :key="groupi" class="group">
          ...
          <view class="group__items">
            <view v-for="(item, itemi) in group.items" :key="itemi" :class="'group-item ' + ( component.selected[item.value] ? 'selected' : '' ) + ' ' + ( !component.selected[item.value] && (group.disabled || component.disabled) ? 'disabled' : '' )" @tap="onGroupItemClick" :data-value="item.value" :data-group="group">
              {{ item.label }}
            </view>
          </view>
        </view>
      ...
      <block v-else>
        <view v-for="(item, itemi) in component.checkboxes" :key="itemi" class="checkbox-item">
          <label class="checkbox" @tap="onCheckboxClick" :data-value="item.value">
            <checkbox :checked="component.selected[item.value]" :disabled="!component.selected[item.value] && component.disabled" @tap.stop="onCheckboxClick" :data-value="item.value"></checkbox>{{ item.label }}
          </label>
        </view>
      </block>
    </view>
        ...
        const newGroups = groups.map((group, groupi) => {
          return {
            ...
            items: group.items.map(item => {
              const value = JSON.stringify(item.value || item);
  ...
  methods: {
    ...
    onGroupItemClick(e) {
      ...
      if (selected[value]) {
        ...
        if (!this.component.disabled && !this.component.groups[group.$index].disabled) {
    ...
    ok() {
      this.close();
      this.$emit("selected", {
        detail: Object.keys(this.data.component.selected).map(JSON.parse)
      });
    }
