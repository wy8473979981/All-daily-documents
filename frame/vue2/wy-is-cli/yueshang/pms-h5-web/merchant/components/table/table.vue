...
<view class="d-tabel">
  ...
  <view class="d-tbody">
    <view v-for="(row, rowi) in dataList" :key="rowi" class="d-row">
      <view v-for="(item, index) in colList" :key="index" :class="'d-col ' + ( item.width ? '' : 'd-flex' ) + ' '" :style="'width:' + item.width + ';color:' + ( item.color || (item.action ? '#399CFF' : '') ) + '; justify-content:' + item.align" :data-id="row[id]" :data-action="item.action" @tap="bindtap">
        <text v-if="item.key">
