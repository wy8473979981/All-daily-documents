	...
	<view class="app-table">
		<view class="app-table-header">
			<view class="app-table-header__items">
				<view
					v-for="(column, columni) in component.columns"
					:key="columni"
					:class="
						'app-table-header__item ' +
						(columni < fixednum ? 'fixed' : '') +
						' ' +
						(column.groupName ? 'in-group' : '') +
						' text-' +
						column.align +
						' ' +
						(column.sortable ? 'sortable' : '')
					"
					@tap="onHeadColumnClick"
				...
				>
					...
					<image
						v-if="column.sortable && component.sorted.key === column.key && component.sorted.direction === 'up'"
						style="width: 18rpx; height: 21rpx"
						src="/static/assets/images/icon-sortup.png"
					></image>
					<image
						v-else-if="column.sortable && component.sorted.key === column.key && component.sorted.direction === 'down'"
						style="width: 18rpx; height: 21rpx"
						src="/static/assets/images/icon-sortdown.png"
					></image>
			...
			<block v-else>
				<view class="app-table-row app-table-row--total" v-if="component.totalRow">
					<view
						v-for="(column, columni) in component.columns"
						:key="columni"
						:class="'app-table-row__item ' + (columni < fixednum ? 'fixed' : '') + ' text-' + column.align + ' ' + (column.sortable ? 'sortable' : '')"
						:style="column.$styles + ';'"
					>
...
const timers = {};
// import mpSticky from "/@miniprogram-component-plus/sticky";
...
export default {
	...
	computed: {
		dynamic() {
			const { fixednum, columns, values, totalRow, selected } = this;
	...
	watch: {
		...
		dynamic(value) {
			const { fixednum, columns, values, totalRow, selected } = value;
	...
	methods: {
		...
		onHeadColumnClick(e) {
			...
			if (sortable) {
				...
				const sortedRows = sorted.direction
					? [...rows].sort((r1, r2) => {
