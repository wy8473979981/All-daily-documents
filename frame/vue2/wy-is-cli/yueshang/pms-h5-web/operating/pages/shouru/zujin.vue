<template>
	...
	<view :class="'shouru-zujin ' + (recovery.show ? 'recovery' : '')">
		...
		<app-section title="项目排名">
			...
			<app-table :fixednum="2" :totalRow="totalRow" :values="dataList" :columns="columns" :selected="tableSelected" @row-column-click="onRowColumnClick"></app-table>
		</app-section>
		...
		<app-filter-dialog
			:checkboxes="dataList"
			@selected="showComparisonList"
			v-if="params.type !== 'dj' && !recovery.show"
			tip="注意：最少选两项"
			@open="bindopen"
			@close="bindclose"
		>
	...
	methods: {
		onRowColumnClick(e) {
			...
			if (column.key === "projectName") {
				...
				app.globalData.page.go("./shouruxq/shouruxq", { ...this.params, bisProjectId: bisProjectId, projectName: projectName, storeType: this.params.storeType });
				return;
			}
		...
		async getDataList() {
			...
			let result = res.data.monthCollectionRateList.map((item, index) => {
				return { ...item, ranking: index, label: item.projectName };
			});
					...
					series: [
						{
							...
							data:
								typeof res.data.curMonthColRateForHeadChart === "object"
									? res.data.curMonthColRateForHeadChart.map((num) => num)
									: JSON.parse(res.data.curMonthColRateForHeadChart).map((num) => num),
							areaStyle: {
						...
						},
						{
							...
							name: legendData[1],
							data:
								typeof res.data.curMonthColRateForHeadPrevChart === "object"
									? res.data.curMonthColRateForHeadPrevChart.map((num) => num)
									: JSON.parse(res.data.curMonthColRateForHeadPrevChart).map((num) => num), // data: curMonthColRateForHeadPrevChart
						},
					],
				},
			...
			await app.globalData.request
				...
				.get("/open/bis-open-mark-info!findPageMarkInfoList.action", { ...params, pageType: params.type, bizProjectId: "", pageSource: getCurrentPages().pop().route })
				.then((res) => {
