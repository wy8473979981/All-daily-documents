...
<template>
  <div class="app-page-main">
    <YsnPageContent
      ...
      persmission="OpenCloseShopsList"
...
export default {
  ...
  data() {
    return {
      ...
      // 接口请求配置
      queryConfig: {
        api: baseProjectApi.openCloseShopList, // 接口请求方式
        deleteApi: baseProjectApi.openCloseShopDelete // 批量删除
      },
  ...
  methods: {
    ...
    rowClick(row) {
      ...
      this.$router.push({
        // 根据状态跳转详情或者编辑
        name: [1, 4].includes(checkStatus) ? 'OpenCloseShopsAdd' : 'OpenCloseShopsDetail',
        query: {
          id: row.id
        }
      })
