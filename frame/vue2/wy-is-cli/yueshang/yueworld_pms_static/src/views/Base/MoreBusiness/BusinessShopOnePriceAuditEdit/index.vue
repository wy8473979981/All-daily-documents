...
export default {
  ...
  data() {
    return {
      // 审核记录页面
      examinePage: 'BusinessShopOnePriceAuditExamine',
  ...
  methods: {
    ...
    async getData() {
      ...
      this.id = id
      try {
        const { data } = await baseBusinessApi.shopOnePriceAuditDetail({ id })
        // 格式化一铺一价信息方便组件编辑
    ...
    async handlerAudit() {
      try {
        await baseBusinessApi.shopOnePriceAuditAudit({
          id: this.id
        })
        setTimeout(() => {
          this.$router.go(-1)
    ...
    async handlerSave() {
      ...
      try {
        ...
        if (res) {
          await baseBusinessApi.shopOnePriceAuditSave({
            id
          })
