...
<template>
  <div class="app-page-main">
    <YsnPageContent
      ...
      persmission="BusinessShopOnePriceAuditList"
...
export default {
  ...
  data() {
    return {
      ...
      // 接口请求配置
      queryConfig: {
        api: baseBusinessApi.shopOnePriceAuditList // 接口请求方式
      },
  ...
  methods: {
    ...
    rowClick(row) {
      ...
      this.$router.push({
        // 根据状态值跳转编辑或者详情
        name: ![1, 4].includes(checkStatus) ? 'BusinessShopOnePriceAuditDetail' : 'BusinessShopOnePriceAuditEdit',
        query: {
          id: row.id
        }
      })
