...
export default {
  ...
  data() {
    return {
      configData: [ // 新单元表单配置
        ...
        { itemType: 'input', prop: 'FloorName', label: '楼层', placeholder: '请选择全部楼层', width: '200px', change: this.handelChange },
        { itemType: 'select', prop: 'OfficeId', label: '单元号', placeholder: '请选择单元号', width: '200px', list: this.$PUBFN.getOptionsList('globalSearchList') }
      ],
      rulesForm: { // 表单验证
  ...
  methods: {
    ...
    handelChange(val) {
      this.currentFormData.BuildingName = 0
    }
  }
