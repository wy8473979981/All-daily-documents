...
<template>
  <div>
    <ysn-dialog :out-close="outClose" :title="'车流目标'" :width="'1050px'" :is-show-dialog="isShowDialog" :external="isShowDialog" @closeDialog="closeDialog">
      <!-- 弹窗表格 -->
      <template #contain>
...
export default {
  ...
  props: {
    ...
    formData: {
      type: Object,
      default: {
        targetTrafficFlowEntry: 0, // 年度指标（万辆）
        list: [{ // 月入场数据集合
          date: new Date(),
          targetTrafficFlowEntryMonthly: 0, // 目标入场车流(辆)
          data: [{
            prop: 'targetTrafficFlowEntryDaliy'
          }]
        }]
      }
    }
  ...
  data() {
    return {
      ...
      prop: 'targetTrafficFlowEntryDaliy',
      tableDescConfig: [
        { text: '目标入场车流(辆)', value: this.toThousands(0), decimal: 0 }
      ]
    }
  },
  watch: {
      ...
      for (var i = 0; i < this.formData.list.length; i++) {
        if (this.getDate(this.dateInfo.date) === this.getDate(this.formData.list[i].date)) {
          ...
          this.tableDescConfig = [
            { text: '目标入场车流(辆)', value: this.toThousands(this.formData.list[i].targetTrafficFlowEntryMonthly), decimal: 0 }
          ]
          return
        }
      }
  ...
  mounted() { // 数据初始化，先默认当月
    ...
    for (var j = 0; j < this.formData.list.length; j++) {
      if (this.getDate(new Date()) === this.getDate(this.formData.list[j].date)) {
        this.tableDescConfig = [
          { text: '目标入场车流(辆)', value: this.toThousands(this.formData.list[j].targetTrafficFlowEntryMonthly), decimal: 0 }
        ]
  ...
  methods: {
    input() { // 输入数据时计算总人数
      ...
      for (var i = 0; i < this.inputConfig.length; i++) {
        if (this.inputConfig[i].targetTrafficFlowEntryDaliy != null && this.inputConfig[i].targetTrafficFlowEntryDaliy !== '') {
          value += parseInt(this.inputConfig[i].targetTrafficFlowEntryDaliy)
        }
      }
      this.tableDescConfig[0].value = this.toThousands(value)
      this.formData.targetTrafficFlowEntry = 0
      for (var j = 0; j < this.formData.list.length; j++) {
        if (this.getDate(this.formData.list[j].date) === this.getDate(this.dateInfo.date)) {
          this.formData.list[j].targetTrafficFlowEntryMonthly = value
        }
        this.formData.targetTrafficFlowEntry += this.formData.list[j].targetTrafficFlowEntryMonthly
      }
      this.formData.targetTrafficFlowEntry = (this.formData.targetTrafficFlowEntry / 10000).toFixed(2)
    },
