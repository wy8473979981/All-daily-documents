...
export default {
  ...
  data() {
    return {
      ...
      searchForm: {
        ...
        auditStatus: [], // 状态
        contFallDates: ['', ''], // 解约日期
        contFallStartDate: '', // 解约日期开始
        contFallEndDate: '' // 解约日期结束
      },
      ...
      // 接口请求配置
      queryConfig: {
        api: contractTerminationApi.contractTerminationList, // 接口请求方式
  ...
  methods: {
    ...
    beforeQueryData(data) {
      ...
      data.contFallStartDate = data.contFallDates[0]
      data.contFallEndDate = data.contFallDates[1]
      return data
    },
    // 行点击详情页面或编辑页面
    rowClick(row) {
      ...
      if (that.auditStatus === 0 || that.auditStatus === 4) {
        this.$router.push({ name: 'ContractTerminationAdd', query: { id: row.id }})// 草稿，驳回 跳转编辑页
      } else {
        this.$router.push({ name: 'ContractTerminationDetail', query: { id: row.id }})// 审核中，已审核 跳转详情页
      }
    },
    // 新增跳转
    addBox() {
      this.$router.push({ name: 'ContractTerminationAdd' })
    },
