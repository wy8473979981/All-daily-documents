<template>
  <div>
    <ysn-dialog title="拆铺" :is-show-dialog="visible" external>
      <ysn-btn class="mr-8" @click="Dismantling">合铺</ysn-btn>
      <!-- 弹窗表单-->
      <template #contain>
...
<script>
import InformationDoalog from './informationDoalog'
export default {
  name: 'FormItem',
  components: { InformationDoalog },
  data() {
    return {
      visible: false,
      formData: { // 表单数据
      ...
      configData: [ // 表单配置
        { itemType: 'datePicker', prop: 's1', type: 'datetimerange', label: '拆铺生效日期', placeholder: '请选择楼层', width: '200px' },
