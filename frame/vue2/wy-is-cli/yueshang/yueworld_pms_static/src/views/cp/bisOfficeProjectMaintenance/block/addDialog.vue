...
<template>
  <div>
    <ysn-dialog title="新增" width="920px">
      ...
      <template #contain>
        <div>
          ...
          <el-form v-for="(item,index) in wholeObj.contractFormData" v-show="index==tab_active" :key="index" :ref="'ruleForm'+index" :model="wholeObj.contractFormData[index]" label-position="top" size="small" :rules="contractRule">
            <ysn-formItem
              :form="item"
  ...
  methods: {
    ...
    handleToggle(type, index) {
      if (type === 'plus') {
        ...
        // 重新定位tab下标
        this.tab_active = this.tab_active === index ? 0 : this.tab_active > index ? this.tab_active - 1 : this.tab_active
        // 删除tab点击的一块
        this.tabs_list.splice(index, 1)
        this.tabs_list = this.tabs_list.map((item, index) => ({
          label: `第${index + 1}单元`
        }))
        // this.$delete(this.$refs, `ruleForm${index}`)
