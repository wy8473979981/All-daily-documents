<template>
  <div class="detail-info pb-20">
    ...
    <el-row class="pt-4 pl-16 info-box">
      <el-col v-for="(item2, index2) in item.statisticalData" :key="index2" :span="6">
        <div class="text-14">{{ item2.name }}</div>
        <div class="text-24 d-flex">{{ item2.num }}
    ...
    <div v-if="item.statisticalData&&item.statisticalData.length>0" class="echarts-content pl-16">
      <el-row>
    ...
    <div v-if="item.series&&item.series.length>0" class="zhexiantu-box">
      <div v-if="item.seriesInfo" class="title pl-8 pr-8 flex-ju-between">
        <div class="flex-ju-al-between">
          <div class="mr-40">{{ item.seriesInfo.title }}</div>
  ...
  mounted() {
    ...
    if (this.item.circleData) {
      ...
      this.item.circleData.forEach(item2 => {
        ...
        const myChart = echarts.init(allDivDocument[i])
    ...
    if (this.item.series && this.item.series.length > 0) {
      const myChart2 = echarts.init(document.getElementById('echarts-zhexian' + this.modalIndex))
