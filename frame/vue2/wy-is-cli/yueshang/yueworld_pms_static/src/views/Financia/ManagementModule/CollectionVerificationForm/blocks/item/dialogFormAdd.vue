...
export default {
  ...
  data() {
    return {
      ...
      outClose: true,
      searchConfigNo: [{ itemType: 'input', prop: 'fuzzyEnquiry', label: '', placeholder: '请输入应收编号', width: '220px', isSearch: true, isFirst: true },
      ...
      searchConfigYes: [
        { itemType: 'monthPicker', prop: 'financialDate', label: '', type: 'monthrange', format: 'yyyy年MM月', valueFormat: 'yyyy-MM', startPlaceholder: '权责月起', endPlaceholder: '权责月止', width: '240px', isFirst: true }
      ],
      // 接口请求配置
      queryConfig: {
        api: collectionList.dialogFormAdd, // 接口请求方式
      ...
      tableConfig: [
        ...
        { receivableAmount: '应收金额(元)', isShow: true, type: 'money' },
  ...
  computed: {
    searchConfig() {
      console.log(this.$route.query.unknownType)
      if (Number(this.$route.query.unknownType) === 1) {
        return [...this.searchConfigNo, ...this.searchConfigYes]
      }
      return [...this.searchConfigNo]
    }
  },
  created() {
    // console.log(Number(this.$route.query.unknownType), '***************')
  },
  methods: {
    // 弹窗删除已选择的
    removeChoose(e) {
