 ...
 * @LastEditTime: 2021-07-15 13:05:03
-->
<template>
  <div>
    <ysn-formTable
    ...
    >
      ...
      <template #t12="scope">
        ...
        <span v-else class="operation minus-icon" @click="minusData(scope.$index,false)" />
        <span v-if="scope.row.disabled" class="operation minus-icon" @click="minusData(scope.$index,true)" />
      </template>
    </ysn-formtable>
...
export default {
  ...
  mounted() {
    ...
    //   this.wholeObj.tableDatas[i] = [list[i]]
    // }
    ...
    for (let j = 0; j < datas.length; j++) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        // console.log(this.wholeObj.tableData)
        // console.log(i)
        this.wholeObj.tableDatas[datas[j]].splice(i + 1, 0, { leaseArea: '11', freeScope: '21', beginningDate: this.wholeObj.tableDatas[datas[j]][0].beginningDate, endingDate: this.wholeObj.tableDatas[datas[j]][0].endingDate, t11: '', id: this.wholeObj.tableDatas[datas[j]][0].id, disabled: true, begin: true, end: true })
      }
  ...
  methods: {
    ...
    addData(row, index) {
      ...
      console.log('ind:' + ind)
      rentFreePeriod.splice(ind, 0, { leaseArea: '11', freeScope: '21', beginningDate: '', endingDate: endDate, t11: '', id: row.id, disabled: false, begin: false, end: true })
      console.log(this.wholeObj.tableData)
      this.getSpanArr(this.wholeObj.tableData)
    },
    // // 后面操作
