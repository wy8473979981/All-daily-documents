 ...
 * @LastEditTime: 2021-09-03 17:39:50
 */
export const pageConfig = (showMenu, contract, edit, listData) => {
  // Contract 合同签署  1 两方  2 第三方
  console.log(showMenu, contract, edit, listData)
  const addList = []
  // console.log(listData.reimbursement)
  if (listData.reimbursement) { // 接口数据拿到情况
    // const addList = []
    console.log(listData.reimbursement, '数据完成')
    listData.reimbursement.tableData.map((item, index) => {
      addList.push({ feeName: item.feeName, showFlod: true, component: require('./blocks/fixedRent.vue').default, dataKey: 'fixedRent', valiForm: ['rulesForm'], title: `周期性费项${index + 1}` })
    })
    // console.log(addList)
  }
  const baseList = [
    ...
    //   title: '起计日', // 标题名
    //   showFlod: true, // 是否显示展开收起 默认为true
    //   component: require('./blocks/startingDat.vue').default, // 内容组件引用
    //   dataKey: 'startingDate', // 对应formData的key
    //   column: showMenu ? 4 : 5,
    //   valiForm: ['rulesForm'] // 需要触发校验的表单
    // },
    {
