 ...
 * @LastEditTime: 2021-08-05 16:46:13
-->
<template>
  <div>
    ...
    <div class="flex-ju-end pr-8 mb-8">
      ...
      >
        ...
        <template #contain>
          ...
          <!-- 提成-总额阶梯提成-否 -->
          <DialogNo v-if="(currentFormData.fixedRent.commissionType===3 ||currentFormData.fixedRent.commissionType===2) &&currentFormData.fixedRent.isClassDeduction === 1" :data="currentFormData.tableList" :type-status="data.businessAffairs" :relist="data.businessAffairs.leaseArea" :list-obj="data" :adds="adds" :tax="currentFormData.fixedRent.t0" @ok="ok" />
          <!-- 提成-总额阶梯提成-是 -->
          <CommissionDoalog v-if="(currentFormData.fixedRent.commissionType===3 ||currentFormData.fixedRent.commissionType===2) &&currentFormData.fixedRent.isClassDeduction === 0" :data="currentFormData.tableList" :type-status="data.businessAffairs" :list-obj="data" :tax="currentFormData.fixedRent.t0" @ok="ok" />
          <!-- 提成-固定提成-否 -->
      ...
      <div class="flex-ju-end pr-8 mb-8">
        ...
        >
          ...
          <template #contain>
            ...
            />
            <!-- 提成-总额阶梯提成-是 -->
            <CommissionDoalog v-if="(currentFormData.sommission.commissionType===3 ||currentFormData.sommission.commissionType===2) &&currentFormData.sommission.isClassDeduction === 0" :data="currentFormData.list" :type-status="data.businessAffairs" :tax="currentFormData.sommission.t0" @ok="ok" />
            <!-- 提成-固定提成-否 -->
...
var listTable = [
  ...
  { leasTterm: '租赁期间', tableList: [
    { t4: '日期', align: 'right', width: 300, itemType: 'text', formatter: row => {
      return row.beginningDate + '~' + row.endingDate
    } }, // TODP 两个字段 数据处理 beginningDate 租期开始日  endingDate 租期结束日
    { isPromotional: '优惠', align: 'right', width: 150 }
  ...
  computed: {
    ...
    tableConfig() {
      ...
      if (this.currentFormData.add === '允许') {
        tableList = [
          { leaseArea: '租赁区域', width: 150, isShowSlots: true },
          { leasTterm: '租赁期间', tableList: [
        ...
        if (this.currentFormData.fixedRent.commissionType === 3 && this.currentFormData.fixedRent.isClassDeduction === 1) {
          ...
          if (this.currentFormData.fixedRent.t0 === 1 || this.currentFormData.fixedRent.t0 === 3) {
            return [
              { leaseArea: '租赁区域', width: 150, isShowSlots: true },
              { leasTterm: '租赁期间', tableList: [
                { t4: '日期', align: 'right', width: 300, formatter: row => {
                  return row.beginningDate + '~' + row.endingDate
                } }
              ] },
          ...
          if (this.currentFormData.sommission.t0 === 1 || this.currentFormData.sommission.t0 === 3) {
            return [
              ...
              { name: '总额阶梯提成申请条件', labelClass: 'borderBottom', tableList: [
                { monthlyEstimatedTurnover: '月预估营业额', type: 'money' },
                { class: '品类' },
                { monthlyEstimatedTurnover: '月营业额起' },
                { monthlyEstimatedTurnover: '月营业额止' },
                { commDiscountApply: '申请扣率', type: 'rate' }
              ] }
            // ...listTable
            ]
  ...
  methods: {
    ...
    listdata() {
      ...
      if (this.isue) {
        ...
        console.log(this.currentFormData.tableList)
        console.log(this.currentFormData.list, '112')
