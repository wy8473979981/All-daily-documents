 ...
 * @LastEditTime: 2021-08-13 15:54:33
-->
<template>
  <div>
    <ysn-formTable
    ...
    >
      ...
      <template #salesTaxRate="scope">
        <div class="ysn-form">
          <el-select v-model="scope.row.salesTaxRate" placeholder="请选择" :disabled="scope.$index!==0" @change="changeOptiosn">
            <el-option
              v-for="item in txd"
              :key="item.value"
              :label="item.label"
...
export default {
  ...
  computed: {
    add() {
      if (this.typeStatus.isRentShop === 1) {
        return ['t12', 'name', 'monthlyEstimatedTurnover', 'commDiscountApply', 'leaseArea', 'beginningDate', 'endingDate', 'salesTaxRate', 'isPromotional']
      }
      return ['t12', 'name', 'monthlyEstimatedTurnover', 'commDiscountApply', 'beginningDate', 'endingDate', 'salesTaxRate', 'isPromotional']
    },
    formTableConfig() {
      ...
      return [
        ...
        { leasTterm: '固定提成申请条件', tableList: [
          ...
          { commDiscountApply: '申请扣率', width: '110px', itemType: 'input', inputType: 'Number', hasSlot: true },
          { name: '操作', width: '', itemType: '', hasSlot: true }
        ]
        }
      ]
  ...
  methods: {
    ...
    operation(row, index) {
      ...
      rentFreePeriod.splice(ind, 0, { leaseArea: '', id: row.id, freeScope: '', freeStartDate: '', freeEndDate: '', apply: '', operation: '' })
      console.log(rentFreePeriod)
