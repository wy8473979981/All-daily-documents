 ...
 * @LastEditTime: 2021-08-05 19:27:18
-->
<template>
  <div>
    <ysn-formTable
    ...
    >
      ...
      <template #monthlyEstimatedTurnover="scope">
        <el-input v-model="scope.row.monthlyEstimatedTurnover" placeholder="请输入" />
      </template>
      <!-- 操作1 -->
      <template #t12="scope">
      ...
      <template #startingMonthlyTurnover="scope">
        <el-input v-model="scope.row.startingMonthlyTurnover" placeholder="请输入" />
      </template>
      <template #endingMonthlyTurnover="scope">
        <el-input v-model="scope.row.endingMonthlyTurnover" placeholder="请输入" />
      </template>
      <template #commDiscountApply="scope">
        <el-input v-model="scope.row.commDiscountApply" placeholder="请输入" />
      </template>
  ...
  computed: {
    add() {
      if (this.typeStatus.isRentShop === '1') {
        return ['t12', 'name', 'monthlyEstimatedTurnover', 'startingMonthlyTurnover', 'classification', 'commDiscountApply', 'endingMonthlyTurnover', 'leaseArea', 'salesTaxRate', 'beginningDate', 'endingDate']
      }
      return ['t12', 'name', 'monthlyEstimatedTurnover', 'startingMonthlyTurnover', 'classification', 'commDiscountApply', 'endingMonthlyTurnover', 'beginningDate', 'endingDate', 'salesTaxRate']
    },
    formTableConfig() {
      if (this.tax === 1 || this.tax === 3) {
        return [
          ...
          { LeaseCycle: '年/月/日', wdith: '300px', itemType: 'text' },
          // ] },
          { t12: '拆分租期', itemType: '', hasSlot: true },
          // { tax: '税率', itemType: 'text' },
          { leasTterm: '总额阶梯提成申请条件', tableList: [

            { monthlyEstimatedTurnover: '月预估营业额', width: '150px', itemType: 'input' },
            { classification: '品类', width: '150px', itemType: 'input' },
            { name: '操作', width: '100px', itemType: 'input', hasSlot: true },
            { endingMonthlyTurnover: '月营业额起', width: '150px', itemType: 'input' },
            { endingMonthlyTurnover: '月营业额止', width: '150px', itemType: 'input' },
            { commDiscountApply: '申请扣率', width: '150px', itemType: 'input' },
      ...
      return [
        ...
        { name: '操作', width: '100px', itemType: 'input', hasSlot: true },
        { startingMonthlyTurnover: '月营业额起', width: '150px', itemType: 'input' },
        { endingMonthlyTurnover: '月营业额止', width: '150px', itemType: 'input' },
        { commDiscountApply: '申请扣率', width: '150px', itemType: 'input' },
        { name1: '操作', width: '100px', itemType: 'input', hasSlot: true }
        // ]
