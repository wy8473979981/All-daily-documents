  ...
  <el-form ref="rulesForm" :model="currentFormData" label-position="top" size="small">
    <ysn-formItem :form="currentFormData" :config="configData" :column="5" clearable>
      <template #effectiveDateLabel>
        生效起始日期
        <el-tooltip class="item" effect="dark" :content="`需晚于原单元最晚合同结束日期${data.contEndDate?':'+data.contEndDate:''}`" placement="top">
...
export default {
  ...
  data() {
    return {
      configData: [
        ...
        { itemType: 'input', width: '206px', prop: 'officeNo', label: '单元编码', disabled: true, placeholder: '—自动带入—' },
        { itemType: 'input', width: '206px', prop: 'officeId', label: '单元号', required: true },
        { itemType: 'select', width: '206px', prop: 'officeType', label: '营运类型', required: true, list: PUBFN.getOptionsList('spaceTypeOpenClose') },
        { itemType: 'select', width: '206px', prop: 'officeEquityNature', label: '产权性质', required: true, list: PUBFN.getOptionsList('propertyType') },
        ...
        { itemType: 'input', width: '206px', prop: 'leasableArea', label: '计租面积(m²)', disabled: true, inputType: 'Number', placeholder: '—自动带入—' },
        ...
        { itemType: 'checkbox', width: '206px', prop: 'otherArea', label: '计入统计', list: PUBFN.getOptionsList('officeSumType') }
        // { itemType: 'input', width: '206px', prop: 'standardDeposit', label: '租赁保证金标准(元)', required: true, type: 'Number' },
        // { itemType: 'input', width: '206px', prop: 'standardDecorationDeposit', label: '装修保证金标准(元)', required: true, type: 'Number' },
        // { itemType: 'input', width: '206px', prop: 'standardRentFreePeriod', label: '免租期(天)', required: true, type: 'Number' }
      ]
    }
  },
  computed: {
  ...
  methods: {
    ...
    triggerParentData() {
      ...
      // 遍历寻找父组件
      while (parent.$vnode.tag.indexOf('OpenCloseOfficesAdd') === -1) {
