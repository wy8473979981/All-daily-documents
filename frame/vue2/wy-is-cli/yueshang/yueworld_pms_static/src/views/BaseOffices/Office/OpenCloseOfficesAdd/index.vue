...
import { baseApi, baseOfficesApi } from '@/api'
...
export default {
  ...
  computed: {
    examinePage() {
      return this.id ? 'OpenCloseOfficesExamine' : ''
    }
  },
  watch: {
    // 项目改变自动填充值
  ...
  methods: {
    ...
    setConfigTitle() {
      ...
      if (id) {
        this.pageConfig[3].group[1].title = '新单元一铺一价'
      } else {
        this.pageConfig[3].group[1].title = '一铺一价标准明细'
      }
    },
    // 获取详情数据
    async getData() {
    ...
    async getProjectInfo() {
      ...
      const { baseInfo } = await baseOfficesApi.openCloseOfficesProjectInfo({
        id: 1 // 项目id
      })
      // 基本信息
    ...
    async handlerClick() {
      ...
      this.$nextTick(async() => {
        ...
        baseOfficesApi.openCloseOfficesSave(this.formData).then(res => {
    ...
    async getShopInfoList() {
      ...
      const { data } = await baseApi.officesBaseInfo({
        id: ids // mock 写死id
      })
      data.forEach(item => {
        groupShopOld.push({
    ...
    getLogData(formData) {
      ...
      // 铺位数量计算
      changeData.officeAmountOld = formData.groupShopOld.length
      changeData.officeAmountNew = formData.groupShopNew.length
      changeData.officeAmountDiff = Math.abs(formData.groupShopOld.length - formData.groupShopNew.length)
      // 计算原铺数据
