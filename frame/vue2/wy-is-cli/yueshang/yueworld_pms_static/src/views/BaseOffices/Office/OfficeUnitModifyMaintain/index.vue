...
export default {
  ...
  data() {
    return {
      ...
      // 接口请求配置
      queryConfig: {
        api: baseOfficesApi.officeUnitModifyMaintenanceList, // 接口请求方式
        deleteApi: baseOfficesApi.officeUnitModifyMaintenanceDel, // 批量删除
      },
  ...
  methods: {
    ...
    rowClick(row) {
      this.formData = row
      this.dialogType =
        row.checkStatus === 2 || row.checkStatus === 3 ? 'detail' : 'edit'
      this.dialogNum++
    },
    /**
