...
<template>
  <div class="app-page-main">
    ...
    >
      ...
      <template #btn>
        <ysn-confirm
          v-if="id!==0&&formData.baseInfo.checkStatus!=5"
          title="提示"
          width="484px"
          top="120px"
          :content="`是否确认取消?`"
          cancel-text="暂不取消"
          confirm-text="确认取消"
          tip="此操作将永久取消,不可撤回,请谨慎操作"
          @confirm="deleteBox"
        >
          <ysn-btn class="mr-12" type="delete">取消拆合铺</ysn-btn>
        </ysn-confirm>
      </template>
    </ysn-pageAccorGroupDetail>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      // 审核记录页面
      examinePage: 'OpenCloseOfficesExamine',
      id: null,
      // 统一数据
      formData: {
        // 基本数据
    ...
    formatterLogData(changeData, formData) {
      ...
      if (isOldInfo) {
        logData.push(
          {
            disName: '原单元',
            officeAmount: changeData.spaceAmountOld,
            totalLeasableArea: changeData.totalLeasableAreaOld,
            totalGrossArea: changeData.totalGrossAreaOld,
            totalNetArea: changeData.totalNetAreaOld
          }
      ...
      if (isNewInfo) {
        logData.push({
          disName: '新单元',
          officeAmount: changeData.spaceAmountNew,
          totalLeasableArea: changeData.totalLeasableAreaNew,
          totalGrossArea: changeData.totalGrossAreaNew,
          totalNetArea: changeData.totalNetAreaNew
        })
      ...
      if (isNewInfo && isOldInfo) {
        logData.push({
          disName: '差异',
          officeAmount: changeData.spaceAmountDiff,
          totalLeasableArea: changeData.totalLeasableAreaDiff,
          totalGrossArea: changeData.totalGrossAreaDiff,
          totalNetArea: changeData.totalNetAreaDiff
        })
