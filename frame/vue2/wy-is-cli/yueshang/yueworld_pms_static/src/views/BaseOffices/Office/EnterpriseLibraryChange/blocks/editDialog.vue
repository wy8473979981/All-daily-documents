    ...
    >
      ...
      <template #btn>
        <div class="flex-ju-end mt-40">
          <ysn-btn class="mr-8" @click="examine">提交审核</ysn-btn>
          <ysn-btn class="mr-8" @click="examine">发起审核</ysn-btn>
          <ysn-btn type="info" class="mr-8" @click="validate">保存</ysn-btn>
          <ysn-btn type="info" class="mr-16" @click="validate">审核记录</ysn-btn>
  ...
  data() {
    return {
      ...
      formLicenseConfig: [
        ...
        { tenantLicenseAttachment: '证照照片', itemType: 'input' }
      ],
      // 开票信息表单配置
      billingTableConfig: [
      ...
      sincerityTableConfig: [
        { tenantCreditRecord: '诚信记录类别', width: '200px', itemType: 'select', list: this.$PUBFN.getOptionsList('enterpriseType'), listName: 'label', listValue: [] },
