...
export default {
  ...
  computed: {
    ...
    configData() {
      const listForm = [
        ...
        { itemType: 'select', prop: 's1', label: '企业性质', placeholder: '请选择', list: this.$PUBFN.getOptionsList('enterpriseType'), listName: 'label', listValue: [], change: this.changeEnterprise }, // TODO 字段未找到
  ...
  methods: {
    ...
    validate() {
      ...
      const list = ['rule', 'formTab', 'ruleForm']
      this.verification(list, this)
      // this.verification(list).then((res) => {
      //   console.log(res)
      // })
    },
