...
<template>
  <div class="app-page-main">
    <YsnPageContent
      ...
      persmission="OfficesOnePriceAuditList"
...
export default {
  ...
  data() {
    return {
      ...
      // 接口请求配置
      queryConfig: {
        api: baseOfficesApi.officesOnePriceAuditList // 接口请求方式
      },
  ...
  methods: {
    ...
    rowClick(row) {
      ...
      this.$router.push({
        // 根据状态值跳转编辑或者详情
        name: ![1, 4].includes(checkStatus) ? 'OfficesOnePriceAuditDetail' : 'OfficesOnePriceAuditEdit',
        query: {
          id: row.id
        }
      })
