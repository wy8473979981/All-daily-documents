          ...
          <el-col v-for="(item, index) in switchList" :key="index" :span="6">
            <div class="switch-date-content" :class="item.title === '区域/城市' ? 'switch-date-city' : ''">
              ...
              <div>
                ...
                />
                <div v-else class="city-btn primary-color" :style="form.searchSelect.length > 0 ? 'width:50px':''">
                  <!-- 全部 -->
                  <ysn-selectDialog :form="form" prop="searchSelect" :list="cityList" placement="bottom-start">
...
export default {
  ...
  data() {
    return {
      ...
      switchList: [
        ...
        { title: '商业街', value: true, label: '20+4' },
        { title: '轻资产', value: true, label: '37+3' },
  ...
  watch: {
    switchList: {
      handler(val) {
        this.$emit('switch', this.switchList)
      },
      deep: true,
      immediate: true
    }
