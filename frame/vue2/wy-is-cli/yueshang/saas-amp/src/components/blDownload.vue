  ...
  <div class="page-title">
    <el-row class="" type="flex" justify="space-between" align="">
      <el-col class="down" align="right" v-if="showDown" >
        <span v-if="permission.indexOf('clzc_import') > -1 && showImport" @click="dealLoad" style="margin-right: 20px;">导入数据</span>
        <span @click="dealDown" style="margin-right: 20px;">下载模板</span>
        <span v-if="permission.indexOf('clzc_import') > -1" @click="getAttachment" >附件</span>
      </el-col>
    </el-row>
    <div>
      <!-- 上传 -->
    ...
    <div>
      <!-- 附件 -->
      <Attachment :open="attachment" :projectConfigId="projectConfigId" :attachmentUpload="attachmentUpload" @close="closeAttachment"></Attachment>
    </div>
  </div>
</template>
...
import Attachment from '@views/stockAssets/attachment'
export default {
  props: {
    showDown: {
      type: Boolean,
