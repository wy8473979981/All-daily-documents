  ...
  <div class="list-page">
    <el-row class="tool-bar" type="flex" justify="space-between" align="middle">
      <el-col :span="24">
        ...
        <el-button @click="toCreate" size="mini" type="primary">新增竞品类比</el-button>
        ...
        <el-button @click="confirm_batch" size="mini" type="success" v-show="params.contract_mould_cate_status === 'DELETED'" plain style="color: #f56c6c;">批量恢复</el-button>
      </el-col>
    </el-row>
      ...
      <el-table-column label="操作" width="160" align="center" fixed="right">
        <template slot-scope="scope">
          <div v-if="scope.row.contract_mould_cate_status !== 'DELETED'">
            <el-button size="mini" type="warning" @click="toUpdate(scope.row.type_id)" plain style="color: #e6a23c;">编辑</el-button>
...
export default {
  ...
  methods: {
    ...
    async handelConfirm (id) {
      ...
      if (this.params.contract_mould_cate_status === 'DELETED') {
        res = await companyContract.analogyDel({market_id: id})
      } else {
    ...
    async confirm_batch () {
      ...
      if (selection.length === 0) {
        this.$message.error(`选择要${type}的竞品类比数量`)
        return
      }
      try {
        await this.$confirm(`确认要${type}吗？`)
        const batchDel = selection
          .map(item => item.rent_calculation_id)
          .join(',')
    ...
    toUpdate (id, layout_floor_type) {
      ...
      this.$router.push({path: './analogyStep', query: { type: id,market_id: this.market_id, layout_floor_type:layout_floor_type }})
    },
    async getList () {
      ...
      const params = {
        ...
        rent_calculation_id: this.market_id
      }
    ...
    toCreate () {
      this.$router.push({path: './analogyStep',query:{ market_id: this.market_id, layout_floor_type: this.layout_floor_type }})
    },
    // 返回上一页
    toBackPage () {
      this.$router.go(-1)
  ...
  mounted () {
    this.getList()
    console.log(this.$route.query.market_id, '----[输出测算列表id]----')
  }
}
</script>
