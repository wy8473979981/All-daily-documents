...
export default {
  ...
  methods: {
    ...
    async handleLogin () {
      ...
      const res = await user.userLoginBL(this.loginForm)
      if (res.code === 200) {
        localStorage.setItem('ys_contract_token', res.result.token)
        localStorage.setItem('ys_contract_jwt_token', res.result.jwt_token)
        localStorage.setItem('ys_contract_permission', res.result.permission)
        const userInfo = { userid: res.result.admin_id, username: res.result.admin_truename, ...res.result }
        this.saveUserInfo(userInfo)
        // if (this.$route.path) {
        //   this.$route.meta.visible = false
        // }
    ...
    getLoginSSO () {
      ...
      user.userLoginBLSSO(params).then(res => {
        if (res.code === 200) {
          localStorage.setItem('ys_contract_token', res.result.token)
          localStorage.setItem('ys_contract_jwt_token', res.result.jwt_token)
          localStorage.setItem('ys_contract_permission', res.result.permission)
