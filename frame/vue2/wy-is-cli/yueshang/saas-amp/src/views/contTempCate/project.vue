  ...
  <div class="list-page">
    <el-row class="tool-bar" type="flex" justify="space-between" align="middle">
      <el-col :span="16" style="display: flex; align-items: center;">
        <el-input placeholder='请输入项目编号' size="mini" v-model='params.project_code' style="margin-right: 20px; width: 200px;" />
        <el-input placeholder='请输入项目名称' size="mini" v-model='params.name' style="margin-right: 20px; width: 200px;" />
    ...
    <el-table :data="tableData" size='mini' ref="table" :header-cell-style="{background:'#f5f7fa'}">
      ...
      <el-table-column label="项目编号">
        <template slot-scope="scope">{{scope.row.project_code}}</template>
      </el-table-column>
      ...
      <el-table-column label="项目体量(㎡)" prop="dimension" width="100"></el-table-column>
      <el-table-column label="商业面积(㎡)" prop="commercial_area" width="100"></el-table-column>
      ...
      <el-table-column label="操作" width="160" align="center" fixed="right">
        <template slot-scope="scope">
          <div v-if="scope.row.contract_mould_cate_status !== 'DELETED'">
            <el-button size="mini" type="warning" @click="toUpdate(scope.row.number_id)" plain style="color: #e6a23c;">编辑</el-button>
            <el-button size="mini" type="danger" @click="confirm(scope.row.number_id)" plain style="color: #f56c6c;">删除</el-button>
          </div>
          <div v-else>
            <el-button size="mini" type="success" @click="confirm(scope.row.number_id)" plain style="color: #f56c6c;">恢复</el-button>
          </div>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    async confirm_batch () {
      ...
      if (selection.length === 0) {
        this.$message.error(`选择要${type}的项目信息`)
        return
      }
      try {
        await this.$confirm(`确认要${type}吗？`)
    ...
    async getList () {
      let { market_research_name = '', project_code = '', name = '', contract_mould_cate_status} = this.params
      const params = {
        ...this.pageInfo,
        name: name.trim(),
