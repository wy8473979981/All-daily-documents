  ...
  <div class="list-page">
    <el-row class="tool-bar" type="flex" justify="space-between" align="middle">
      ...
      <el-col :span="8" style="text-align: right;">
        <el-button @click="toCreate" size="mini" type="primary">新增权重定价</el-button>
    ...
    <el-table :data="tableData" size='mini' ref="table" :header-cell-style="{background:'#f5f7fa'}">
      ...
      <el-table-column label="竞品权重数量" width="100">
        <template slot-scope="scope">
          <router-link :to="{path:'./weightList', query: {market_id: scope.row.id, layout_floor_type: scope.row.layout_floor_type, type_id: scope.row.type_id}}"><span style="color: #0f96ff;" @click="pId(scope.row.id)">{{scope.row.num}}</span></router-link>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="160" align="center" fixed="right">
        <template slot-scope="scope">
          <div v-if="scope.row.contract_mould_cate_status !== 'DELETED'">
            <el-button size="mini" type="warning" @click="toUpdate(scope.row.id, scope.row.rent_calculation_id, scope.row.pms_project_id)" plain style="color: #e6a23c;">编辑</el-button>
...
export default {
  ...
  methods: {
    ...
    async handelConfirm (id) {
      const params = {id: id}
      let res = await weightedPricing.del(params)
      if (res.code === 200) {
        this.getList()
      }
    },
    ...
    async confirm_batch () {
      ...
      if (selection.length === 0) {
        this.$message.error(`选择要${type}的权重定价`)
        return
      }
      try {
        await this.$confirm(`确认要${type}吗？`)
    ...
    // 编辑权重定价列表 start
    toUpdate (id, rent_calculation_id, pms_project_id) {
      this.$router.push({path: 'weightedPricingUpdate', query: {market_id: id, rent_calculation_id: rent_calculation_id, pms_project_id: pms_project_id}})
    },
    async getList () {
      ...
      const res = await weightedPricing.find(params)
    ...
    // 权重定价列表编辑 start
    toCreate (id, market_research_id, pms_project_id) {
      this.$router.push({path: 'weightedPricingCreate', query: {id:id, market_research_id:market_research_id, pms_project_id:pms_project_id}})
    },
    // pId统一处理更新日期 start
    pId (id) {
