...
<!--        </el-select>-->
    ...
    <el-table ref="table" :data="tableData" size="mini" border header-row-class-name="table-header" :height="tableHeight">
      ...
      <el-table-column min-width="150" label="计租总面积(㎡)" show-overflow-tooltip align="right" >
        <template  slot-scope="scope">
          <span v-if="query.quanchang === 1">{{scope.row.totalSquare.toFixed(1)}}</span>
          <span v-if="query.quanchang === 2">{{scope.row.totalSquare.toFixed(1)}}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="150" label="出租面积(㎡)" show-overflow-tooltip align="right" >
        <template  slot-scope="scope">
          <span v-if="query.quanchang === 1">{{scope.row.rentedSquareQc?scope.row.rentedSquareQc : ''}}</span>
          <span v-if="query.quanchang === 2">{{scope.row.rentedSquareJy?scope.row.rentedSquareJy : ''}}</span>
        </template>
      </el-table-column>
      <el-table-column label="出租率（%）" align="center">
        <el-table-column
          :label="month1 + '月'"
          align="right"
          min-width="100">
...
export default {
  ...
  methods: {
    ...
    async getList () {
      ...
      await TypeApi.occupancy(params).then(res => {
        if (res.code === 200 && res.result) {
    ...
    exportExcel () {
      ...
      if (token) {
        window.location.href = `../${this.$baseURL}/projectConfig/cl_month_compare_rent_rate_export?token=${token}&quanchang=${this.query.quanchang}`
      }
    }
  }
}
