  ...
  <div class="list-page">
    ...
    <el-row class="tool-bar" type="flex" justify="space-between" align="middle" style="flex-flow: wrap;">
      <el-col class="colcol" :span="22">
        ...
        <div>
          <el-select placeholder="出租/经营/自用" size="mini" v-model="query.status_six" @change="getListChange" multiple collapse-tags clearable>
            <el-option
    ...
    <el-table ref="table" :data="tableData" size="mini" border header-row-class-name="table-header" :max-height="this.$store.state.tableHeight">
      ...
      <el-table-column min-width="120" label="房间数" prop="room_num" align="right">
      </el-table-column>
      <el-table-column min-width="120" label="建筑面积(㎡)" prop="build_area" align="right">
      </el-table-column>
      <el-table-column min-width="120" label="本月实际总收入(元)" prop="month_real_rent" align="right">
      </el-table-column>
      <el-table-column min-width="120" label="本月实际业主利润(元)" prop="month_owner_profit" align="right">
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    // 获取存量项目列表
    getProList() {
      TypeApi.projectDropDown({project_type: 1, layout: 2}).then(res => {
    ...
    // 业态下拉数据
    getDownList() {
      StockAssets.smallLayoutDropList({big_layout_id: 2}).then(res => {
        if(res.code === 200 && res.result){
        ...
        if(res.code === 200 && res.result){
          ...
          if (res.result.data.res_sum[0]) {
            this.tableData.push({
              ...
              month_owner_profit: res.result.data.res_sum[0].month_owner_profit_sum,
            })
          }
    ...
    exportExcel() {
      ...
      if(token) {
        ...
        console.log(`token=${token}&project_config_id=${this.query.project_config_id}&type=${this.big_layout_id}&report_time=${this.query.report_time}&excel_type=2`)
