<template>
  ...
  <div class="list-page">
    ...
    <el-table ref="table" :data="tableData" size="mini" border header-row-class-name="table-header" :max-height="this.$store.state.tableHeight">
      <el-table-column fixed min-width="140" label="业态" prop="layout" align="center">
      </el-table-column>
      <el-table-column width="340" label="资产类别">
      ...
      <el-table-column min-width="80" label="办公楼号" prop="building_no" align="center">
      </el-table-column>
      <el-table-column min-width="80" label="房间号" show-overflow-tooltip prop="room_no" align="right">
      </el-table-column>
      <el-table-column min-width="60" label="套数(个)" prop="apartment_num" align="right">
      </el-table-column>
      <el-table-column min-width="100" label="建筑面积(㎡)" prop="build_area" align="right">
      </el-table-column>
      <el-table-column min-width="130" label="本月实际租金单价(元)" prop="month_rent_unit_price" align="right">
      </el-table-column>
      <el-table-column min-width="80" label="出租率(%)" prop="rent_rate" align="right">
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    // 获取存量项目列表
    getProList() {
      TypeApi.projectDropDown({project_type: 1, layout: 3}).then(res => {
    ...
    // 业态下拉数据
    getDownList() {
      StockAssets.smallLayoutDropList({big_layout_id: 3}).then(res => {
        if(res.code === 200 && res.result){
        ...
        if(res.code === 200 && res.result){
          ...
          if (res.result.data.res_sum[0]) {
            this.tableData.push({
              ...
              apartment_num: res.result.data.res_sum[0].apartment_num_sum,
              month_rent_unit_price: res.result.data.res_sum[0].month_rent_unit_price_sum,
            })
          }
