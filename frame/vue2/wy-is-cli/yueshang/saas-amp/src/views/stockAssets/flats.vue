<template>
  ...
  <div class="list-page">
    ...
    <el-table ref="table" :data="tableData" size="mini" border header-row-class-name="table-header" :max-height="this.$store.state.tableHeight">
      ...
      <el-table-column min-width="120" label="公寓楼号" prop="apartment_no" align="center">
      </el-table-column>
      <el-table-column min-width="120" label="产证编号" show-overflow-tooltip prop="dev_code_no" align="center">
      </el-table-column>
      <el-table-column min-width="100" label="房间号" show-overflow-tooltip prop="room_no" align="right">
      </el-table-column>
      <el-table-column min-width="80" label="套数(个)" prop="apartment_num" align="right">
      </el-table-column>
      <el-table-column min-width="90" label="建筑面积(㎡)" prop="build_area" align="right">
      </el-table-column>
      <el-table-column min-width="140" label="本月实际租金单价(元)" prop="month_rent_unit_price" align="right">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 获取存量项目列表
    getProList() {
      TypeApi.projectDropDown({project_type: 1, layout: 4}).then(res => {
    ...
    // 业态下拉数据
    getDownList() {
      StockAssets.smallLayoutDropList({big_layout_id: 4}).then(res => {
        if(res.code === 200 && res.result){
