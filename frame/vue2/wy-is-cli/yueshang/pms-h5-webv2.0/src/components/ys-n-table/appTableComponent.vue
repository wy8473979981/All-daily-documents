  ...
  <div class="app-table">
    ...
    <div class="app-table-header">
      <div class="app-table-header__items">
        <div v-for="(column, columni) in component.columns" :key="columni" :class="'app-table-header__item ' + ( columni < fixednum ? 'fixed' : '' ) + ' ' + ( column.groupName ? 'in-group' : '' ) + ' span-' + column.align + ' ' + ( column.sortable ? 'sortable' : '' )" @click="onHeadColumnClick" :data-column="JSON.stringify(column)" :style="column.$styles">
          <div class="app-table-header__item__group" v-if="column.groupName">
            <div class="app-table-header__item__group-name">
              {{ column.groupIndex === 0 ? column.groupName : '' }}
            </div>
          </div>
          <div class="app-table-header__item__column-name">{{ column.label }}</div>
    ...
    <div class="app-table-rows">
      ...
      <template v-else>
        <div class="app-table-row app-table-row--total" v-if="component.totalRow">
          <div v-for="(column, columni) in component.columns" :key="columni" :class="'app-table-row__item ' + ( columni < fixednum ? 'fixed' : '' ) + ' span-' + column.align + ' ' + ( column.sortable ? 'sortable' : '' )" :style="column.$styles + ';'">
            <span :style="column.key && component.totalRow[column.key].toString().indexOf('-') === 0 && component.totalRow[column.key] != '-'? 'color:#FF4954':''">{{ column.key ? component.totalRow[column.key] : '' }}</span>
        ...
        <div v-for="(row, rowi) in component.rows" :key="rowi" class="app-table-row">
          <div v-for="(column, columni) in component.columns" :key="columni" @click="onRowColumnClick(rowi,row,column)" :data-index="rowi" :data-row="row" :data-column="column" :style="column.$styles + ';color: ' + column.color + ';'" :class="'app-table-row__item ' + ( columni < fixednum ? 'fixed' : '' ) + ' span-' + (column.align || 'center') + ' ' + ( column.sortable ? 'sortable' : '' ) + ' ' + ( row['$selected']?(row['$selected'][column.key] ? 'selected' : '' ): '')">
            <span :style="column.key && row[column.key].toString().indexOf('-') === 0 && row[column.key] != '-'? 'color:#FF4954':''">{{ column.key ? row[column.key] : rowi + 1 }}</span>
    ...
    totalRow: {
      handler: function (newValue) {
        if (newValue) {
          ...
          // console.log(this.component.totalRow, 'this.component.totalRow')
        }
      },
      immediate: true,
      deep: true
