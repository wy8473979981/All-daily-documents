<template>
  <div class="select-project-list">
    ...
    <div class="index-anchor">
      <span :class="'index-anchor-item' + (activeIndex==item.key?' index-anchor-item-active':'') " v-for="item in projectList" :key="item.key" @click="toTegional(item.key)" @touchstart="toTegional(item.key)">{{ item.key }}</span>
    </div>
  </div>
</template>
<script>
    ...
    async getProjectList () {
      try {
        ...
        if (res.code == 0) {
          ...
          this.setWatermarkText(res.data.userInfo.acct.userName)
