<template>
  <div :class="'shouru-zujin ' + (recovery.show ? 'recovery' : '')" v-webTitle :data-title="`招商信息`">
...
export default {
  ...
  methods: {
    ...
    dateSelected (date) {
      ...
      this.setData({
        ["params.yearMonth"]: date.split('-')[0] + '-' + date.split('-')[1]
      });
      this.getEchartData();
      this.getTableData();
      console.log(date);
      ...
      if (column.key === "name") {
        ...
        this.$router.push({
          ...
          query: {
            ...
            chargeType: this.params.chargeType
          },
        })
        return;
      } else if (column.key === "emptyRentSquare") {
        let projectId = e.detail.row.id;
        let projectName = e.detail.row.name;
        this.$router.push({
