  ...
  methods: {
    ...
    changeSpan(e) {
      try {
        ...
        if (mode == '1') {
          // 年
          let data = this.tableData.passengerFlowWhenCurYearForPassengerFlowComparisonList.slice(0, -1);
          let projectList = data.map((item, index) => {
            return { ...item, ranking: index, label: item.projectName };
          });
          let dataList = data.filter((item) => { return this.params.ids.includes(item.projectId); });
          this.setData({ "dataList": dataList, "projectList": projectList, "cross": yearColumns });
        } else if (mode == '0') {
          // 月
          let data = this.tableData.passengerFlowWhenCurMonthForPassengerFlowComparisonList.slice(0, -1);
          let projectList = data.map((item, index) => {
            return { ...item, ranking: index, label: item.projectName };
          });
          let dataList = data.filter((item) => { return this.params.ids.includes(item.projectId); });
          this.setData({ "dataList": dataList, "projectList": projectList, "cross": monthColumns });
        } else if (mode == '2') {
          // 日
          let data = this.tableData.passengerFlowWhenCurDayForPassengerFlowComparisonList.slice(0, -1);
          let projectList = data.map((item, index) => {
            return { ...item, ranking: index, label: item.projectName };
          });
          let dataList = data.filter((item) => { return this.params.ids.includes(item.projectId); });
          this.setData({ "dataList": dataList, "projectList": projectList, "cross": dayColumns });
        }
      } catch (e) {
        console.log(e);
      }
