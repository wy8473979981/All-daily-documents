<template>
  ...
  <div class="yusuan-zfgk" v-webTitle :data-title="`租费概况`">
        ...
        <ys-n-section :hasTable="true">
          <div class="head-actions_left_div" slot="head-actions_left">
            <div class="selector" @click="openSelector">
              <div class="search-title">租费概况</div>
              <van-image class="van-image" :src="searchIcon" />
            </div>
          </div>
          <ys-n-table :fixednum="3" :columns="page.table.columns" :values="page.table.rows" :total-row="page.table.totalRow"></ys-n-table>
        </ys-n-section>
      </van-pull-refresh>
  ...
  methods: {
    ...
    loadData: async function (isUse = false) {
      try {
        ...
        await this.$axios.yusuanServe.getZfgkSheet(this.query)
          .then((res) => {
            if (res.code == 0) {
