<template>
  ...
  <div class="list-page wsn-page">
    <el-row class="tool-bar" type="flex" justify="space-between" align="middle" style="flex-flow: wrap;">
      <!-- <el-col class="select-title" :span="24">
        <div class="select-cutover">
          <span v-if="routerQuery.bunk" @click="$router.push({path: '/management/plait/lease/index',query:routerQuery})">租费收入-商业</span>
          <span v-if="routerQuery.mutibiz" class="select-blue">多经收入-商业</span>
		      <span v-if="routerQuery.office" @click="$router.push({path: '/management/plait/lease/houseMassage',query:routerQuery})">住宅/写字楼</span>
          <span v-if="routerQuery.proplan" @click="$router.push({path: '/management/plait/feeSchedule/index',query:routerQuery})">管理及企划费用表</span>
          <span v-if="routerQuery.mandate" @click="$router.push({path: '/management/plait/otherIncome',query:routerQuery})">其他收入表</span>
        </div>
      ...
      <!-- <el-col class="select-title" :span="24">
        <div class="select-cutover">
          <span class="select-blue">多经收入-商业</span>
          <span @click="$router.push({path: '/management/plait/income/wsngk',query:routerQuery})">多经概况</span>
        </div>
      </el-col> -->
      <el-col :span="20">
    ...
    <el-row>
      ...
      <el-button size="mini" type="text" @click="$router.push({path: '/management/plait/summary/wsngk',query:routerQuery})">多经概况</el-button>
    </el-row>
