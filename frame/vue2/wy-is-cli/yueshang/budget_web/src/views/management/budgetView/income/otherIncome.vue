<template>
  ...
  <div class="list-page">
    ...
    <el-table class="table-input-box" ref="table" :data="tableData" size="mini" border stripe header-row-class-name="table-header" :height="this.$store.state.tableHeight" :cell-style="{ padding: '4px 0' }">
      <el-table-column fixed min-width="140" label="业态" show-overflow-tooltip prop="subjectList">
        <template slot-scope="scope">
          <p @click="showInput(scope.$index)">{{ getSubjectName(scope,1) }}</p>
        </template>
      </el-table-column>
      <el-table-column fixed min-width="140" label="类别" show-overflow-tooltip prop="subjectList">
        <template slot-scope="scope">
      ...
      <el-table-column min-width="170" :label="`${getBudgetYear+1}年度预算合计`" show-overflow-tooltip prop="nextYtd" align="right">
        <template slot-scope="scope">{{ returnValue(scope.row,'nextYtd',true,false) }}</template>
      </el-table-column>
      <el-table-column min-width="170" :label="`${getBudgetYear+2}年度预算合计`" show-overflow-tooltip prop="nextYtd" align="right">
        <template slot-scope="scope">{{ returnValue(scope.row,'nextYtd',true,false) }}</template>
      </el-table-column>
