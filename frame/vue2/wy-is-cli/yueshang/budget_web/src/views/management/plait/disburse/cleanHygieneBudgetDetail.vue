    ...
    <el-row class="tool-bar" type="flex" justify="space-between" align="middle" style="flex-flow: wrap;">
      <el-col class="select-title" :span="20">
        <div class="select-cutover">
          <span class="select-blue">清洁卫生费明细</span>
          <span @click="$router.push({path: '/management/plait/disburse/cleanHygieneBudget',query:toRouter})">清洁卫生费</span>
        </div>
      </el-col>
      <el-col :span="16">
        <!-- <el-select v-if="!isEdit" placeholder="业态" size="mini" @change="bizTypeSelectFunc" v-model="query.bizTypeName" value-key="subjectId" filterable clearable style="margin-right: 5px; width: 180px;">
          <el-option v-for="item in selectLayoutList" :key="item.subjectId" :label="item.subjectName" :value="item">
          </el-option>
        </el-select>
        <el-select v-if="!isEdit" placeholder="级别" size="mini" @change="search" v-model="query.level" filterable clearable style="margin-right: 5px; width: 180px;">
    ...
    <el-table class="table-input-box" ref="table" :data="tableData" size="mini" border stripe header-row-class-name="table-header" :height="this.$store.state.tableHeight" :cell-style="{ padding: '4px 0' }">
      ...
      <el-table-column min-width="140" label="履约起始期" :show-overflow-tooltip="!isEdit" prop="start">
        <template slot-scope="scope">
          ...
          <p v-else @click="showInput(scope.$index)">{{ returnValue(scope.row,'start',true,true,'subject') }}</p>
        </template>
      </el-table-column>
      ...
      <el-table-column min-width="140" label="合约期限(月)" :show-overflow-tooltip="!isEdit" prop="term" align="right">
        <template slot-scope="scope">
      ...
      <el-table-column min-width="140" label="核对列" :show-overflow-tooltip="!isEdit" prop="checkCol" align="right">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    async getFormId() {
      try {
        // 查询
        const params = { detailId: this.routerQuery.budgetInstanceSheetId, feeType: 1 };
