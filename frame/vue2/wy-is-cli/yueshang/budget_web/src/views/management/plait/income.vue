<template>
  ...
  <div class="list-page plait">
    <el-row class="tool-bar">
      ...
      <el-row class="page-txt">
        <el-col :span="16">
          <el-cascader placeholder="请输入关键字搜索" size="mini" :show-all-levels="false" :options="options" ref="cascaderAddr" @change="search" v-model="query.instanceId" filterable style="margin-right: 5px; width: 380px;"></el-cascader>
      ...
      <el-row class="page-txt-tab">
        <el-col class="select-title" :span="20">
          <div class="select-cutover">
            <span class="select-blue">收入</span>
            <span @click="$router.push({path: '/management/plait/disburse/index',query:routerQuery})">费用支出</span>
            <span @click="$router.push({path: '/management/plait/summary/index',query:routerQuery})">汇总</span>
          </div>
        </el-col>
      </el-row>
    </el-row>
    <el-table ref="table" :data="tableData" size="mini" border stripe header-row-class-name="table-header" :height="this.$store.state.tableHeight">
      ...
      <el-table-column min-width="150" show-overflow-tooltip label="收入表" prop="name">
        <template slot-scope="scope">
...
import { toThousandsClean, getDynamicTableHeight, rTime, chartGather } from '@/utils/index'
...
export default {
  ...
  methods: {
    ...
    async getList() {
      try {
        const params = {
          ...
          ...this.query,
          instanceId: this.query.instanceId && this.query.instanceId[2],
        }
        await PlaitApi.find(params).then(res => {
          if (res.code == 0) {
            ...
            this.setInstanceId({ instanceId: data.instanceIds });
            this.query.instanceId = data.instanceIds;
            this.setCompanyName({ companyName: data.instanceName });


            // let all = data.sum.all;
            // let commit = data.sum.commit;
            // let forCommit = data.sum.forCommit;
            // if (all > commit) {
            //   this.formNum = `总共${all}张，其中${commit}张已提交，${forCommit}张待提交，只有全部提交才能进行审批`;
            // } else {
            //   this.formNum = `总共${all}张，已全部提交审批`;
            // }
            setTimeout(() => {
    ...
    goDetail(item) {
      try {
        const routerPath = this.chartGather;
        const annual = item.annual; //预算年度
        ...
        const chargeType = budgetSheetId == 63 || budgetSheetId == 65 ? null : 5;
        this.setBudgetYear({ budgetYear: annual });//预算年度
        if (budgetInstanceSheetId && budgetSheetId) {
          if (routerPath[budgetSheetId][1]) {
            if (budgetSheetId >= 63 && budgetSheetId <= 66) {
              const query = { budgetInstanceSheetId: budgetInstanceSheetId, budgetInstanceId: budgetInstanceId, budgetSheetId: budgetSheetId, annual: annual, projectId: projectId, chargeType: chargeType }
              this.$router.push({ path: routerPath[budgetSheetId] && routerPath[budgetSheetId][1], query: query });
            } else {
              const query = { budgetInstanceSheetId: budgetInstanceSheetId, budgetSheetId: budgetSheetId, annual: annual, projectId: projectId }
              this.$router.push({ path: routerPath[budgetSheetId] && routerPath[budgetSheetId][1], query: query });
            }
          } else {
    ...
    search() {
      ...
      this.setInstanceId({ instanceId: this.query.instanceId });
      this.pageInfo.pageNum = 1;
      this.getList();
    },
    reset() {
      this.query.instanceId = null;
      this.pageInfo.pageNum = 1;
