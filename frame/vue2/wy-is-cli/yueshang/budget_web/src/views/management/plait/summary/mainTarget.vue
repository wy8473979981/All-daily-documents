<template>
  ...
  <div class="list-page">
    ...
    <el-table class="table-input-box" ref="table" :data="tableData" size="mini" border stripe header-row-class-name="table-header" :height="this.$store.state.tableHeight" :cell-style="{ padding: '4px 0' }">
      <el-table-column fixed min-width="120" label="业态" show-overflow-tooltip prop="projectType">
        <template slot-scope="scope">
          <p>{{ getSubjectName(scope,1) }}</p>
        </template>
      </el-table-column>
      <el-table-column fixed min-width="120" label="主要指标" show-overflow-tooltip prop="subjectName">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'subjectName',true,false,'projectType') }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="170" :label="`${getBudgetYear-1}年度预计发生额`" :show-overflow-tooltip="!isEdit" prop="preYearBudget" align="right">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'preYearBudget',true,false,'projectType') }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="140" :label="`${getBudgetYear-1}年度预算合计`" :show-overflow-tooltip="!isEdit" prop="preYearBudgetResult" align="right">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'preYearBudgetResult',true,false,'projectType') }}</p>
        </template>
      </el-table-column>
      <el-table-column width="5" class-name="common-column-line"></el-table-column>
      <el-table-column :label="`${getBudgetYear}年预算`" align="center">
        <el-table-column min-width="140" label="一季度" :show-overflow-tooltip="!isEdit" prop="budgetQaOne" align="right">
          <template slot-scope="scope">
            <p>{{ returnValue(scope.row,'budgetQaOne',true,false,'projectType') }}</p>
          </template>
        </el-table-column>
        <el-table-column min-width="140" label="二季度" :show-overflow-tooltip="!isEdit" prop="budgetQaTwo" align="right">
          <template slot-scope="scope">
            <p>{{ returnValue(scope.row,'budgetQaTwo',true,false,'projectType') }}</p>
          </template>
        </el-table-column>
        <el-table-column min-width="140" label="三季度" :show-overflow-tooltip="!isEdit" prop="budgetQaThree" align="right">
          <template slot-scope="scope">
            <p>{{ returnValue(scope.row,'budgetQaThree',true,false,'projectType') }}</p>
          </template>
        </el-table-column>
        <el-table-column min-width="140" label="四季度" :show-overflow-tooltip="!isEdit" prop="budgetQaFour" align="right">
          <template slot-scope="scope">
            <p>{{ returnValue(scope.row,'budgetQaFour',true,false,'projectType') }}</p>
          </template>
        </el-table-column>
        <el-table-column min-width="140" label="合计" :show-overflow-tooltip="!isEdit" prop="budgetTotal" align="right">
          <template slot-scope="scope">
            <p>{{ returnValue(scope.row,'budgetTotal',true,false,'projectType') }}</p>
          </template>
        </el-table-column>
      </el-table-column>
      <el-table-column width="5" class-name="common-column-line"></el-table-column>
      <el-table-column min-width="140" :label="`${getBudgetYearNumber}VS${getBudgetYearNumber-1}增长率`" :show-overflow-tooltip="!isEdit" prop="increasePre" align="right">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'increasePre',true,false,'projectType') }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="140" :label="`${getBudgetYear+1}年预算`" :show-overflow-tooltip="!isEdit" prop="nextYearBudget" align="right">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'nextYearBudget',true,false,'projectType') }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="140" :label="`${getBudgetYearNumber+1}VS${getBudgetYearNumber}递增率`" :show-overflow-tooltip="!isEdit" prop="increaseNext" align="right">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'increaseNext',true,false,'projectType') }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="140" :label="`${getBudgetYear+2}年预算`" :show-overflow-tooltip="!isEdit" prop="nextTwoYearBudget" align="right">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'nextTwoYearBudget',true,false,'projectType') }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="140" :label="`${getBudgetYearNumber+2}VS${getBudgetYearNumber+1}递增率`" :show-overflow-tooltip="!isEdit" prop="increaseNextTwo" align="right">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'increaseNextTwo',true,false,'projectType') }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="200" label="备注" :show-overflow-tooltip="!isEdit" prop="remark">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'remark',false,false,'projectType') }}</p>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  computed: {
    ...
    importParams() {
      return {
        importUrl: '/collecttarget/targetImp',
  ...
  methods: {
    ...
    async getList() {
      ...
      this.isEdit = false;
      await SummaryApi.mainTargetList(params).then((res) => {
        if (res.code === 0) {
          this.tableData = []
