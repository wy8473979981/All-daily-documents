<template>
  ...
  <div class="list-page">
    <el-row class="tool-bar" type="flex" justify="space-between" align="middle" style="flex-flow: wrap;">
      <el-col class="select-title" :span="20">
        <div class="select-cutover">
          <span @click="$router.push({path: '/management/budgetView/summary/staffCost',query:toRouter})">人事费用</span>
          <span class="select-blue">人数概况</span>
        </div>
      </el-col>
      <el-col :span="16">
    ...
    <el-table class="table-input-box" ref="table" :data="tableData" size="mini" border stripe header-row-class-name="table-header" :height="this.$store.state.tableHeight" :cell-style="{ padding: '4px 0' }">
      ...
      <el-table-column min-width="120" label="类别" show-overflow-tooltip prop="empType">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'empType',false,false,'projectType') }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="120" label="编制人数" show-overflow-tooltip prop="empStaffing" align="right">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'empStaffing',true,false,'projectType',0) }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="120" label="实际到岗" show-overflow-tooltip prop="empReal" align="right">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'empReal',true,false,'projectType',0) }}</p>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  created() {
    ...
    this.getPersonId().then(res => {
      if (res.code === 0) {
        ...
        this.toRouter = { ...this.routerQuery, budgetInstanceSheetId: res.data[0] };
      }
    })
  },
  mounted() {
  ...
  methods: {
    ...
    async getList() {
      ...
      this.isEdit = false;
      await SummaryApi.personSurveyList(params).then((res) => {
        if (res.code === 0) {
          this.tableData = []
