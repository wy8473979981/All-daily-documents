      ...
      <el-row class="page-txt-tab">
        <el-col class="select-title" :span="20">
          <div class="select-cutover">
            ...
            <span @click="$router.push({path: '/management/budgetView/disburse/index',query:routerQuery})">费用支出</span>
        ...
        if (budgetInstanceSheetId && budgetSheetId) {
          if (routerPath[budgetSheetId][1]) {
            if (budgetSheetId >= 63 && budgetSheetId <= 66) {
              const query = { budgetInstanceSheetId: budgetInstanceSheetId, budgetInstanceId: budgetInstanceId, budgetSheetId: budgetSheetId, annual: annual, projectId: projectId, chargeType: chargeType, companyId: companyId }
              this.$router.push({ path: routerPath[budgetSheetId] && routerPath[budgetSheetId][1], query: query });
            } else {
              const query = { budgetInstanceSheetId: budgetInstanceSheetId, budgetSheetId: budgetSheetId, annual: annual, projectId: projectId, companyId: companyId }
              this.$router.push({ path: routerPath[budgetSheetId] && routerPath[budgetSheetId][1], query: query });
            }
          } else {
