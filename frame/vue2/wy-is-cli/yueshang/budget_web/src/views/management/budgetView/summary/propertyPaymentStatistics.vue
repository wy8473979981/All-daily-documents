<template>
  ...
  <div class="list-page">
    ...
    <el-table class="table-input-box" ref="table" :data="tableData" size="mini" border stripe header-row-class-name="table-header" :height="this.$store.state.tableHeight" :cell-style="{ padding: '4px 0' }">
      <el-table-column fixed min-width="120" label="类别" show-overflow-tooltip>
        <template slot-scope="scope">
          <p>{{ getSubjectName(scope,1) }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="120" label="项目" show-overflow-tooltip prop="project">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'project',false,false,'category') }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="120" label="住在+写字楼" show-overflow-tooltip prop="rbToatal" align="right">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'rbToatal',true,false,'category') }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="120" label="住宅" show-overflow-tooltip prop="residence" align="right">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'residence',true,false,'category') }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="100" label="写字楼" show-overflow-tooltip prop="building" align="right">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'building',true,false,'category ') }}</p>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      this.isEdit = false;
      await SummaryApi.propertystatList(params).then((res) => {
        if (res.code === 0) {
          this.tableData = []
