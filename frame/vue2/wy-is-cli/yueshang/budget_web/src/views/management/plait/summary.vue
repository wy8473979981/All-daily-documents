  ...
  <div class="list-page plait">
    <el-row class="tool-bar">
      <el-row class="page-txt">
        <el-col :span="16">
          <el-cascader placeholder="请输入关键字搜索" size="mini" :show-all-levels="false" :options="options" ref="cascaderAddr" @change="search" v-model="query.instanceId" filterable clearable style="margin-right: 5px; width: 380px;"></el-cascader>
      ...
      <el-row class="page-txt-tab">
        <el-col class="select-title" :span="20">
          <div class="select-cutover">
            <span @click="$router.push({path: '/management/plait/income/index',query:routerQuery})">收入</span>
            <span @click="$router.push({path: '/management/plait/disburse/index',query:routerQuery})">费用支出</span>
            <span class="select-blue">汇总</span>
          </div>
        </el-col>
      </el-row>
    </el-row>
    <el-table ref="table" :data="tableData" size="mini" border stripe header-row-class-name="table-header" :height="this.$store.state.tableHeight">
      ...
      <el-table-column min-width="150" show-overflow-tooltip label="汇总表" prop="name">
        <template slot-scope="scope">
      ...
      try {
        ...
        if (budgetInstanceSheetId && budgetSheetId) {
          if (routerPath[budgetSheetId][1]) {
            const query = { budgetInstanceSheetId: budgetInstanceSheetId, budgetInstanceId: budgetInstanceId, budgetSheetId: budgetSheetId, annual: annual, projectId: projectId }
