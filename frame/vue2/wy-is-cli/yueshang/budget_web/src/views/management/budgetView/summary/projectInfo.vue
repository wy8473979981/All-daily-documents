<template>
  ...
  <div class="list-page">
    <el-row class="tool-bar" type="flex" justify="space-between" align="middle" style="flex-flow: wrap;">
      <el-col class="select-title" :span="20">
        <div class="select-cutover">
          <span @click="$router.push({path: '/management/budgetView/summary/areaInfo',query:toAreaRouter})">面积信息</span>
          <span class="select-blue">项目信息</span>
          <span @click="$router.push({path: '/management/budgetView/summary/personInfo',query:toPersonRouter})">人员信息</span>
        </div>
      </el-col>
      <el-col :span="16">
    ...
    <el-table class="table-input-box" ref="table" :data="tableData" size="mini" border stripe header-row-class-name="table-header" :height="this.$store.state.tableHeight" :cell-style="{ padding: '4px 0' }">
      <el-table-column fixed min-width="120" label="项目" show-overflow-tooltip>
        <template slot-scope="scope">
          <p>{{ getSubjectName(scope,1) }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="120" label="面积" :show-overflow-tooltip="!isEdit" prop="projectArea" align="right">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'projectArea',true,false,'projectName') }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="120" label="配置人数" :show-overflow-tooltip="!isEdit" prop="projectEmp" align="right">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'projectEmp',true,false,'projectName',0) }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="120" label="单价（元/月/人）" :show-overflow-tooltip="!isEdit" prop="projectEmpSalary" align="right">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'projectEmpSalary',true,false,'projectName') }}</p>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  computed: {
    ...
    importParams() {
      return {
        importUrl: '/collectbaseProject/baseProjectImp',
  ...
  methods: {
    ...
    async getList() {
      ...
      this.isEdit = false;
      await SummaryApi.projectInfoList(params).then((res) => {
        if (res.code === 0) {
          this.tableData = []
