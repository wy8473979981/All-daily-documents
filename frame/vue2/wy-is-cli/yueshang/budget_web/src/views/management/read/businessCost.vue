  ...
  <div class="list-page">
    <el-row class="tool-bar" type="flex" justify="space-between" align="middle" style="flex-flow: wrap;">
      <el-col class="select-title">
        <div class="select-cutover">
          <span @click="$router.push({path: '/management/read/totalRevenue',query: {areaId: routerParam.areaId,projectId: routerParam.projectId}})">总收入</span>
          <span class="select-blue">经营费用</span>
    ...
    <el-table ref="table" :data="tableData" size="mini" border stripe header-row-class-name="table-header" :height="this.$store.state.tableHeight">
      ...
      <el-table-column fixed="right" width="100" label="对应业务表">
        <template slot-scope="scope">
          <el-button type="text" size="mini" @click="goMember(scope.row.id)">人事费用表</el-button>
...
export default {
  ...
  methods: {
    ...
    async getList () {
      // 查询-列表
      const params = {
        type: 4, // 类型1总收入2经营费用3业主费用4商业利润5管理及企划费用表6其他收入表7开办项目增补项
        ...this.pageInfo,
        ...this.query,
        ...this.routerParam
      }
