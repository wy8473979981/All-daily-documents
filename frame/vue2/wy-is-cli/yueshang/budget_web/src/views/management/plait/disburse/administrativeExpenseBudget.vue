<template>
  ...
  <div class="list-page">
    <el-row class="tool-bar" type="flex" justify="space-between" align="middle" style="flex-flow: wrap;">
      <el-col :span="12">
        <el-select v-if="!isEdit" placeholder="请选择业态" size="mini" @change="bizTypeSelectFunc" v-model="query.bizTypeName" value-key="subjectId" filterable clearable style="margin-right: 5px; width: 180px;">
          <el-option v-for="item in paramFirstList" :key="item.subjectId" :label="item.subjectName" :value="item">
          </el-option>
        </el-select>
    ...
    <el-table class="table-input-box" ref="table" :data="tableData" size="mini" border stripe header-row-class-name="table-header" :height="this.$store.state.tableHeight" :cell-style="{ padding: '4px 0' }">
      ...
      <el-table-column fixed min-width="150" label="费项" show-overflow-tooltip prop="subjectList.subSubject.subSubject.subjectName">
        <template slot-scope="scope">
          <p @click="showInput(scope.$index)">{{ scope.row.subjectList.subjectName == '合计' ? '/' : getSubjectName(scope) }}</p>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 获取级别列表
    getSelectList() {
      GlobalApi.getSelectList({ selectType: 'bizTypeCategory' }).then(res => {
        if (res.code == 0) {
