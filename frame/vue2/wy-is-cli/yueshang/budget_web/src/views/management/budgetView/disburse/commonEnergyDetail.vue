<template>
  ...
  <div class="list-page">
    <el-row class="tool-bar" type="flex" justify="space-between" align="middle" style="flex-flow: wrap;">
      <el-col class="select-title" :span="20">
        <div class="select-cutover">
          <span class="select-blue">能耗明细</span>
          <span @click="$router.push({path: '/management/budgetView/disburse/commonEnergySummary',query:toSummaryRouter})">能耗汇总</span>
        </div>
      </el-col>
      <el-col :span="16">
    ...
    <el-table class="table-input-box" ref="table" :data="tableData" size="mini" border stripe header-row-class-name="table-header" :height="this.$store.state.tableHeight" :cell-style="{ padding: '4px 0' }">
      ...
      <el-table-column min-width="180" label="用电分类" show-overflow-tooltip prop="subjectList">
        <template slot-scope="scope">
          <p @click="showInput(scope.$index)">{{ scope.row.subjectList.subjectName == '合计' ? '/' : getSubjectName(scope,4) }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="180" label="用电设备" show-overflow-tooltip prop="subjectList">
        <template slot-scope="scope">
          <p @click="showInput(scope.$index)">{{ scope.row.subjectList.subjectName == '合计' ? '/' : getSubjectName(scope,5) }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="260" label="设备明细" show-overflow-tooltip prop="subjectList">
        <template slot-scope="scope">
          <p @click="showInput(scope.$index)">{{ scope.row.subjectList.subjectName == '合计' ? '/' : getSubjectName(scope,6) }}</p>
        </template>
      </el-table-column>
...
export default {
  ...
  created() {
    ...
    this.getEnergyItemBysum().then(res => {
      if (res.code === 0) {
        ...
        this.toSummaryRouter = { ...this.routerQuery, ...obj };
      }
    })
  },
  mounted() {
  ...
  methods: {
    ...
    async getEnergyItemBysum() {
      try {
        // 查询
        const params = { itemInstanceSheetId: this.routerQuery.budgetInstanceSheetId };
    ...
    getSubjectName(scope, level) {
      ...
      switch (level) {
        case 1:
          ...
          break
        case 2:
          return row.subjectList && row.subjectList.subSubject && row.subjectList.subSubject && row.subjectList.subSubject.subjectName || '-'
          break
        case 3:
          ...
          break
        case 4:
          return row.subjectList && row.subjectList.subSubject && row.subjectList.subSubject.subSubject && row.subjectList.subSubject.subSubject.subSubject && row.subjectList.subSubject.subSubject.subSubject.subjectName || '-'
          break
        case 5:
          return row.subjectList && row.subjectList.subSubject && row.subjectList.subSubject.subSubject && row.subjectList.subSubject.subSubject.subSubject && row.subjectList.subSubject.subSubject.subSubject.subSubject && row.subjectList.subSubject.subSubject.subSubject.subSubject.subjectName || '-'
          break
        case 6:
          return row.subjectList && row.subjectList.subSubject && row.subjectList.subSubject.subSubject && row.subjectList.subSubject.subSubject.subSubject && row.subjectList.subSubject.subSubject.subSubject.subSubject && row.subjectList.subSubject.subSubject.subSubject.subSubject.subSubject && row.subjectList.subSubject.subSubject.subSubject.subSubject.subSubject.subjectName || '-'
          break
        default:
          return '-'
      }
