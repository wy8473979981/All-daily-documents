<template>
  ...
  <div class="list-page">
    <el-row class="tool-bar" type="flex" justify="space-between" align="middle" style="flex-flow: wrap;">
      <el-col :span="16">
        <el-select v-if="!isEdit" placeholder="请选择级别" size="mini" @change="search" v-model="query.liveType" filterable clearable style="margin-right: 5px; width: 180px;">
    ...
    <el-table class="table-input-box" ref="table" :data="tableData" size="mini" border stripe header-row-class-name="table-header" :height="this.$store.state.tableHeight" :cell-style="{ padding: '4px 0' }">
      <el-table-column fixed min-width="100" label="类别" show-overflow-tooltip prop="category">
      ...
      <el-table-column fixed min-width="140" label="项目" show-overflow-tooltip prop="project">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'project',true,false,'category') }}</p>
        </template>
      </el-table-column>
      <el-table-column fixed min-width="140" label="用电分类" show-overflow-tooltip prop="classify">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'classify',true,false,'category') }}</p>
        </template>
      </el-table-column>
      <el-table-column fixed min-width="140" label="用电设备" show-overflow-tooltip prop="device">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'device',true,false,'category') }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="180" :label="`${getBudgetYear-1}年度预计发生额`" :show-overflow-tooltip="!isEdit" prop="estimateQuota" align="right">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'estimateQuota',true,false,'category') }}</p>
        </template>
      </el-table-column>
      <el-table-column min-width="140" :label="`${getBudgetYear}年度预算合计`" :show-overflow-tooltip="!isEdit" prop="expectTotal" align="right">
        <template slot-scope="scope">
      ...
      <el-table-column :label="`${getBudgetYear}年能耗预算`" align="center">
        <el-table-column min-width="140" label="MALL" :show-overflow-tooltip="!isEdit" prop="expectMall" align="right">
          <template slot-scope="scope">
            <p>{{ returnValue(scope.row,'expectMall',true,false,'category') }}</p>
          </template>
        </el-table-column>
        <el-table-column min-width="140" label="商业街" :show-overflow-tooltip="!isEdit" prop="expectCommercial" align="right">
          <template slot-scope="scope">
            <p>{{ returnValue(scope.row,'expectCommercial',true,false,'category') }}</p>
          </template>
        </el-table-column>
        <el-table-column min-width="140" label="住宅" :show-overflow-tooltip="!isEdit" prop="expectResidence" align="right">
          <template slot-scope="scope">
            <p>{{ returnValue(scope.row,'expectResidence',true,false,'category') }}</p>
          </template>
        </el-table-column>
        <el-table-column min-width="140" label="写字楼" :show-overflow-tooltip="!isEdit" prop="expectBuilding" align="right">
          <template slot-scope="scope">
            <p>{{ returnValue(scope.row,'expectBuilding',true,false,'category') }}</p>
          </template>
        </el-table-column>
      </el-table-column>
      <el-table-column width="5" class-name="common-column-line"></el-table-column>
      <el-table-column min-width="200" label="说明" :show-overflow-tooltip="!isEdit" prop="remake">
        <template slot-scope="scope">
          <p>{{ returnValue(scope.row,'remake',true,false,'category') }}</p>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      this.isEdit = false;
      await SummaryApi.energyList(params).then((res) => {
        if (res.code === 0) {
          this.tableData = []
