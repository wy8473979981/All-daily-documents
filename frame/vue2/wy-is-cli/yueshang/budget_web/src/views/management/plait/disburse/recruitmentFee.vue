<template>
  ...
  <div class="list-page">
    <el-row class="tool-bar" type="flex" justify="space-between" align="middle" style="flex-flow: wrap;">
      <el-col :span="18">
        <el-select v-if="!isEdit" placeholder="请选择业态" size="mini" v-model="query.paramFirst" @change="getListChange" filterable clearable style="margin-right: 5px; width: 180px;">
          <el-option v-for="item in paramFirstList" :key="item.subjectId" :label="item.subjectName" :value="item.subjectId">
          </el-option>
        </el-select>
        <el-select v-if="!isEdit" placeholder="请选择渠道" size="mini" @change="secondNameFunc" v-model="query.secondName" value-key="subjectId" filterable clearable style="margin-right: 5px; width: 180px;">
          <el-option v-for="item in paramSecondList" :key="item.subjectId" :label="item.subjectName" :value="item">
          </el-option>
        </el-select>
    ...
    <el-table class="table-input-box" ref="table" :data="tableData" size="mini" border stripe header-row-class-name="table-header" :height="this.$store.state.tableHeight" :cell-style="{ padding: '4px 0' }">
      ...
      <el-table-column fixed min-width="140" label="渠道" show-overflow-tooltip prop="subjectList.subSubject.subjectName">
        <template slot-scope="scope">
      ...
      <el-table-column min-width="140" label="费用标准" :show-overflow-tooltip="!isEdit" prop="feeStandard">
        <template slot-scope="scope">
          <el-input v-if="isEdit && scope.$index == selectIndex" v-model="scope.row.feeStandard" placeholder="请输入" size="mini"></el-input>
          <p v-else @click="showInput(scope.$index)">{{ (scope.$index == 0 && !isEdit) ? '/' : scope.row.feeStandard }}</p>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getBizTypeList() {
      ...
      GlobalApi.getBizTypeList({ sheetId: this.routerQuery.budgetSheetId, level: 2 }).then(res => {
        if (res.code == 0) {
          this.paramSecondList = res.data
        }
      })
    },
    // 获取级别列表
    ...
    secondNameFunc(data) {
      this.query.paramSecond = data.subjectId;
      this.query.secondName = data.subjectName;
      this.getListChange();
    },
    // 重新选择搜索项重置页码
    getListChange() {
