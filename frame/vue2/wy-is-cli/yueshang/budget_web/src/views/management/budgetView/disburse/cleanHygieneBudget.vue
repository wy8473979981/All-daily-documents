<template>
  ...
  <div class="list-page">
    <el-row class="tool-bar" type="flex" justify="space-between" align="middle" style="flex-flow: wrap;">
      <el-col class="select-title" :span="20">
        <div class="select-cutover">
          <span @click="$router.push({path: '/management/budgetView/disburse/cleanHygieneBudgetDetail',query:toRouter})">清洁卫生费明细</span>
          <span class="select-blue">清洁卫生费</span>
        </div>
      </el-col>
      <el-col :span="16">
    ...
    <el-table class="table-input-box" ref="table" :data="tableData" size="mini" border stripe header-row-class-name="table-header" :height="this.$store.state.tableHeight" :cell-style="{ padding: '4px 0' }">
      ...
      <el-table-column fixed min-width="120" label="费项" show-overflow-tooltip prop="subjectList">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    async getFormId() {
      try {
        // 查询
        const params = { feeId: this.routerQuery.budgetInstanceSheetId, feeType: 1 };
