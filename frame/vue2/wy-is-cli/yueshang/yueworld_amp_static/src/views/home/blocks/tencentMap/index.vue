<template>
  <div>
    <img v-if="monClick === 4" class="mapbg" :src="BK_IMG" alt="">
    <div v-if="monClick === 0" id="mapContainer" class="ysn-map" />
    <ul
      v-if="spenSionShow"
      class="box"
...
export default {
  ...
  methods: {
    ...
    init(val) {
      ...
      if (val) {
        console.log(this.map)
        this.makeMarkers(val, '2')
      } else {
        console.log(this.map, '3')
        this.makeMarker()
      }
      console.log(this.map, '4')
    },
    destroy() {
      this.map.destroy()
    },
    ...
    makeMarker() {
      ...
      // this.eventClicks 图标点击事件
    },
    // 点击投测高亮
    // 同时接受右侧四个按钮的点击参数
    clickMakeMarkers(index) {
      mapL.$on('clicks', (message) => {
        console.log(message)
        if (message === 0) {
          this.destroy()
          this.clickShow = false
