...
<template>
  <div class="home-content">
    ...
    <div class="home-left d-inline-block" :class="isThree ? 'three' : ''">
      ...
      <transition name="el-zoom-in-top">
        ...
        <main-data
          ...
          :prog-all-max-num="progAllMaxNum"
          :progress-all-data="progressAllData"
          @getData="getData"
        />
      </transition>
    ...
    <div class="home-right d-inline-block" :class="isThree ? 'three' : ''">
      ...
      <!-- 顶部按钮 -->
      <div
        v-if="pageModel === 'map' && mapType != 'china'"
        class="search-btn mb-8 d-flex"
        :class="isThree ? 'flex-ju-end three' : ''"
      >
        <div
      ...
      <template v-if="!isThree">
        ...
        <ysn-glbSearch
          ...
          @glbBtnGroupChange="glbBtnGroupChange"
        />
        <p class="clearfix" />
      ...
      <template v-else>
        ...
        <!-- 三级地图右侧详情  -->
        <div :class="threeFilter ? 'showFilter' : ''">
          <Details v-if="threeType === 'detail' && place" />
        </div>
        <Comprehensive
          v-if="threeType === 'detail' && !place"
          ref="comprehensive"
...
export default {
  ...
  mounted() {
    this.getHomeData()
    this.fn() // 地图的三个图标点击
    this.placefun() // 红色地块点击
  },
  methods: {
    // 接受地图的传值（三个图标的点击)
    fn() {
      mapL.$on('myFun', (message) => {
        console.log(message)
        const item = {
          type: ''
        }
        ...
        this.glbBtnGroupChange(message - 1, item)
      })
    },
    // 地块点击
    placefun() {
      mapL.$on('place', (message) => {
        ...
        this.glbBtnGroupChange(0, item, 77)
        if (message === 1) {
    ...
    // 顶部按钮事件
    glbBtnGroupChange(index, item, val) {
      // 点击投测，估值
      if (index === 0) {
        mapL.$emit('clicks', 0)
        if (val) {
