            ...
            <el-scrollbar ref="scroll" :wrap-class="'wrapClass'" :style="nextAttachmentDisplay?'height: calc(100% - 80px)':'height: 100%'">
              <div class="point-position-doc-list read-con">
                <div class="point-position-doc-wrapper" :style="{width:pointPositionDocWrapperWidth+'px'}">
                  <div class="point-position-doc-pages" :style="{transform: 'scale(' + scaleSize + ')', transformOrigin: 'left top', width: useQfzDrag?'913px':'793px'}">
                    ...
                    <div class="point-position-doc-page" v-for="(attachmentItemDetail,attachmentDetailIndex) in attachmentDetailList" :key="attachmentDetailIndex" @click="attachmentDetailClick(attachmentItemDetail)">
                      ...
                      <div class="point-position-doc-page-con" :style="{width:useQfzDrag?'913px':'793px', height: '1121px'}">
                        ...
                        <template v-for="(dragItem,dragIndex) in attachmentItemDetail.dragList">
                          <VueDragResize :parentLimitation="true" :isResizable="false" :isDraggable="true" :key="dragItem.id" :w="dragItem.w" :h="dragItem.h" :x="dragItem.x" :y="dragItem.y" :parentW="dragItem.parentW" :parentH="dragItem.parentH" @clicked="onActivated(dragItem,dragIndex,attachmentItemDetail)" @dragstop="onDragstop" v-if="dragItem.dragType!=='qfz'">
          ...
          <el-scrollbar style="height: calc(100% - 37px)">
            <div class="point-position-site-container">
              ...
              <ul class="point-position-site-ul">
                ...
                <li class="step" v-if="currentAttachmentTotalPage>1 && currentUserType == 'ENTERPRISE'">
...
export default {
  ...
  methods: {
    openComponentDefaultData () {
      this.currentAttachmentName = this.attachmentList[0].name;                         // 默认第一个附件 文件名称
      ...
      this.currentQyfName = this.attachmentList[0].roles[0].userInfo.enterpriseName;    // 默认第一个附件 签约方数组的第一个签约方名称
    ...
    computedStyle () {
      ...
      if (this.currentAttachmentTotalPage == 1 || this.currentUserType != 'ENTERPRISE') {
    ...
    qyfClick (item, index) {
      ...
      this.currentQyfName = item.userInfo.enterpriseName;
      ...
      currentAttachment.attachmentDetailList.map((item, index) => {
        item.dragList.map((itemSon, indexSon) => {
          if (itemSon.currentQyfIndex == this.currentQyfIndex) {
            if (itemSon.dragType == 'gz') {
              gzNumber = gzNumber + 1;
            } else if (itemSon.dragType == 'rq') {
              rqNumber = rqNumber + 1;
            } else if (itemSon.dragType == 'qm') {
              qmNumber = qmNumber + 1;
            }
          }
        })
      });
      if (currentQyf.userType == 'ENTERPRISE') {
    ...
    attachmentClick (item, index) {
      ...
      this.currentAttachmentName = item.name;
    ...
    onActivated (item, dragIndex, attachmentItemDetail) {
      ...
      this.currentPageDragType = item.dragType;
    ...
    onDragstop (newRect) {
      ...
      if (this.currentPageDragType == 'qfz') {
        currentAttachment.attachmentDetailList.map((item, index) => {
          item.dragList.map((itemSon, indexSon) => {
            if (itemSon.dragType == 'qfz' && indexSon == this.currentPageDragIndex) {
      ...
      if (this.qfzChecked) {
        ...
        currentAttachment.attachmentDetailList.map((item, index) => {
          item.dragList.map((dragItem, dragIndex) => {
            if (dragItem.dragType == 'qfz' && dragItem.currentQyfIndex == this.currentQyfIndex) {
              item.dragList.splice(dragIndex, 1)
            }
          })
        })
    ...
    saveFunc () {
      this.$emit('save', this.attachmentList)
      console.log("保存 this.attachmentList >>> ", this.attachmentList)
    },
    nextAttachmentFunc () {
      this.$refs['scroll'].wrap.scrollTop = 0//重置滚动条的位置
      ...
      let currentAttachment = this.attachmentList[this.attachmentIndex];//当前操作附件
      this.currentAttachmentName = currentAttachment.name;                          // 默认第一个附件 文件名称
      ...
      this.qfzChecked = this.currentQyfList[this.currentQyfIndex]['qfzChecked'];    // 当前签约方 是否使用骑缝章
      this.currentQyfName = currentAttachment.roles[0].userInfo.enterpriseName;     // 默认第一个附件 签约方数组的第一个签约方名称
      this.currentUserType = this.currentQyfList[0].userType;                       // 默认第一个附件 第一个签约方的类型
