...
class Approval extends Base {
  ...
  async findPageApproveEs (p, showMsg) {
    const res = await post('approve/findPageApproveEs', p)
    return res
  }
    // 事业部假期取消单-关联请假单查询号模糊搜索接口
  ...
  // 单项营销方案审批表- 查询销售合同/认购书网批号
  async quickSearchForMarketing (p, showMsg) {
    const res = await post('costAmt/quickSearchForMarketing', p)
    return res
  }

  // 单项营销方案审批表- 当前季度查询营销费用预算维护
  async getCostQuarBudgetAmt (p, showMsg) {
    const res = await post('costAmt/getCostQuarBudgetAmt', p)
    return res
  }
  ...
  // 工资费用申请单-模糊查询
  async getSubjectId(p, showMsg) {
    const res = await get('api/finSalaryInfoApi/querySubjectId', p);
    return res;
  }

    // 查询补贴金额
    async budgetTask (p, showMsg) {
        const res = await post('hrChangeBldcOpenApi/getTaskListByUserCd', p)
        return res
    }

    // 模糊查询职位-调整前
    async findStaffPositionsLeft (p, showMsg) {
        const res = await post('hrChangeBldcOpenApi/findStaffPositionsLeft', p)
        return res
    }

    // 模糊查询职位-调整后
    async findStaffPositionsRight (p, showMsg) {
        const res = await post('hrChangeBldcOpenApi/findStaffPositionsRight', p)
        return res
    }
  ...
  async getCostQuarBudgetAmt (p, showMsg) {
    ...
    //出差申请单日期重复发起校验
    async compareTime(p, showMsg) {
        const res = await post('approve/compareTime', p)
        return res
    }
  ...
  //判断人事网批网批是否可以翻起
  async searchCard (p, showMsg) {
    const res = await post('approve/searchCard', p)
    return res
  }

  // 礼品领用单-礼品品类字典信息查询
  async giftTypeInfo(p,showMsg) {
    const res = await get('api/dictApi/getDictByTypeCd', p)
    return res
  }
  // 礼品领用单-获取营销合同类别和营销合同明细
  async getMarketExtension(p,showMsg) {
    const res = await post('giftApplyBillNewApi/getMarketExtension',p)
    return res
  }

  // 礼品模糊查询
  async searchGiftName(p,showMsg) {
    const res = await post('giftApplyBillNewApi/searchGiftName',p)
    return res
  }

  // 网批号查询
  async searchGiftDisplayNo(p,showMsg) {
    const res = await post('giftApplyBillNewApi/searchGiftDisplayNo',p)
    return res
  }
  // 网批号查询-自购核销
  async searchGiftDisplayNoZGHX(p,showMsg) {
    const res = await post('giftApplyBillNewApi/searchGiftDisplayNoZGHX',p)
    return res
  }
  ...
  // 无预控-筛选中心和项目
  async getTypeByOrgCds(p, showMsg) {
    const res = await post('application/getTypeByOrgCds', p)
    return res
  }

  // 城市公司-校验包干比例
  async checkRatioByOrgCds(p, showMsg) {
    const res = await post('application/checkRatioByOrgCds', p)
    return res
  }

  //更新审批内容附件信息
  async updateEntityFiles(p, showMsg) {
    const res = await post('approve/updateEntityFiles', p)
    return res
  }

  // 验证是否超过一个自然月
  async isOverdueMonth(p, showMsg) {
    const res = await get('api/isOverdueMonthApi/isOverdueMonth', p)
    return res
  }

  // 部门级别验证
  async getTypeByOrgCd(p, showMsg) {
    const res = await get('api/isOverdueMonthApi/getTypeByOrgCd', p)
    return res
  }
	...
	async chapterFilesAbbs(p) {
		const res = await post('upsign/abbAndHdPage', p,'notloading','JSON',true)
		return res
  }

	//缩略图缓存
	async abbsAndHdsPage(p) {
		const res = await post('upsign/abbsAndHdsPage', p,'notloading','JSON',true)
		return res
  }
  //缓存进度
	async abbsAndHdsPageState(p) {
		const res = await post('upsign/abbsAndHdsPageState', p,'notloading','JSON',true)
		return res
	}
	//base64编码
	async loadBase64Codes(p) {
		const res = await post('upsign/UrlLoadBase64code', p,'notloading','JSON',false)
		return res
  }
	...
	async employeesInfo(p) {
		const res = await post('upsign/employeesInfo', p,'notloading','JSON',true)
		return res
  }
  
  //合同详情
	async queryContractDetail(p, showMsg) {
		const res = await post('upsign/queryContractDetail', p,'notloading','JSON',true)
		return res
  }

	//获取合同详情的文件图片base64code
	async viewUrl(p, showMsg) {
		const res = await post('upsign/viewUrl', p,'notloading','JSON',true)
		return res
  }
  
	//缓存缩略图和详情图
	async abbsAndHdsPage(p,showMsg) {
		const res = await post('upsign/abbsAndHdsPage', p,'notloading','JSON',true)
		return res
  }

	//开发者账号
	async accountInfo(p,showMsg) {
		const res = await post('upsign/accountInfo', p,'notloading','JSON',true)
		return res
	}

  // 判断是否为主数据
  async checkOrgCdSapInfoHide(p, showMsg) {
    const res = await get('api/isOverdueMonthApi/checkOrgCdSapInfoHide', p)
    return res
  }
  
  //根据uiid带出数据
  async getCenterAndDeptAndPositonNameAndSexCdAndBirthByUiid(p, showMsg) {
    const res = await post('hrRenewContractApplyBlzbOpenApi/getCenterAndDeptAndPositonNameAndSexCdAndBirthByUiid', p)
    return res
  }

  //职级下拉
  async quickSearchLevel(p, showMsg) {
    const res = await post('employeeOpenApi/quickSearchLevel', p)
    return res
  }

  // 根据uiid获取费用承担部门id和名称
  async getDeptCdAndName(p, showMsg) {
    const res = await post('hrTripApplyNew1OpenApi/getDeptCdAndName', p)
    return res
  }

  // 根据部门Cd获取部门所在中心名称
  async getOrgNameByOrgCd(p, showMsg) {
    const res = await post('hrTripApplyNew1OpenApi/getOrgNameByOrgCd', p)
    return res
  }
  
  // 延期签约申请API-查询项目
  async getProjectList(p, showMsg) {
    const res = await post('downpayTurnSignapplyOpenApi/getProjectList', p)
    return res
  }
  
  // 计算住宿补贴
  async getProjectList(p, showMsg) {
    const res = await post('hrTripApplyNew1OpenApi/searchCityReside', p)
    return res
  }

  //获取解约日前一天解约金额
  async getCancelDateMoney (p, showMsg) {
     const res = await post('bisMust/getCancelDateMoney', p)
     return res
  }

  //根据解约日期计算应收、欠收金额
  async getSumMoneyByQzMonth (p, showMsg) {
     const res = await post('bisMust/getSumMoneyByQzMonth', p)
     return res
  }

   //查询到期解约合同
   async getQuickSearchContDq (p, showMsg) {
     const res = await post('bisMust/quickSearchCont', p)
     return res
  }
