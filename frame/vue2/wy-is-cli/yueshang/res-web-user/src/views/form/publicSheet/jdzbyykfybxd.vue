        ...
        <div class="d-min d-checkbox">
          ...
          <div class="d-red"
               :class="{'novalue': form.bizEntityJson.positionLevel1 != 'true'
                          ...
                          && form.bizEntityJson.positionLevel5 != 'true', 'required': !form.bizEntityJson.disabled}">
            <d-radio-group
      ...
      <el-col :span="24" class="table-col right-col table-checkbox">
        <el-table ref="table" :data="form.bizEntityJson.detailList" :span-method="spanMethod" size="mini" border stripe header-row-class-name="table-header">
          ...
          <el-table-column label="费用中心">
            <template slot-scope="scope">
              <span v-if="checkBudget" style="color: #468CEB; cursor: pointer;" @click="findBudgetUsedDetail(scope.row)">{{ scope.row.fullPath }}</span>
              <span v-else>{{ scope.row.fullPath }}</span>
            </template>
          </el-table-column>
  ...
  export default {
    data: function() {
      return{
        ...
        disableCurrency: true, // 是否禁用币种选择
        checkBudget: false // 费用中心是否校验预算，酒店总部和下属公司控制不校验
      }
    },
    computed: {
      totalAmt() {
    ...
    watch: {
      ...
      realAmt(val) {
        if (Number(this.form.bizEntityJson.lendAmt) + Number(this.form.bizEntityJson.advanceAmt) > Number(this.form.bizEntityJson.totalAmt)) {
          ...
          // 酒店总部和下属公司用，来流程金额判断
          this.$set(this.form.bizEntityJson, 'realAmtDec', Number(val.toFixed(2)))
          console.log('===============', this.form.bizEntityJson.realAmtDec)
          // 缺票金额
        ...
        if(res.code == 1){
          ...
          if (res.data.isReimburse == 1) {
            ...
            for(let index in res.data.detailList) {
              ...
              // 实报实销
              if (res.data.detailList[index].realExecFlg == 1 && this.checkBudget) {
