  ...
  <div :class="['public-box',{'disabled': form.bizEntityJson.disabled}]">
    <el-row :gutter="24" class="top-row">
      <el-col :span="12" class="red-box-fix">
        <div class="d-min">
          <span>报销类别</span>
          <div class="d-red" :class="{'novalue': form.bizEntityJson.preType1 != 'true'
                              && form.bizEntityJson.preType2 != 'true'
                              && form.bizEntityJson.preType3 != 'true', 'required': !form.bizEntityJson.disabled}">
            <d-radio-group
                v-model="form.bizEntityJson"
                :nameList="[{label:'出差申请单',name:'preType1'},
                          {label:'创想实验室支出(租金、物管、能耗)',name:'preType3'},
      ...
      <el-col :span="12" class="red-box-fix right-col">
        <div class="d-min d-checkbox">
          <div v-if="form.bizEntityJson.preType1 == 'true'" class="d-red"
               :class="{'novalue': form.bizEntityJson.positionLevel14 != 'true'
                          && form.bizEntityJson.positionLevel1 != 'true'
                          && form.bizEntityJson.positionLevel2 != 'true'
                          && form.bizEntityJson.positionLevel3 != 'true'
                          && form.bizEntityJson.positionLevel4 != 'true', 'required': !form.bizEntityJson.disabled}">
            <d-radio-group
                v-model="form.bizEntityJson"
                :nameList="[{label:'副总裁（主席办）',name:'positionLevel14'},
                          {label:'副总裁',name:'positionLevel1'},
                          {label:'中心负责人',name:'positionLevel2'},
      ...
      <el-col :span="12" class="right-col">
        <div class="explain-p">
          <p v-if="form.bizEntityJson.lastBillFlgYes == 'true'" class="explain-p-one">最后一单报销完成后，预控申请即作废，您确认当前是最后一单报销吗？</p>
        </div>
      </el-col>
      <el-col :span="12">
        <div class="d-min">
...
export default {
  ...
  created() {
  let costBxSceneConfigId = this.$route.query.costBxSceneConfigId;
  // 总部有预控特殊处理、在建工程走不同的流程，广告公司差旅费特殊处理
  if (costBxSceneConfigId) {
    if(costBxSceneConfigId == '76FE93DF22FF9BB9E0530D03FBD19A40'){
      this.$set(this.form.bizEntityJson, 'zjProject', 'true')
    }else{
      this.$set(this.form.bizEntityJson, 'zjProject', 'false')
    }
    if(costBxSceneConfigId == '76FE93DF22C79BB9E0530D03JE0A9A38'){
      this.$set(this.form.bizEntityJson, 'isAdvertising', 'true')
    }else{
      this.$set(this.form.bizEntityJson, 'isAdvertising', 'false')
    }
  }else{
    if(this.form.bizEntityJson.zjProject == '' || this.form.bizEntityJson.zjProject == null){
      this.$set(this.form.bizEntityJson, 'zjProject', 'false')
    }
    if(this.form.bizEntityJson.isAdvertising == '' || this.form.bizEntityJson.isAdvertising == null){
      this.$set(this.form.bizEntityJson, 'isAdvertising', 'false')
    }
  }
  console.log('zjProject：', this.form.bizEntityJson.zjProject)
  console.log('isAdvertising：', this.form.bizEntityJson.isAdvertising)
        ...
        if(res.code == 1){
          if (res.data.isReimburse == 1) {
            ...
            this.form.bizEntityJson.detailList.forEach((item, index) => {
              ...
              // 承担部门中是否包含决策层的标识,走不通流程
              if(item.orgCd == 168){
                this.$set(this.form.bizEntityJson, 'isDecisionLevel', 'true')
              }else{
                this.$set(this.form.bizEntityJson, 'isDecisionLevel', 'false')
              }
              console.log('isDecisionLevel====', this.form.bizEntityJson.isDecisionLevel)
              ...
              this.$set(this.form.bizEntityJson.detailList[index], 'accReduce', val.sumKjhjPrice)
              this.$set(this.form.bizEntityJson.detailList[index], 'accReduce2', val.sumKjhjTax)
              // 记录费用名称为交通的index
              if (item.displayName == '交通') this.trafficIndex = index
            })
            ...
            if (res.data.detailList[index].realExecFlg == 1) {
              await this.$approval.findBudgetUsedDetail({orgCd: res.data.detailList[index].orgCd, subjectId: res.data.detailList[index].subjectId}).then(res => {
                if(res.code == 1){
                  if (this.form.bizEntityJson.currencyAmt == 'CNY') {
                    ...
                    if (ratioValue * (Number(this.form.bizEntityJson.detailList[index].currentAmt) + Number(this.form.bizEntityJson.detailList[index].currentAmt2)) > Number(res.data.remainderBudget)) {
                    ...
                    // 折本位币不能大于剩余预算
                    if (ratioValue * (Number(this.form.bizEntityJson.detailList[index].standardMoney)) > Number(res.data.remainderBudget)) {
                      this.$alert(`预算不够请检查，预算可用为：${res.data.remainderBudget}元`, '提示', {
                        confirmButtonText: '确定',
                        callback: action => {
                          return false
              ...
              if (this.form.bizEntityJson.currencyAmt == 'CNY') {
                if (res.data.detailList[index].displayName != '交通') {
                  ...
                  if (Number(res.data.detailList[index].agreeAmt) + ratioValue * (Number(this.form.bizEntityJson.detailList[index].currentAmt) + Number(this.form.bizEntityJson.detailList[index].currentAmt2)) > Number(res.data.detailList[index].budgetAmt)) {
                    this.$message.closeAll()
                    this.$message.error('本次报销金额超过申请预算金额，请确认!')
                    this.isYykCheck = false
                  }
                }
              } else {
                if (res.data.detailList[index].displayName != '交通') {
                  ...
                  if (Number(res.data.detailList[index].agreeAmt) + ratioValue * (Number(this.form.bizEntityJson.detailList[index].standardMoney)) > Number(res.data.detailList[index].budgetAmt)) {
                    this.$message.closeAll()
                    this.$message.error('本次报销金额超过申请预算金额，请确认!')
                    this.isYykCheck = false
                  }
