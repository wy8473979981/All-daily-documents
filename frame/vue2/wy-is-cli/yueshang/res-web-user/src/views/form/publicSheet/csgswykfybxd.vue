<template>
  ...
  <div :class="['public-box',{'disabled': form.bizEntityJson.disabled}]">
    <el-row :gutter="24" class="top-row green-row">
        <dl-col :span="24" :novalue="form.bizEntityJson.positionLevel1 != 'true' && form.bizEntityJson.positionLevel2 != 'true'" place="right">
    ...
    <el-row :gutter="24" class="top-row margin-row blue-row">
      <dl-col :span="4" >
        <div class="explain-p">
    ...
    <el-row class="top-row margin-row blue-row">
      ...
      <el-col :span="24" class="table-col right-col table-checkbox">
        <el-table ref="table" :data="form.bizEntityJson.billList" size="mini" border stripe header-row-class-name="table-header">
          ...
          <el-table-column label="包干比例" prop="ratio">
            <template slot-scope="scope">
              <el-input v-model="scope.row.ratio" readonly :placeholder="form.bizEntityJson.disabled ? '' : ''"></el-input>
            </template>
          </el-table-column>

          <el-table-column label="场景" prop="sceneName" class-name="no-print">
            <template slot-scope="scope">
              <el-select v-if="$route.query.costBxSceneConfigId =='76FE93DF23759BB9E0530D03FB0A9A38' || form.bizEntityJson.costBxSceneConfigId =='76FE93DF23759BB9E0530D03FB0A9A38'" v-model="scope.row.sceneName" @change="selectSubjectBySceneId(scope.row.sceneName, scope.$index)" :placeholder="form.bizEntityJson.disabled ? '' : '请选择'" :disabled="form.bizEntityJson.disabled">
                <el-option label="外包劳务费" value="76FE93DF23759BB9E0530D03FB0A9A38"></el-option>
                <el-option label="工资及津贴(新)" value="66FE93DF22E89BB9E0530D03FB0A9A48"></el-option>
                <el-option label="公积金(新)" value="66FE93DF22E89BB9E0530D03FB0A9A58"></el-option>
                <el-option label="社会保障(新)" value="76FE93DF22DF9BB9E0530D03FB0A9A56"></el-option>
                <el-option label="业绩提成" value="76FE93DF23579BB9E0531D03FB0A9A40"></el-option>
              </el-select>
              <el-input  v-else :class="{'novalue': !scope.row.sceneName, 'required': !form.bizEntityJson.disabled}" v-model="scope.row.sceneName" @focus="showScene = true; trafficIndex = scope.$index" readonly></el-input>
            </template>
          </el-table-column>
    ...
    <select-scene :open="showScene && !form.bizEntityJson.disabled" :plate="2" @close="closeScene"></select-scene>
...
export default {
  ...
  created() {
    if (!this.$route.query.id) {
      ...
      if(this.$route.query.costBxSceneConfigId == '76FE93DF23759BB9E0530D03FB0A9A38'){
        this.addList()
        this.addList()
        this.addList()
        this.addList()
        this.addList()
        this.selectSubjectBySceneId('76FE93DF23759BB9E0530D03FB0A9A38', 0)
        this.selectSubjectBySceneId('66FE93DF22E89BB9E0530D03FB0A9A48', 1)
        this.selectSubjectBySceneId('66FE93DF22E89BB9E0530D03FB0A9A58', 2)
        this.selectSubjectBySceneId('76FE93DF22DF9BB9E0530D03FB0A9A56', 3)
        this.selectSubjectBySceneId('76FE93DF23579BB9E0531D03FB0A9A40', 4)
      }else{
  ...
  methods: {
    ...
    selectSubjectBySceneId(id, index){
      ...
      this.$approval.selectSubjectBySceneId(param).then(res => {
         if(res.code == 1 && res.data) {
          ...
          this.$set(this.form.bizEntityJson.billList[index], 'billContent', (this.$route.query.costBxSceneConfigId == '76FE93DF23759BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF23759BB9E0530D03FB0A9A38') ? '外包外服费' : res.data.accountName)
    ...
    closeSearhOrg(val) {
      if (val) {
        ...
        orgList.push({
           orgCd: val.id,
           subjectName: this.form.bizEntityJson.billList[this.selectIndex].subjectName
        })
        this.$approval.getSubjectCdByOrgCd(orgList).then(res => {
           if(res.code == 1 && res.data) {
             console.log(res.data)
             this.$approval.findBGBL({orgCd:val.id,subjectId:res.data[0]}).then(res => {
               if(res.code == 1 && res.data) {
                  this.$set(this.form.bizEntityJson.billList[this.selectIndex], 'ratio', res.data)
                  this.$set(this.form.bizEntityJson.billList[this.selectIndex], 'ratioType','on')
               }else{
                  this.$set(this.form.bizEntityJson.billList[this.selectIndex], 'ratio', '')
                  this.$set(this.form.bizEntityJson.billList[this.selectIndex], 'ratioType','')
               }
             })
           }else{
           }
    ...
    closeScene(val){
      ...
      if (val) {
        ...
        var orgList = []
        orgList.push({
           orgCd: this.form.bizEntityJson.billList[this.trafficIndex].orgCd,
           subjectName: val.subjectName
        })
        this.$approval.getSubjectCdByOrgCd(orgList).then(res => {
           if(res.code == 1 && res.data) {

             this.$approval.findBGBL({orgCd:this.form.bizEntityJson.billList[this.trafficIndex].orgCd,subjectId:res.data[0]}).then(res => {
               if(res.code == 1 && res.data) {
                  this.$set(this.form.bizEntityJson.billList[this.trafficIndex], 'ratio', res.data)
                  this.$set(this.form.bizEntityJson.billList[this.trafficIndex], 'ratioType','on')
               }else{
                  this.$set(this.form.bizEntityJson.billList[this.trafficIndex], 'ratio', '')
                  this.$set(this.form.bizEntityJson.billList[this.trafficIndex], 'ratioType','')
               }
    ...
    async yykCheck() {
      ...
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF235C9BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF235C9BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('日常办公用品费用报销金额必须小于等于一万元')
        return false
      }
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF23639BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF23639BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('保险费报销金额必须小于等于一万元')
        return false
      }
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF235E9BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF235E9BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('绿化支出报销金额必须小于等于一万元')
        return false
      }

      if ((this.$route.query.costBxSceneConfigId == '76FE93DF237F9BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF237F9BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('清洁卫生费报销金额必须小于等于一万元')
        return false
      }
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF23709BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF23709BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('修理费用报销金额必须小于等于一万元')
        return false
      }
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF23609BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF23609BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('制服费报销金额必须小于等于一万元')
        return false
      }
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF23879BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF23879BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('招聘类费用报销金额必须小于等于一万元')
        return false
      }

      if ((this.$route.query.costBxSceneConfigId == '76FE93DF23719BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF23719BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('人事代理费报销金额必须小于等于一万元')
        return false
      }
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF23679BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF23679BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('其他福利报销金额必须小于等于一万元')
        return false
      }
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF237C9BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF237C9BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('诉讼相关费用报销金额必须小于等于一万元')
        return false
      }
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF237B9BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF237B9BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('公司工商相关服务费报销金额必须小于等于一万元')
        return false
      }
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF23789BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF23789BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('审计费用报销金额必须小于等于一万元')
        return false
      }
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF23799BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF23799BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('融资、发债相关费用报销金额必须小于等于一万元')
        return false
      }
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF237A9BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF237A9BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('土地、资产咨询服务费报销金额必须小于等于一万元')
        return false
      }
      var orgList = []
      var resultList = []
      ...
      if(res.code == 1 && res.data) {
          ...
          this.form.bizEntityJson.billList.forEach((item,index) => {
              ...
              paramList.push({
                orgCd: item.orgCd,
                billAmount: item.ratioType == 'on' ? Number((Number(item.ratio)/100)*((Number(item.currentAmt)) + (Number(item.currentAmt2)))).toFixed(2):(Number(item.currentAmt)) + (Number(item.currentAmt2)),
                subjectName: item.subjectName,
                subjectId: resultList[index]
              })
          })
