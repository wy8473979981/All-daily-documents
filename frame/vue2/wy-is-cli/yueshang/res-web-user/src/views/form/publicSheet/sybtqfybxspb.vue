  ...
  <div :class="['public-box',{'disabled': form.bizEntityJson.disabled}]">
    <el-row :gutter="24" class="top-row margin-row green-row">
      <el-col :span="24" class="right-col">
        <div class="d-min">
          <div class="d-red" :class="{'novalue': form.bizEntityJson.positionLevel1 != 'true' && form.bizEntityJson.positionLevel3 != 'true' && form.bizEntityJson.positionLevel2 != 'true', 'required': !form.bizEntityJson.disabled}">
            <d-radio-group 
              v-model="form.bizEntityJson" 
              :nameList="[{label:'事业部总经理',name:'positionLevel1'},
                          {label:'事业部副总经理',name:'positionLevel3'},
      ...
      <dl-col v-if="form.bizEntityJson.strFrame == 'true'" :span="12" title="框架合同附件" place="right" :novalue="!form.bizEntityJson.strFrameId">
        <one-file :form="form" value="strFrameId" />
      </dl-col>
      
      <dl-col :span="12" v-else class="right-col">
         <div class="explain-p">
          <p class="explain-p-one">*此入账公司为增值税一般纳税人</p>
        </div>
      </dl-col>
    ...
    <el-row :gutter="24" class="top-row margin-row blue-row">
      <el-col :span="form.bizEntityJson.disabled ? 24:8" class="title-col" :class="form.bizEntityJson.disabled ? 'right-col':''">
        <h2>报销信息</h2>
      </el-col>
      <el-col :span="8" v-if="!form.bizEntityJson.disabled && form.bizEntityJson.billList.length < 20" >
        <div class="explain-p">
          <p class="explain-p-one"><span class="explain-btn-add" @click="addList">添加</span></p>
        </div>
      </el-col>
      <el-col :span="8" v-if="!form.bizEntityJson.disabled" class="right-col">
        <div class="explain-p">
          <p class="explain-p-one"><span class="explain-btn-add" @click="findMoreBudgetUsedDetail">查询实时预算</span></p>
        </div>
      </el-col>
      <el-col :span="24" class="table-col right-col table-checkbox">
        <el-table ref="table" :data="form.bizEntityJson.billList" size="mini" border stripe header-row-class-name="table-header">
          ...
          <el-table-column label="场景" prop="sceneName" class-name="no-print">
            <template slot-scope="scope">
              <el-select v-if="$route.query.costBxSceneConfigId =='76FE93DF23329BB9E0530D03FB0A9A38' || form.bizEntityJson.costBxSceneConfigId =='76FE93DF23329BB9E0530D03FB0A9A38'" v-model="scope.row.sceneName" @change="selectSubjectBySceneId(scope.row.sceneName, scope.$index)" :placeholder="form.bizEntityJson.disabled ? '' : '请选择'" :disabled="form.bizEntityJson.disabled">
                <el-option label="外包劳务费" value="76FE93DF22E89BB9E0530D03FB0A9A38"></el-option>
                <el-option label="工资及津贴(新)" value="66FE93DF22E89BB9E0530D03FB0A9A37"></el-option>
                <el-option label="公积金(新)" value="66FE93DF22E89BB9E0530D03FB0A9A47"></el-option>
                <el-option label="社会保障(新)" value="76FE93DF22DF9BB9E0530D03FB0A9A43"></el-option>
              </el-select>
          ...
          <el-table-column :label="form.bizEntityJson.currencyAmt == 'CNY' ? '金额(A)' : '金额(原币)'" prop="currentAmt">
            <template slot-scope="scope">
              <el-input v-if="scope.row.subjectName == '差旅费' || scope.row.subjectName == '市内交通费'" 
              :class="{'novalue': !scope.row.currentAmt, 'required': !form.bizEntityJson.disabled}" 
              v-model="scope.row.currentAmt" 
              @focus="showTrafficCostDetail = true; form.bizEntityJson.disabled ? reveal = true : reveal = false ; trafficIndex = scope.$index" 
              :yykDisabled="form.bizEntityJson.disabled ? true : false"  readonly maxlength="15"
              :placeholder="form.bizEntityJson.disabled ? '' : '请输入'"
              ></el-input>
          ...
          <el-table-column v-if="form.bizEntityJson.currencyAmt == 'CNY'" label="税额(B)" prop="currentAmt2">
            <template slot-scope="scope">
      ...
      <el-col :span="12" class="right-col">
        <div class="d-min">
          ...
          <el-input v-if="form.bizEntityJson.whethersLendAmt == 'true'" :class="{'novalue': !form.bizEntityJson.borrower, 'required': !form.bizEntityJson.disabled}" v-model="form.bizEntityJson.borrow" @focus="showSearchUser1 = true" readonly :placeholder="form.bizEntityJson.disabled ? '' : '请选择'"></el-input>
        </div>
      </el-col>
    </el-row>
    <el-row :gutter="24" class="top-row margin-row blue-row">
    ...
    <!-- <select-scene :open="showScene && !form.bizEntityJson.disabled" @close="closeScene"></select-scene> -->
...
export default {
  ...
  created() {
    if (!this.$route.query.id) {
      ...
      if(this.$route.query.costBxSceneConfigId == '76FE93DF23329BB9E0530D03FB0A9A38'){
        this.addList()
        this.addList()
        this.addList()
        this.addList()
        this.selectSubjectBySceneId('76FE93DF22E89BB9E0530D03FB0A9A38', 0)
        this.selectSubjectBySceneId('66FE93DF22E89BB9E0530D03FB0A9A37', 1)
        this.selectSubjectBySceneId('66FE93DF22E89BB9E0530D03FB0A9A47', 2)
        this.selectSubjectBySceneId('76FE93DF22DF9BB9E0530D03FB0A9A43', 3)
        
      }else{
        this.selectSubjectBySceneId('76FE93DF23139BB9E0530D03FB0A9A38', 0)
        this.addList()
      }
      ...
      this.form.bizEntityJson.billList.forEach((item, index) => {
        ...
        item.currentAmt=Number(item.currentAmt).toFixed(2)
        item.currentAmt2=Number(item.currentAmt2).toFixed(2)
        item.accReduce=Number(item.accReduce).toFixed(2)
        item.accReduce2=Number(item.accReduce2).toFixed(2)

        
      })
      this.form.bizEntityJson.totalAmt=Number(this.form.bizEntityJson.totalAmt).toFixed(2)
      this.form.bizEntityJson.lendAmt=Number(this.form.bizEntityJson.lendAmt).toFixed(2)
      this.form.bizEntityJson.checkAmt=Number(this.form.bizEntityJson.checkAmt).toFixed(2)
      this.form.bizEntityJson.advanceAmt=Number(this.form.bizEntityJson.advanceAmt).toFixed(2)
      this.form.bizEntityJson.realAmt =Number(this.form.bizEntityJson.realAmt).toFixed(2)
     }
  ...
  methods: {
    changePlateType() {
      ...
      this.$set(this.form.bizEntityJson,'receiveNameTrue','')     
      this.$set(this.form.bizEntityJson,'receiveName','')
      this.$set(this.form.bizEntityJson,'receiveNameIn','')
      this.$set(this.form.bizEntityJson,'isManual','false')
      this.$set(this.form.bizEntityJson,'receiveAcctNo','')
    ...
    selectSubjectBySceneId(id, index){
      ...
      this.$approval.selectSubjectBySceneId(param).then(res => {
         if(res.code == 1 && res.data) {
          ...
          this.$set(this.form.bizEntityJson.billList[index], 'billContent', (this.$route.query.costBxSceneConfigId == '76FE93DF23329BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF23329BB9E0530D03FB0A9A38') ? '外包外服费' : res.data.accountName)
    ...
    closeSearhOrg(val) {
      if (val) {
        ...
        this.$approval.getCenterParent({orgCd:val.id}).then(res => {
          if(res.code == 1 && res.data) {
            this.form.bizEntityJson.billList[this.selectIndex].fullPath = (res.data).replace('-','')
          }else{
            this.form.bizEntityJson.billList[this.selectIndex].fullPath = (val.text).replace('-','')
          }
        })
    ...
    async yykCheck() {
      ...
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF233B9BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF233B9BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('汽车日常运营费报销金额必须小于等于一万元')
        return false
      }
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF23129BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF23129BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('日常办公用品费用报销金额必须小于等于一万元')
        return false
      }
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF23279BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF23279BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('上市维护费报销金额必须小于等于一万元')
        return false
      }
      ...
      await this.$approval.getSubjectCdByOrgCd(orgList).then(res => {
        res.data.forEach(item => { 
          resultList.push(item)
          // this.form.bizEntityJson.billList[index].subjectId = item.id
        })
      })
      // console.log(this.form.bizEntityJson.billList)
      // return false
      for (let key in this.form.bizEntityJson.billList) {
        // 2045 待摊,168 决策层,当费用承担部门是这两个时,不控预算
        if (this.form.bizEntityJson.billList[key].orgCd == 168 || this.form.bizEntityJson.billList[key].orgCd == 2045) return
        // 实报实销
        await this.$approval.findBudgetUsedDetail({orgCd: this.form.bizEntityJson.billList[key].orgCd, subjectId: resultList[key]}).then(res => {
          if(res.code == 1){
            if (this.form.bizEntityJson.currencyAmt == 'CNY') {
              // 本次报销金额+本次报销税额不能大于剩余预算
              if ((Number(this.form.bizEntityJson.billList[key].currentAmt) + Number(this.form.bizEntityJson.billList[key].currentAmt2)).toFixed(2) > Number(res.data.remainderBudget)) {
                this.isYykCheck = false
                if(!res.data.remainderBudget){
                  this.$alert(`预算科目无预算,请先控制预算!`, '提示', {
                    confirmButtonText: '确定',
                    callback: action => {
                      return false
                    }
              ...
              // 折本位币不能大于剩余预算
              if (Number(this.form.bizEntityJson.billList[key].standardMoney) > Number(res.data.remainderBudget)) {
                this.isYykCheck = false
                if(!res.data.remainderBudget){
                  this.$alert(`预算科目无预算,请先控制预算!`, '提示', {
                    confirmButtonText: '确定',
