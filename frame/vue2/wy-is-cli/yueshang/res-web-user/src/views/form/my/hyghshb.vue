    ...
    <el-row v-if="isShow1" gutter="24">
      <dl-col :span="24" place="table right">
        <el-table ref="table" :data="form.bizEntityJson.detail1" size="mini" border stripe
          ...
          <el-table-column label="科目名称" prop="budgetname" align="center">
            <template slot-scope="detail">
              <el-input v-if="detail.row.budgetname != '合计'" v-model="detail.row.budgetname" readonly/>
              <span v-else>合计</span>
            </template>
          </el-table-column>
          <el-table-column label="合约包数量(个)" prop="budgetamount" align="center">
            <template slot-scope="detail">
          ...
          <el-table-column label="合约规划金额(万元)" prop="budgettaxamount" align="center">
            <template slot-scope="detail">
