        ...
        <el-row :gutter="24" class="top-row">
          ...
          <span>
            ...
            <el-col :span="24" class="table-col right-col table-checkbox">
                <el-table ref="table" :data="form.bizEntityJson.accumulationFundInfos" size="mini" border stripe header-row-class-name="table-header">
                    ...
                    <el-table-column label="单位缴纳" prop="company" align="right" />
                    <el-table-column label="个人缴纳" prop="personal" align="right" />
                    <el-table-column label="合计" prop="totall" align="right" />
    ...
    export default {
        ...
        created() {
            if (!this.$route.query.id) {
                ...
                this.$set(this.form.bizEntityJson, 'accumulationFundInfos', [])
                this.$set(this.form.bizEntityJson, 'rdList', [])
                this.$set(this.form.bizEntityJson, 'receiveAcctNo', '')
                this.$set(this.form.bizEntityJson, 'receivingUnit', '公积金管理中心')
        ...
        methods: {
            openUrl(actionId) {
                if(actionId==null || actionId ==""){
                    window.open('https://demo.powerlong.com/cost_static/main/main_old.html#/fin_accumulation_info/')
                }else{
                    window.open('https://demo.powerlong.com/cost_static/main/main_old.html#/fin_accumulation_info/'+actionId)
                }
            },
            ...
            remoteMethod2(query) {
                ...
                if (query) {
                    this.loading = true
                    this.$approval.queryActionId(param).then(res => {
                        if(res.code == 1) {
                            if (res.data.length > 0) {
            ...
            actionIdChange(val) {
                ...
                this.form.bizEntityJson.accumulationFundInfos = []
                const param = {
                    actionId: val,
                }
                this.$approval.queryActionList(param).then(res => {
                    if(res.code == 1 && res.data){
                        this.form.bizEntityJson.accumulationFundInfos = res.data.vos
          ...
          async yykCheck() {
                ...
                var isMainData = true;
                var checkMoney = true;
                console.log("adsa----:"+this.form.bizEntityJson.accumulationFundInfos);
                for(let index in this.form.bizEntityJson.accumulationFundInfos) {
                    console.log("orgCoreCd:"+this.form.bizEntityJson.accumulationFundInfos[index].orgCoreCd);
                    const param = {
                        orgCoreCd: this.form.bizEntityJson.accumulationFundInfos[index].orgCoreCd,
                    }
                    var checkDept = false;
                    ...
                    if(checkDept == 'true') {
                        isMainData = true;
                        var ratio = "";
                        if (this.form.bizEntityJson.accumulationFundInfos[index].ratio == "" || this.form.bizEntityJson.accumulationFundInfos[index].ratio == null || this.form.bizEntityJson.accumulationFundInfos[index].ratio ==0) {
                            ratio = parseFloat(1);
                        } else {
                            ratio = parseFloat(this.form.bizEntityJson.accumulationFundInfos[index].ratio) / 100;//包干比例
                        }
                        var company = parseFloat(this.form.bizEntityJson.accumulationFundInfos[index].company) * ratio;//工资

                        console.log("subjectId:"+this.form.bizEntityJson.accumulationFundInfos[index].subjectId);
                        console.log("orgCd:"+this.form.bizEntityJson.accumulationFundInfos[index].orgCd);
                        orgCd = this.form.bizEntityJson.accumulationFundInfos[index].orgCd;
                        subjectId = this.form.bizEntityJson.accumulationFundInfos[index].subjectId;
                    ...
                    if(isMainData == true ){
                      ...
                      console.log("1234567890-"+result.data.remainderBudget);
                      if(company<=result.data.remainderBudget){
                            checkMoney = true;
                        }else {
