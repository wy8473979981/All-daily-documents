  ...
  <div :class="['public-box',{'disabled': form.bizEntityJson.disabled}]" style="min-width: 1120px;">
    ...
    <table class="tl-row margin-table">
      ...
      <tr>
        ...
        <td colspan="3">
          <el-input v-if="bisShopConnIdOptions.length < 1" v-model="form.bizEntityJson.bisShopConnName" readonly />
      ...
      <tr>
        <td class="title">
          <span>
            店型
            <el-tooltip class="item" effect="dark" content="通过意向商户品牌自动勾选" placement="top-start">
              <i class="el-icon-warning"></i>
            </el-tooltip>
          </span>
        </td>
      ...
      <div v-for="(item, index) in form.bizEntityJson.bondFeeVOList" :key="index">
        ...
        <dl-col :span="3">
          <div v-if="item.name != '履约保证金'" style="width: 100%;">
            <el-input v-if="item.required" :class="{'novalue': item.standardPrice === '', 'required': !form.bizEntityJson.disabled}" v-model="item.standardPrice" @change="standardPriceChange(index, $event)" maxlength="15" v-limit-number="2" @focus="toFixedFocus" @blur="toFixedBlur" :placeholder="form.bizEntityJson.disabled ? '' : '请输入'" />
            <el-input v-else v-model="item.standardPrice" @change="standardPriceChange(index, $event)" maxlength="15" v-limit-number="2" @focus="toFixedFocus" @blur="toFixedBlur" :placeholder="form.bizEntityJson.disabled ? '' : '请输入'" />
          </div>
    ...
    <table class="tl-row margin-table">
      ...
      <tr>
        ...
        <td :style="{width: 7 / 24 * 100 + '%'}">
          <el-input v-model="form.bizEntityJson.otherItem1" type="textarea" resize="none" :placeholder="form.bizEntityJson.disabled ? '' : '请输入'"></el-input>
        </td>
        <td :style="{width: 7 / 24 * 100 + '%'}">
          <el-input v-model="form.bizEntityJson.otherItem2" type="textarea" resize="none" :placeholder="form.bizEntityJson.disabled ? '' : '请输入'"></el-input>
        </td>
        <td :style="{width: 6 / 24 * 100 + '%'}">
          <el-input v-model="form.bizEntityJson.otherItem3" type="textarea" resize="none" :placeholder="form.bizEntityJson.disabled ? '' : '请输入'"></el-input>
        </td>
      </tr>
    </table>
    <table class="tl-row margin-table">
    ...
    <el-row v-if="form.status == '完成' && form.psv.contractText">
      ...
      <dl-col :span="13" place="right">
        <p v-if="contractCode" class="explain-p-one">合同台账编号:{{ contractCode }}</p>
              ...
              if (key != 0) {
                for (let keyy in this.form.bizEntityJson.rentFeeList) {
                  if (this.form.bizEntityJson.rentFeeList[keyy].yearIndex == newRentFeeList[key].yearIndex) {
                    this.form.bizEntityJson.rentFeeList[keyy].increasingRate = (Number(newRentFeeList[key].priceActual) - Number(newRentFeeList[key - 1].priceActual)) / Number(newRentFeeList[key - 1].priceActual)
                  }
                }
              }
            }
