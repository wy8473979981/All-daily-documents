<template>
  ...
  <div :class="['public-box',{'disabled': form.bizEntityJson.disabled}]">
    <el-row :gutter="24" class="top-row">
        ...
        <div class="d-min">
          ...
          <div class="d-red" :class="{'novalue': !form.bizEntityJson.reason, 'required': !form.bizEntityJson.disabled}">
            <el-radio-group v-model="form.bizEntityJson.reason" @change="reasonChange">
              ...
              <el-radio label="4">
                赴外培训
                <el-tooltip class="item" :value="form.bizEntityJson.reason == 4 && !form.bizEntityJson.disabled && !$route.query.id ? true : false" effect="dark" content="附件中需上传赴外申请网批截图" placement="top-start">
                  <i class="el-icon-warning"></i>
                </el-tooltip>
              </el-radio>
              <el-radio label="5">
                酌情请假
                <el-tooltip class="item" :value="form.bizEntityJson.reason == 5 && !form.bizEntityJson.disabled && !$route.query.id ? true : false" effect="dark" content="仅限1天以内" placement="top-start">
                  <i class="el-icon-warning"></i>
                </el-tooltip>
              </el-radio>
            </el-radio-group>
      ...
      <el-col v-if="form.bizEntityJson.reason == 3 || form.bizEntityJson.reason == 4" :span="24" class="right-col">
  ...
  methods: {
    ...
    async yykCheck() {
      ...
      const params = {
        ...
        resDate:new Date().format('yyyy-MM-dd hh:mm:ss')
      }
      await this.$approval.searchCard(params).then(res => {
        if(res.code == 1 && !res.data){
          this.$message.error('每月仅可发起一次未刷卡申请')
          isYykCheck = false
        }
      })
      if (!isYykCheck) {
      ...
      var nowDay = new Date().format('yyyy-MM-dd').split('-')
      var beginDay = this.form.bizEntityJson.noPlayCardDate.split(' ')[0].split('-')
      console.log(nowDay[0], nowDay[1], nowDay[2], beginDay[0], beginDay[1], beginDay[2])
      if(Number(nowDay[0]) > Number(beginDay[0]) + 1){
        this.$message.error('请检查假期发起日期')
        return false
      }
      if(Number(nowDay[0]) == Number(beginDay[0]) + 1){
        if (Number(nowDay[1]) != 1 || Number(beginDay[1]) != 12) {
          this.$message.error('请检查假期发起日期')
          return false
        }
        if (Number(nowDay[2]) > 3) {
          this.$message.error('根据规定,次月3日系统将自动卡控无法发起上月网批')
          return false
        }
      }
      if (Number(nowDay[0]) == Number(beginDay[0])) {
        if (Number(nowDay[1]) > Number(beginDay[1]) + 1) {
          this.$message.error('根据规定,次月3日系统将自动卡控无法发起上月网批')
          return false
        }
        if (Number(nowDay[1]) == Number(beginDay[1]) + 1) {
