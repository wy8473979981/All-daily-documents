<template>
  <el-container 
    ...
    <div ref="printForm" class="print-form" id="print-form">
      ...
      <electronicSignature v-show="signature" :data="templateData" ref="electronicSignature" @getContract="getContract"></electronicSignature>
    ...
    <signature :signFiles="signatureShow" class="no-print"></signature>
...
export default {
  ...
  methods: {
		getContract(data) {
			...
			obj.contracstList.map(item=>{
				//delete item.documents
				item.documents.map(obj => {
					delete obj.imgList
					delete obj.appendingSignLabels
					delete obj.content
					delete obj.fileName
				})
			})
			this.sendContractsSyncQo = JSON.parse(JSON.stringify(obj))
		},
    pushBackSucc() {
    ...
    closeSpSearhUser(val) {
     ...
     // 发起
    async save() {
			let signtre = document.querySelector('.signatureStatus')
			//判断电子签约是否已签订
			if(this.$refs.electronicSignature.sendContractsSyncQo.signatureType == 1 && signtre.style.color != 'rgb(96, 188, 117)') {
				this.$message.error('指定位置没有签署!')
				return false
			}
			console.log(signtre,this.$refs.electronicSignature.sendContractsSyncQo.signatureType )
      const valiList = this.stepList.filter(e => e.displayNumber || e.cooperationFlg);
      ...
      if(this.editMode) {
        ...
        bizEntityJson = JSON.stringify({sendContractsSyncQoNew:{
					sendContractsSyncQo:{
						contractsList:this.$refs.electronicSignature.sendContractsSyncQo.contractsList,
						currentSender:this.$refs.electronicSignature.sendContractsSyncQo.currentSender,
						isBySignOrder:this.$refs.electronicSignature.sendContractsSyncQo.isBySignOrder,
						signatureType:this.$refs.electronicSignature.sendContractsSyncQo.signatureType
					},
					attachmentList:this.$refs.electronicSignature.attachmentList
				},...form.bizEntityJson});
      } else {
    ...
    async preSave(draft) {
      ...
      let bizEntityJson;
			let attachmentList = this.$refs.electronicSignature.attachmentList
      if(this.editMode) {
        const form = JSON.parse(JSON.stringify(this.formForEdit));
				form.bizEntityJson.sendContractsSyncQoNew = {
        ...
        bizEntityJson = JSON.stringify({...form.bizEntityJson});
			...
			console.log(JSON.parse(bizEntityJson),'bizEntityJson')
    ...
    async getTemplateContentText() {
      ...
      await approval.getTemplateContentText(param).then(res => {
        if(res.code == 1 && res.data){
					if(this.danList.indexOf(res.data.processCode) != -1) {
						this.signatureShow = true
					}else {
						this.signatureShow = false
					}
					/* if(this.danList.indexOf(res.data.modulePath)) {
						this.signature =  true
					}else {
    ...
    async doStepFlow(type) {
      ...
      let approveCheck
			let names = []
			this.form.approveStepList.map(item => {
				names.push(item.approveUserName)
			})
			let curentIndex = names.lastIndexOf(this.userInfo.username)
			console.log(curentIndex,this.templateData)
        ...
        this.$confirm(`您的意见为空，将默认填写${type == 'A' ?  '同意发起人意见' : '不同意'}，请问是否继续？`,{type: 'warning'}).then(() => {
          ...
          approval.doStepFlow(param).then(res => {
            if(res.code == 1 && res.message == 'success'){
              ...
              // this.getDetail();
							if(type == 'A' && curentIndex == this.form.approveStepList.length-1 && this.templateData.bizEntityJson.sendContractsSyncQoNew && this.templateData.bizEntityJson.sendContractsSyncQoNew.attachmentList) {
								this.$refs.electronicSignature.save()
							}else {
								setTimeout(() => {
									this.back(this.$route.query.id)
								}, 500)
