...
<template>
    ...
    <el-row>
      <d-col place="top" title="中心/公司" row="1" :novalue="!form.bizEntityJson.centerOrgName">
...
export default {
  ...
  methods: {
    ...
    confirmValue(val,item,name) {
      console.log('q321421412',item)
      const rg = /^([01]\d|2[01234]):([0-5]\d|60)$/
      const res = rg.test(item[name]);
      if(!res && item[name]) {
