  ...
  <div :class="['public-box',{'disabled': form.bizEntityJson.disabled}]">
    <el-row :gutter="24" class="top-row">
      <el-col :span="24" class="right-col">
        <div class="d-min">
          <span></span>
          <div class="d-red right-col" :class="{novalue: !form.bizEntityJson.positionLevel1 && !form.bizEntityJson.positionLevel2 && !form.bizEntityJson.positionLevel5,required: !form.bizEntityJson.disabled}">
            <d-radio-group 
              v-model="form.bizEntityJson" 
              :nameList="[{label:'执行董事',name:'positionLevel1'},
                        {label:'酒店集团总经理',name:'positionLevel2'},
      ...
      <el-col :span="24" class="right-col">
        <div class="d-min">
          <span></span>
          <div class="d-red right-col" :class="{novalue: !form.bizEntityJson.hotel1 && !form.bizEntityJson.hotel2,required: !form.bizEntityJson.disabled}">
            <d-radio-group 
              v-model="form.bizEntityJson" 
      ...
      <el-col :span="8">
        <div class="explain-p">
          <p class="explain-p-one"><span v-if="!form.bizEntityJson.disabled && form.bizEntityJson.billList.length < 6" class="explain-btn-add" @click="addList">添加</span></p>
        </div>
      </el-col>
      ...
      <el-col :span="24" class="table-col right-col table-checkbox">
        <el-table ref="table" :data="form.bizEntityJson.billList" size="mini" border stripe header-row-class-name="table-header">
          ...
          <el-table-column label="场景" prop="sceneName">
            <template slot-scope="scope">
              <el-input :class="{'novalue': !scope.row.sceneName, 'required': !form.bizEntityJson.disabled}" v-model="scope.row.sceneName" @focus="trafficIndex = scope.$index" readonly></el-input>
            </template>
          </el-table-column>
          ...
          <el-table-column :label="form.bizEntityJson.currencyAmt == 'CNY' ? '金额(A)' : '金额(原币)'" prop="currentAmt">
            <template slot-scope="scope">
              <el-input v-if="scope.row.subjectName == '差旅费' || scope.row.subjectName == '市内交通费'" :class="{'novalue': !scope.row.currentAmt, 'required': !form.bizEntityJson.disabled}" v-model="scope.row.currentAmt" @focus="showTrafficCostDetail = true; form.bizEntityJson.disabled ? reveal = true : reveal = false ; trafficIndex = scope.$index" readonly :placeholder="form.bizEntityJson.disabled ? '' : '请输入'"></el-input>
          ...
          <el-table-column v-if="form.bizEntityJson.currencyAmt == 'CNY'" label="税额(B)" prop="currentAmt2">
            <template slot-scope="scope">
              <el-input v-if="scope.row.subjectName == '差旅费' || scope.row.subjectName == '市内交通费'" v-model="scope.row.currentAmt2" @focus="showTrafficCostDetail = true; form.bizEntityJson.disabled ? reveal = true : reveal = false ;trafficIndex = scope.$index" readonly :placeholder="form.bizEntityJson.disabled ? '' : '请输入'"></el-input>
              <div v-else>
...
export default {
  ...
  methods: {
    ...
    selectSubjectBySceneId(){
      
      this.$approval.selectSubjectBySceneId({sceneConfigId:'76FE93DF22C99BB9E0530D03FB0A9A38'}).then(res => {
         if(res.code == 1 && res.data) {
          this.$set(this.form.bizEntityJson.billList[0], 'sceneName', res.data.sceneName)
          this.$set(this.form.bizEntityJson.billList[0], 'billContent', res.data.accountName)
          this.$set(this.form.bizEntityJson.billList[0], 'subjectName', res.data.subjectName)
          this.$set(this.form.bizEntityJson.billList[0], 'subjectId', res.data.finBudgetSubjectId)
          this.$set(this.form.bizEntityJson.billList[0], 'billItemId', res.data.finBudgetSubjectId)
         } else {
    ...
    async yykCheck() {
      ...
      for (let key in this.form.bizEntityJson.billList) {
        ...
        // 实报实销
        await this.$approval.findBudgetUsedDetail({orgCd: this.form.bizEntityJson.billList[key].orgCd, subjectId: this.form.bizEntityJson.billList[key].subjectId}).then(res => {
          if(res.code == 1){
            if (this.form.bizEntityJson.currencyAmt == 'CNY') {
