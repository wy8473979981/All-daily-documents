<template>
  ...
  <div :class="['public-box',{'disabled': form.bizEntityJson.disabled}]">
    ...
    <el-row v-if="isShow1" gutter="24">
      ...
      <dl-col :span="12" title="指令单名称" place="right">
        <el-button @click="openUrl(form.bizEntityJson.orderName)" type="text" size="mini">指令单名称</el-button>
      </dl-col>
      <dl-col :span="12" title="签证单编号">
        <el-input v-model="form.bizEntityJson.visadocumentNo" readonly/>
      </dl-col>
      <dl-col :span="12" title="项目名称" place="right">
        <el-input v-model="form.bizEntityJson.projname" readonly/>
      </dl-col>
      <dl-col :span="12" title="申报金额(含税)">
        <el-input v-model="form.bizEntityJson.taxdeclaredamount" readonly/>
      </dl-col>
      <dl-col :span="12" title="地产合同申报金(含税)" place="right">
        <el-input v-model="form.bizEntityJson.dcTaxDeclaredAmount" readonly/>
      </dl-col>
      <dl-col :span="12" title="申报金额(不含税)">
        <el-input v-model="form.bizEntityJson.declaredamount" readonly/>
      </dl-col>
      <dl-col :span="12" title="地产合同申报(不含税)" place="right">
        <el-input v-model="form.bizEntityJson.dcDeclaredAmount" readonly/>
      </dl-col>
      <dl-col :span="12" title="申报金额(进项税额)">
        <el-input v-model="form.bizEntityJson.declaredInputTaxAmount" readonly/>
      </dl-col>
      <dl-col :span="12" title="地产合同申请进项税额" place="right">
        <el-input v-model="form.bizEntityJson.dcDeclaredInputTaxAmount" readonly/>
      </dl-col>
      <dl-col :span="12" title="变更类型">
        <el-input v-model="form.bizEntityJson.alterType" readonly/>
      </dl-col>
      <dl-col :span="12" title="变更原因" place="right">
        <el-input v-model="form.bizEntityJson.alterReason" readonly/>
      </dl-col>
      <dl-col :span="24" title="完成时间" place="right">
        <el-input v-model="form.bizEntityJson.downDate" readonly/>
      </dl-col>
      <dl-col :span="24" title="变更内容" place="right">
        <el-input v-model="form.bizEntityJson.alterContent" type="textarea" :autosize="{ minRows: 3}" readonly/>
      </dl-col>
    </el-row>
    <el-row gutter="24" class="padding-row">
      <dl-col :span="24" class="title-col topRight-col" place="relative">
    ...
    <el-row v-if="isShow2" gutter="24">
      <dl-col :span="24" place="table right">
        <el-table ref="table" :data="form.bizEntityJson.contractPlanList" size="mini" border stripe
          ...
          <el-table-column label="本次变更对应金额(含税)(元)" prop="bcchangetotal" align="center">
            <template slot-scope="detail">
              <el-input v-model="detail.row.bcchangetotal" readonly/>
            </template>
          </el-table-column>
          <el-table-column label="预估变更余额(含税)(元)" prop="setCollectionAmountNoTax" align="center">
            <template slot-scope="detail">
              <el-input v-model="detail.row.ygchangetotal" readonly/>
            </template>
          </el-table-column>
        </el-table>
      </dl-col>
    ...
    <el-row v-if="isShow3" gutter="24">
      ...
      <dl-col :span="24" place="table right">
        <el-table ref="table" :data="form.bizEntityJson.businessDetailList" size="mini" border stripe
          ...
          <el-table-column label="合同金额(含税)" prop="contotaltax" align="center">
            <template slot-scope="detail">
              <el-input v-model="detail.row.contotaltax" readonly/>
            </template>
          </el-table-column>
          <el-table-column label="合同金额(不含税)" prop="contotal" align="center">
            <template slot-scope="detail">
              <el-input v-model="detail.row.contotal" readonly/>
            </template>
          </el-table-column>
          ...
          <el-table-column label="变更金额(含税)(元)" prop="taxchangetotal" align="center">
            <template slot-scope="detail">
              <el-input v-model="detail.row.taxchangetotal" readonly/>
            </template>
          </el-table-column>
          <el-table-column label="变更金额(不含税)(元)" prop="changetotal" align="center">
            <template slot-scope="detail">
              <el-input v-model="detail.row.changetotal" readonly/>
            </template>
          </el-table-column>
          <el-table-column label="变更金额(进项税额)(元)" prop="changetotalInputTax" align="center">
            <template slot-scope="detail">
              <el-input v-model="detail.row.changetotalInputTax" readonly/>
            </template>
          </el-table-column>
        </el-table>
      </dl-col>
