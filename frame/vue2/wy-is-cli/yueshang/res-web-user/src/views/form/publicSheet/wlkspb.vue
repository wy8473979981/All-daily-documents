<template>
  ...
  <div :class="['public-box',{'disabled': form.bizEntityJson.disabled}]">
    <el-row :gutter="24" class="green-row">
      ...
      <dl-col :span="12" :novalue="form.bizEntityJson.isLand != 'true' && form.bizEntityJson.isHotel != 'true' && form.bizEntityJson.isBusiness != 'true'">
        <d-radio-group-string
          ...
          @change="isLandChange($event); clearReceiveUnitName()"
          :nameList="[{label:'地产',name:'isLand'},
                      {label:'酒店',name:'isHotel'},
                      {label:'商业',name:'isBusiness'}]"
        />
      </dl-col>
      <dl-col v-if="form.bizEntityJson.isLand == 'true' || form.bizEntityJson.isBusiness == 'true'" :span="12" title="往来分类" place="right" :novalue="!form.bizEntityJson.intercourseType">
        <el-select v-model="form.bizEntityJson.intercourseType" :placeholder="form.bizEntityJson.disabled ? '' : '请选择'" :disabled="form.bizEntityJson.disabled">
          ...
          <el-option label="股东往来资金流出" value="4"></el-option>
      ...
      <dl-col v-else :span="12" place="right" />
    ...
    <el-row class="margin-row blue-row">
      ...
      <el-col :span="12" v-if="(form.bizEntityJson.area2 != 'true')" class="right-col">
        <div class="d-min">
          <span>收款单位开户银行</span>
      ...
      <div v-if="(form.bizEntityJson.isLand == 'true' || form.bizEntityJson.isBusiness == 'true') && form.bizEntityJson.area2 == 'true'">
        <dl-col :span="8" title="走账" :novalue="form.bizEntityJson.accountKeepingTrue != 'true' && form.bizEntityJson.accountKeepingFalse != 'true'">
          <d-radio-group-string
            v-model="form.bizEntityJson" 
        ...
        <el-col v-if="form.bizEntityJson.accountKeepingTrue == 'true'" :span="8" place="right">
          <div class="d-min">
            <span>走款方案审批表：</span>
            <el-select
              ...
              :disabled="form.bizEntityJson.disabled">
              <el-option
                v-for="(item, index) in authTypeSelect"
                :key="index"
        ...
        <dl-col v-else :span="8" />
        <el-col v-if="form.bizEntityJson.intercourseType == '5'" :span="8" place="right">
          <div class="d-min">
            ...
            <el-select
              ...
              :disabled="form.bizEntityJson.disabled">
              <el-option
                v-for="(item, index) in authTypeSelectHZXM"
                :key="index"
                :label="item.displayNo"
                :value="item.displayNo">
                <el-row :gutter="24">
        ...
        <dl-col v-else :span="8" place="right" />
      </div>
    </el-row>
    <el-row v-if="form.bizEntityJson.disabled && $route.query.id && form.status != '草稿'" class="margin-row blue-row">
      <el-col :span="24" class="title-col topRight-col">
        <h2>付款会计信息</h2>
      </el-col>
      ...
      <dl-col :span="6" v-if="form.bizEntityJson.disabled && $route.query.id && form.status == '草稿'">
        <d-radio-group-string
          v-model="form.bizEntityJson" 
          :nameList="[{label:'已提交全额发票',name:'isAllInvoice'}]"
        />
      </dl-col>
      <dl-col :span="6" title="缺票金额" v-if="form.bizEntityJson.disabled && $route.query.id && form.status == '草稿'">
        <el-input v-model="form.bizEntityJson.missingTicketAmount" :placeholder="form.bizEntityJson.disabled ? '' : '请输入'"></el-input>
      </dl-col>
      <dl-col :span="6" v-else></dl-col>
    </el-row>
    <el-row v-if="form.bizEntityJson.disabled && form.bizEntityJson.area2!='true' &&$route.query.id && form.status != '草稿'" class="margin-row blue-row">
      <el-col :span="24" class="title-col topRight-col">
        <h2>收款会计信息</h2>
      </el-col>
      <dl-col :span="12" title="业务类型">
      ...
      <dl-col :span="6" v-else class="right-col"></dl-col>
    </el-row>
    <el-row class="margin-row green-row">
      ...
      <dl-col :span="12" title="关联资金申请" place="right">
        <el-checkbox v-model="form.bizEntityJson.noApply" @change="clearResDisplayNo" true-label="true" false-label="false">不关联资金申请</el-checkbox>
      </dl-col>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    async yykCheck() {
      ...
      if (this.form.bizEntityJson.area2 == 'true') {
        var isGo = true
        await this.$approval.checkSupInsideSupplier(this.form.bizEntityJson.receiveUnitName).then(res => {
          if(res.code == 1) {
            if(res.data != null){
              if(res.data == '1'){
                this.$alert('勾选‘集团外’，请确保收款单位不为内部供应商') 
                isGo = false
              }
            }
          }
      ...
      if(this.form.bizEntityJson.intercourseType == '5'  && this.form.bizEntityJson.area1 == 'true'){
        this.$set(this.form.bizEntityJson, 'newIntercourseType', '51')
      }
      if(this.form.bizEntityJson.intercourseType == '5' && this.form.bizEntityJson.area0 == 'true'){
        this.$set(this.form.bizEntityJson, 'newIntercourseType', '50')
      }
      if(this.form.bizEntityJson.intercourseType == '5' && this.form.bizEntityJson.area2 == 'true'){
        this.$set(this.form.bizEntityJson, 'newIntercourseType', '52')
      }
    },
  }
}
