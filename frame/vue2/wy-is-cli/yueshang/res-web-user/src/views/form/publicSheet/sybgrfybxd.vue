  ...
  <div :class="['public-box',{'disabled': form.bizEntityJson.disabled}]">
    <el-row :gutter="24" class="top-row green-row">
      <el-col :span="24" class="red-box-fix">
        <div class="d-min">
          <span>类别</span>
          <div class="d-red right-col" :class="{'novalue': form.bizEntityJson.positionLevel1 != 'true' && form.bizEntityJson.positionLevel3 != 'true' && form.bizEntityJson.positionLevel4 != 'true'&& form.bizEntityJson.positionLevel2 != 'true', 'required': !form.bizEntityJson.disabled}">
            <d-radio-group 
              v-model="form.bizEntityJson" 
              :nameList="[{label:'事业部负责人',name:'positionLevel1'},
                          {label:'事业部副总经理',name:'positionLevel3'},
                          {label:'部门负责人',name:'positionLevel4'},
      ...
      <el-col :span="24" class="table-col right-col table-checkbox">
        <el-table ref="table" :data="form.bizEntityJson.billList" size="mini" border stripe header-row-class-name="table-header">
          ...
          <el-table-column label="会计核减金额" prop="accReduce" align="center">
            <template slot-scope="scope">
              <el-input v-model="scope.row.accReduce" @change="accReduceChange(scope.$index, $event)" maxlength="15" v-limit-number="2" @focus="toFixedFocus" @blur="toFixedBlur" :readonly="!form.mapEdit.isAllInvoice" :yykReadonly="form.mapEdit.isAllInvoice ? true : false" :placeholder="form.mapEdit.isAllInvoice ? '请输入' : ''"></el-input>            </template>
          </el-table-column>
          <el-table-column label="会计核减税额" prop="accReduce2" align="center">
            <template slot-scope="scope">            
              <el-input v-model="scope.row.accReduce2" @change="accReduce2Change(scope.$index, $event)" maxlength="15" v-limit-number="2" @focus="toFixedFocus" @blur="toFixedBlur" :readonly="!form.mapEdit.isAllInvoice" :yykReadonly="form.mapEdit.isAllInvoice ? true : false" :placeholder="form.mapEdit.isAllInvoice ? '请输入' : ''"></el-input>            </template>
          </el-table-column>
...
export default {
 ...
 created() {
    ...
    if (!this.$route.query.id) {
      ...
      this.$set(this.form.bizEntityJson, 'resAuthTypeCd', 'TEST_BX_FLSY_10')
