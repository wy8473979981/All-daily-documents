<template>
  ...
  <div :class="['public-box',{'disabled': form.bizEntityJson.disabled}]">
    <el-row :gutter="24" class="top-row green-row">
      ...
      <el-col :span="12" class="red-box-fix right-col">
        <div class="d-min d-checkbox">
          <div v-if="form.bizEntityJson.preType1 == 'true'" style="width: 100%;">
            <div v-if="form.bizEntityJson.positionLevel1 == 'true' || form.bizEntityJson.positionLevel2 == 'true' || form.bizEntityJson.positionLevel3 == 'true' || form.bizEntityJson.positionLevel4 == 'true'" class="d-red" :class="{'novalue': form.bizEntityJson.positionLevel1 != 'true' && form.bizEntityJson.positionLevel2 != 'true' && form.bizEntityJson.positionLevel3 != 'true' && form.bizEntityJson.positionLevel4 != 'true', 'required': !form.bizEntityJson.disabled}">
              <d-radio-group
                v-model="form.bizEntityJson" 
                :nameList="[{label:'事业部总经理',name:'positionLevel1'},
                            {label:'其他副总裁级人员',name:'positionLevel2'},
                            {label:'事业部部门负责人',name:'positionLevel3'},
            ...
            <div v-else class="d-red" :class="{'novalue': form.bizEntityJson.newPositionLevel1 != 'true' && form.bizEntityJson.newPositionLevel2 != 'true' && form.bizEntityJson.newPositionLevel3 != 'true', 'required': !form.bizEntityJson.disabled}">
              <d-radio-group
                v-model="form.bizEntityJson" 
                :nameList="[{label:'事业部总经理',name:'newPositionLevel1'},
      ...
      <el-col v-if="isListingFees || (form.bizEntityJson.disabled && (form.bizEntityJson.attachPhoto == 'true' || form.bizEntityJson.resourceDepartment == 'true'))" :span="24" class="right-col">
        <div class="d-min">
          <span>上市费用责任归属</span>
          <div class="d-red" :class="{'novalue': form.bizEntityJson.attachPhoto != 'true' && form.bizEntityJson.resourceDepartment != 'true', 'required': !form.bizEntityJson.disabled}">
            <d-radio-group
              v-model="form.bizEntityJson"
            ...
            if (res.data.detailList[index].realExecFlg == 1) {
              await this.$approval.findBudgetUsedDetail({orgCd: res.data.detailList[index].orgCd, subjectId: res.data.detailList[index].subjectId}).then(res => {
                if(res.code == 1){
                  if (this.form.bizEntityJson.currencyAmt == 'CNY') {
                    ...
                    if ((Number(this.form.bizEntityJson.detailList[index].currentAmt) + Number(this.form.bizEntityJson.detailList[index].currentAmt2)).toFixed(2) > Number(res.data.remainderBudget)) {
                    ...
                    // 折本位币不能大于剩余预算
                    if (Number(this.form.bizEntityJson.detailList[index].standardMoney) > Number(res.data.remainderBudget)) {
                      this.$alert(`预算不够请检查，预算可用为：${res.data.remainderBudget}元`, '提示', {
                        confirmButtonText: '确定',
                        callback: action => {
                          return false
              ...
              if (this.form.bizEntityJson.currencyAmt == 'CNY') {
                if (res.data.detailList[index].displayName != '交通') {
                  ...
                  if ((Number(res.data.detailList[index].agreeAmt) + Number(this.form.bizEntityJson.detailList[index].currentAmt) + Number(this.form.bizEntityJson.detailList[index].currentAmt2)).toFixed(2) > Number(res.data.detailList[index].budgetAmt)) {
                    this.$message.closeAll()
                ...
                if (res.data.detailList[index].displayName != '交通') {
                  ...
                  if ((Number(res.data.detailList[index].agreeAmt) + Number(this.form.bizEntityJson.detailList[index].standardMoney)).toFixed(2) > Number(res.data.detailList[index].budgetAmt)) {
                    this.$message.closeAll()
                    this.$message.error('本次报销金额超过申请预算金额，请确认!')
                    this.isYykCheck = false
                  }
