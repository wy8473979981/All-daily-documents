<template>
  ...
  <div :class="['public-box',{'disabled': form.bizEntityJson.disabled}]">
        ...
        <el-table ref="table" :data="form.bizEntityJson.billList" size="mini" border stripe header-row-class-name="table-header">
          <el-table-column label="年月" prop="preStartDate" align="center">
           <template slot-scope="scope">
              <el-input v-model="scope.row.preStartDate" readonly></el-input>
            </template>
          </el-table-column>
          <el-table-column label="费用内容" align="center" prop="content">
            <template slot-scope="scope">
              <!-- <span style="color: #468CEB; cursor: pointer;">{{ scope.row.content }}</span> -->
              <el-input v-model="scope.row.content" readonly></el-input>
            </template>
          </el-table-column>
          ...
          <el-table-column label="金额(A)" prop="amountAmt" align="center">
            <template slot-scope="scope">
             ...
             <el-input v-if="$route.query.id && form.status != '草稿'" :value="(parseFloat(scope.row.amountAmt).toFixed(2)).replace(/(\d)(?=(\d{3})+\.)/g, '$1,')"></el-input>
              <el-input v-else :class="{'novalue': !scope.row.amountAmt, 'required': !form.bizEntityJson.disabled}" v-model="scope.row.amountAmt" maxlength="15" v-limit-number="2" @focus="toFixedFocus" @blur="toFixedBlur"  @change="currentAmountAmtChange(scope.$index, $event)" :placeholder="form.bizEntityJson.disabled ? '' : '请输入'"></el-input>
          ...
          <el-table-column label="税额(B)" prop="amountAmt2" align="center" >
            <template slot-scope="scope">
              <el-input v-if="$route.query.id && form.status != '草稿'" :value="(parseFloat(scope.row.amountAmt2).toFixed(2)).replace(/(\d)(?=(\d{3})+\.)/g, '$1,')"></el-input>
              <el-input v-else v-model="scope.row.amountAmt2" maxlength="15" v-limit-number="2" @focus="toFixedFocus" @blur="toFixedBlur" @change="currentAmountAmt2Change(scope.$index, $event)" :placeholder="form.bizEntityJson.disabled ? '' : '请输入'"></el-input>
            </template>
          </el-table-column>

          <el-table-column label="会计核减金额" prop="accReduce" align="center">
            <template slot-scope="scope">
              <el-input v-if="$route.query.id && form.status != '草稿'" :value="(parseFloat(scope.row.accReduce).toFixed(2)).replace(/(\d)(?=(\d{3})+\.)/g, '$1,')"></el-input>
              <el-input v-else v-model="scope.row.accReduce" @change="accReduceChange(scope.$index, $event)" maxlength="15" v-limit-number="2" @focus="toFixedFocus" @blur="toFixedBlur" :readonly="!form.mapEdit.isAllInvoice" :yykReadonly="form.mapEdit.isAllInvoice ? true : false" :placeholder="form.mapEdit.isAllInvoice ? '请输入' : ''"></el-input>            </template>
          </el-table-column>
          <el-table-column label="会计核减税额" prop="accReduce2" align="center">
            <template slot-scope="scope">
              <el-input v-if="$route.query.id && form.status != '草稿'" :value="(parseFloat(scope.row.accReduce2).toFixed(2)).replace(/(\d)(?=(\d{3})+\.)/g, '$1,')"></el-input>
              <el-input v-else v-model="scope.row.accReduce2" @change="accReduce2Change(scope.$index, $event)" maxlength="15" v-limit-number="2" @focus="toFixedFocus" @blur="toFixedBlur" :readonly="!form.mapEdit.isAllInvoice" :yykReadonly="form.mapEdit.isAllInvoice ? true : false" :placeholder="form.mapEdit.isAllInvoice ? '请输入' : ''"></el-input>            </template>
          </el-table-column>

           <el-table-column label="预算科目" prop="finSubjectTypeName" align="center">
            <template slot-scope="scope">
              <el-input v-model="scope.row.finSubjectTypeName" readonly></el-input>
            </template>
           </el-table-column>

          <el-table-column label="费用承担部门" prop="orgName" align="center">
            <template slot-scope="scope">
              <el-input v-model="scope.row.orgName" readonly></el-input>
            </template>
          </el-table-column>
    ...
    <el-row :gutter="24" class="top-row" style="margin-top: 15px !important;">
      <el-col :span="12">
        <div class="d-min">
...
export default {
 ...
 created() {
    ...
    if (!this.$route.query.id) {
      ...
      this.$set(this.form.bizEntityJson, 'resAuthTypeCd', 'TEST_BX_FL_10')
