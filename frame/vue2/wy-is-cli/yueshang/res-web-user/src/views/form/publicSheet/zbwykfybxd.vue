<template>
  ...
  <div :class="['public-box',{'disabled': form.bizEntityJson.disabled}]">
    <el-row :gutter="24" class="top-row">
        <dl-col :span="24" :novalue="form.bizEntityJson.positionLevel5 != 'true' && form.bizEntityJson.positionLevel14 != 'true'">
          <d-radio-group-string
            v-model="form.bizEntityJson" 
            :nameList="[{label:'董事局主席',name:'positionLevel5'},
                        {label:'副总裁（主席办）',name:'positionLevel14'}
                      ]"
          />
        </dl-col>
        ...
        <el-table ref="table" :data="form.bizEntityJson.billList" size="mini" border stripe header-row-class-name="table-header">
          <el-table-column label="费用承担部门" prop="fullPath">
            <template slot-scope="scope">
              <el-input :class="{'novalue': !scope.row.fullPath, 'required': !form.bizEntityJson.disabled}" v-model="scope.row.fullPath" @focus="showSearchOrg = true; selectIndex = scope.$index" readonly :placeholder="form.bizEntityJson.disabled ? '' : '请选择'"></el-input>
            </template>
          </el-table-column>
          <el-table-column label="场景" prop="sceneName">
            <template slot-scope="scope">
          ...
          <el-table-column label="费用实质" prop="realDesc">
            <template slot-scope="scope">
          ...
          <el-table-column label="操作" align="center" width="70" v-if="!$route.query.id">
            <template slot-scope="scope">
    ...
    <el-row v-if="form.bizEntityJson.disabled" :gutter="24" class="padding-row">
      <el-col :span="24" class="title-col topRight-col">
        <h2>会计信息</h2>
      </el-col>
      <el-col :span="12">
