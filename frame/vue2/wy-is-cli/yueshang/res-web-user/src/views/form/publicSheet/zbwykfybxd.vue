<template>
  ...
  <div :class="['public-box',{'disabled': form.bizEntityJson.disabled}]">
      ...
      <el-row class="top-row margin-row blue-row">
        ...
        <dl-col v-else :span="16" place="right">
          <div class="d-min">
          </div>
        </dl-col>
        ...
        <el-col :span="24" class="table-col right-col table-checkbox">
          <el-table ref="table" :data="form.bizEntityJson.billList" size="mini" border stripe header-row-class-name="table-header">
            ...
            <el-table-column label="场景" prop="sceneName" class-name="no-print">
              <template slot-scope="scope">
                <el-select v-if="$route.query.costBxSceneConfigId =='76FE93DF22E89BB9E0530D03FB0A9A38' || form.bizEntityJson.costBxSceneConfigId =='76FE93DF22E89BB9E0530D03FB0A9A38'" v-model="scope.row.sceneName" @change="selectSubjectBySceneId(scope.row.sceneName, scope.$index)" :placeholder="form.bizEntityJson.disabled ? '' : '请选择'" :disabled="form.bizEntityJson.disabled">
      ...
      <select-scene :open="showScene && !form.bizEntityJson.disabled" :plate="1" @close="closeScene"></select-scene>
...
export default {
  ...
  created() {
    if (!this.$route.query.id) {
      ...
      if(this.$route.query.costBxSceneConfigId == '76FE93DF22E89BB9E0530D03FB0A9A38'){
        this.addList()
        this.addList()
        this.addList()
        this.addList()
  ...
  methods: {
    ...
    selectSubjectBySceneId(id, index){
      ...
      this.$approval.selectSubjectBySceneId(param).then(res => {
         if(res.code == 1 && res.data) {
          ...
          this.$set(this.form.bizEntityJson.billList[index], 'billContent', (this.$route.query.costBxSceneConfigId == '76FE93DF22E89BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF22E89BB9E0530D03FB0A9A38') ? '外包外服费' : res.data.accountName)
    ...
    async yykCheck() {
      ...
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF22F19BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF22F19BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('汽车日常运营费报销金额必须小于等于一万元')
        return false
      }
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF22C89BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF22C89BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
        this.$message.error('日常办公用品费用报销金额必须小于等于一万元')
        return false
      }
      if ((this.$route.query.costBxSceneConfigId == '76FE93DF22DD9BB9E0530D03FB0A9A38' || this.form.bizEntityJson.costBxSceneConfigId =='76FE93DF22DD9BB9E0530D03FB0A9A38') && Number(this.form.bizEntityJson.totalAmt) > 10000) {
