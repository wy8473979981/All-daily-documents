<template>
	<!--pages/yusuan/dczf/index.wxml-->
	<!-- 地产支付物业款项统计 -->
	<view class="yusuan-dczf">
		<app-header>
			<app-project-selector :selected="{ label: projectName }" @selected="onProjectSelected" v-if="showSelector"></app-project-selector>
		</app-header>
		<app-section>
			<view slot="head-title">
				<view class="selector" @tap="openSelector">地产支付物业款项统计<image src="/static/assets/images/icon-selector-arrow-blue.svg"></image> </view>
			</view>
	...
	methods: {
		loadData: async function () {
			const { data } = await app.globalData.request
				...
				.post("http://pms.powerlong.com/budget/open/tmp/data/propertystat/query", this.query);
			const setdata = {
				"page.table": {
					columns,
					rows: data || [],
