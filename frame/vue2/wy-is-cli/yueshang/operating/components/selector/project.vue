<template>
	<view>
		<view class="app-project-selector" v-if="globalBisProjectId">
			<view class="project-name">{{ globalProjectName }}</view>
		...
		<view class="app-project-selector" @tap="onClick" v-else>
			<view class="project-name">{{ component.selected.label }}</view>
			<image v-if="!disabled" class="app-project-selector-icon" src="/static/assets/images/icon-selector-arrow.png"></image>
			<view class="project-leader" v-if="component.selected.label !== '全部'" @tap.stop="showLeaderDetail">
				<image class="leader-avatar" src="/static/assets/images/leader.png" lazy-load="false"></image>
				<view class>项目总</view>
			</view>
		</view>
...
export default {
	data() {
		return {
			...
			globalProjectName: app.globalData.projectName || app.globalData.label,
			bisProjectId: app.globalData.bisProjectId || app.globalData.value,
			showLeader: false,
			leaderDetail: "",
		};
	},
	...
	watch: {
		selected: {
			handler: function (newValue) {
				...
				if (newValue) {
					let bisProjectId = newValue.value.bisProjectId || newValue.value;
	...
	methods: {
		...
		async getLeaderInner() {
			let res = await app.globalData.request
				...
				.get(`/open/bis-open-report!bisProjectPrincipalInfo.action?bisProjectId=${this.bisProjectId || this.component.selected.value}`);
