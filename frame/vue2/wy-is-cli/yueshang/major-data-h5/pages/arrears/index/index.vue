	...
	export default {
		data() {
			return {
				title: '全宝龙',
				watermarkText:'',
				tipsList: this.$store.state.arrearsPageTipsList,
				list: [],
				reqParam: {
				...
				actionSheets: {
					...
					1: [
						{name: '月度选择',code: false,color: '#00a1ff'},
					]
					// 2: [
		...
		methods: {
			// 初始化数据-首页
			async initData() {
				await this.$http('post', 'appRegulateArrearsDept', this.reqParam, false, true).then(res => {
					if (res && res.data) {
						...
						this.initSelectTabList();
						// this.createHtml(this.$store.state.isFullCal?this.result:this.equityResult, this.$store.state.isFullCal);
						uni.hideLoading();
					}
				})
			},
			...
			createHtml(data) {
				...
				Object.keys(data).forEach(key => {
					...
					signAndPaymentItem = [
						{col1: '完成率(B/A)',col2: '差额(C=B-A)',col3: ''},
						{col1: '',col2: '',col3: '',col1_unit:'',col2_unit:''},
						
						{col1: '欠款回款目标(A)',col2: '欠款回款实际(B)',col3: ''},
						{col1: item.arrearsBackTar,col2: item.arrearsBackAct,col3: '',col1_unit:'亿元',col2_unit:'亿元'},
					...
					signAndPaymentItem[1].col1 = "<span style='font-size:18px;'>" + item.arrearsRate + "<span style='font-size:10px'>%</psan></span>"
					signAndPaymentItem[1].col1color = item.arrearsRate>=100?{color:'green'}:{color:'red'};
					signAndPaymentItem[1].col2 = "<span style='font-size:18px;'>" + item.arrearsDiff + "<span style='font-size:10px'>亿元</psan></span>"
					signAndPaymentItem[1].col2color = item.arrearsRate>=100?{color:'green'}:{color:'red'};
					if(this.reqParam.queryType == 1){
						signAndPaymentItem[0].col3 = "<span style='font-size:12px;'>奖罚(C*奖罚系数)</span>"
						signAndPaymentItem[1].col3 = "<span style='font-size:18px;'>" + item.arrearsAward + "<span style='font-size:10px'>万元</psan></span>"
						signAndPaymentItem[1].col3color = item.arrearsRate>=100?{color:'green'}:{color:'red'};
					}
					// let users = []
					...
					// 	}
					// })
					//let params = {grpMarketingCode:item.businessCode,grpMarketingName:item.businessName,queryType:this.reqParam.queryType}
					this.list.push({
