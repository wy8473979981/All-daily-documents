<template>
	<div class="ys-page">
		...
		<page-content :data="list[0]" title="动态回款" :users="users1" @userTab="userTab"  url="/pages/payment/division/index" show></page-content>
		<!-- 下拉框组件数据 -->
	...
	export default {
		...
		methods: {
			...
			createSignAndPaymentHtml(data,state) {
				let paymentItem = [{col1: '',col2: '',col3: ''},
								{col1: '昨日',col1class:'custom-title-cls',col2: '',col3: ''},
								{col1: data.paymentAmount,col2: '',col3: ''
								,col1_unit : '亿元'},
								
								{col1: '本月',col1class:'custom-title-cls',col2: '',col3: ''},
								{col1: '本月指标',col2: '本月实际',col3: '本月完成率'},
								{col1: data.monthPaymentIndexAmount,col2: data.monthPaymentAmount,col3: data.monthPaymentRate
								,col1_unit : '亿元',col2_unit : '亿元',col3_unit : '%'},
								
								{col1: '本年',col1class:'custom-title-cls',col2: '',col3: ''},
								{col1: '本年指标',col2: '本年实际',col3: '本年完成率'},
								{col1: data.yearPaymentIndexAmount,col2: data.yearPaymentAmount,col3: data.yearPaymentRate
								,col1_unit : '亿元',col2_unit : '亿元',col3_unit : '%'},
								
								{col1: '截止上月',col1class:'custom-title-cls',col2: '',col3: ''},
								{col1: '截至上月指标',col2: '截至上月实际',col3: '缺口/超额'},
								{col1: data.lastMonthPaymentIndexAmount.toFixed(2),col2: data.lastMonthPaymentAmount.toFixed(2),col3: ''
				...
				this.list.forEach((item, index) => {
					[0, 1].forEach((son, sindex) => {
			            // 回款
			            if (index === 0) {
							paymentItem[11].col3 = "<span style='font-size:15px;'>" + data.lastMonthPaymentRate + '%</span>'
							 + "<span style='font-size:12px;'>"+ (data.lastMonthPaymentAmount - data.lastMonthPaymentIndexAmount).toFixed(2) + '亿元</span>';
							paymentItem[11].col3color = data.lastMonthPaymentRate>=0?{color:'green'}:{color:'red'};
			            }
			        })
				})
				// 赋值
			    this.list[0].items = paymentItem;
			...
			showPop(res) {
				...
				if(this.index === 2){
					uni.navigateTo({
						url: '/pages/payment/ranking/index?queryType='+this.reqParam.queryType
					})
				}
				//迫使 Vue 实例重新渲染
				this.$forceUpdate();
