<template>
	<div class="ys-page">
		...
		<index-content v-for="data in list" :data="data" :users="data.users" @userTab="userTab" url="/pages/business/ranking/index"></index-content>
		<!-- 下拉框组件数据 -->
	...
	export default {
		...
		methods: {
			...
			createHtml(data,state) {
				...
				Object.keys(data).forEach(key => {
					...
					dataItem = [
						...
						{col1: item.untilLastMonthIndex.toFixed(2),col2: item.untilLastMonthSigned.toFixed(2),col3: ''
					...
					dataItem[13].col3 = "<span style='font-size:15px;'>" + item.deviationRatio.toFixed(2) + '%</span>'
						...
						dataItem[13].col3_toast= ((item.untilLastMonthSigned - item.untilLastMonthIndex) < 0 ? '缺口：':'超额:')
						 + (item.untilLastMonthSigned - item.untilLastMonthIndex).toFixed(2) + '亿元'
			...
			showPop(res) {
				...
				if(this.index === 3){
					uni.navigateTo({
						url: "/pages/business/allProject/index?dataType=SY&month="+this.reqParam.month
					})
				}else{
