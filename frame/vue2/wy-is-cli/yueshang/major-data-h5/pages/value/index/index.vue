<template>
	<div class="ys-page">
		...
		<page-content  :data="list[1]" title="商办去化" :users="users1" @userTab="userTab"  url="/pages/business/division/index" show></page-content>
		<page-content  :data="list[2]" title="车位去化" :users="users1" @userTab="userTab"  url="/pages/parking/division/index" show></page-content>
		<!-- 下拉框组件数据 -->
	...
	export default {
		...
		methods: {
			...
			createHtml(data,state) {
				...
				this.list[0].params = {groupType:'marketingGroup'};
			},
			
			
			// 初始化数据  - 商办去化
			async initBusiness() {
				this.reqNextParam.month = this.reqParam.monthly;
				this.reqNextParam.dataType = 'SY';
			    this.$http('post', 'getBusinessAndParkingIndex', this.reqNextParam,false,true).then(res => {
			        if (res && res.data) {
			            this.resultBusiness = res.data[0].all;
			            this.equityBusiness = res.data[0].right;
						//默认权益口径数据
			...
			createHtmlBusiness(data,state) {
				...
				this.list[1].params = {groupType:'marketingGroup',month:this.reqParam.monthly,dataType:'SY'};
			},
			
			// 初始化数据  - 车位去化
			async initParking() {
				this.reqNextParam.month = this.reqParam.monthly;
				this.reqNextParam.dataType = 'CW';
			    this.$http('post', 'getBusinessAndParkingIndex', this.reqNextParam,false,true).then(res => {
			        if (res && res.data) {
			...
			createParkingHtml(data,state) {
				...
				this.list[2].params = {groupType:'marketingGroup',month:this.reqParam.monthly,dataType:'CW'};
			},
			onSelect(res) {
				//选中后修改close属性 并关闭弹窗
