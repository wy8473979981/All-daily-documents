<template>
  <div class="ys-page">
    ...
    <index-content v-for="data in list" :data="data" url="/pages/balance/division/index"></index-content>
    <!-- 下拉框组件数据 -->
...
export default {
  ...
  methods: {
    // 初始化数据-首页
    async initData () {
      await this.$http('post', 'appRegulateFunBalanceDeptSum', this.reqParam, false, true).then(res => {
        if (res && res.data) {
      ...
      Object.keys(data).forEach(key => {
        ...
        if (this.reqParam.queryType == 0) {
          signAndPaymentItem = [
            ...
            { col1: item.accuCollection, col2: '-', col3: item.regulateRatioAct, col1_unit: '亿元', col2_unit: '', col3_unit: '%' },
        ...
        this.list.push({
          ...
          params: { queryType: this.reqParam.queryType, currMonth: this.reqParam.currMonth }
        })
      });
    },
    test () {
