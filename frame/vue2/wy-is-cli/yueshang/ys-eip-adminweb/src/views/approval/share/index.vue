<template>
  <div class="list-page">
    ...
    <el-table ref="table" :data="tableData" size="mini" border header-row-class-name="table-header" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="接收人" prop="userName" show-overflow-tooltip/>
      <el-table-column label="备注" prop="remark" show-overflow-tooltip/>
      <el-table-column align="center" width="80" label="回复标识" prop="replyFlg">
         <template slot-scope="scope">
            <span>{{scope.row.replyFlg == 1 ? '已回复' : '未回复' }}</span>
          </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      await approval.getShareList(params).then(res => {
    ...
    dealDel() {
      ...
      this.$confirm(`确认要删除吗？`).then(() => {
        ...
        approval.deleShare(params).then(res => {
