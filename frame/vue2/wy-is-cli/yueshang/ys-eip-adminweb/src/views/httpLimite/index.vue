<template>
  ...
  <div class="page-content">
    ...
    <ysNTable class="table" :tableList="tableList" :column="column" :tableHeight="this.$store.state.tableHeight" @checkSubmit="checkSubmit" @deleteHttp="deleteHttp" @changeStatus="changeStatus" @checkDelete="checkDelete"></ysNTable>
...
export default {
  ...
  methods: {
    ...
    async changeStatus (row) {
      try {
        let res = await this.$axios.httpLimiteServe.updateHttp(row)
        if (res.code == 200) {
          this.getList(this.searchParams)
        }
      } catch (e) {
    ...
    async checkSubmit (row) {
      try {
        let res = await this.$axios.httpLimiteServe.addHttp(row)
        if (res.code == 200) {
    ...
    async getList (data) {
      try {
        ...
        await this.$axios.httpLimiteServe.getHttpList(params).then(res => {
          if (res.code == 200) {
            let {
              data
            } = res
    ...
    async deleteConfirmFunc (row) {
      ...
      try {
        ...
        await this.$axios.httpLimiteServe.deleteHttp(params).then(res => {
          if (res.code == 200) {
            this.getList(this.searchParams)
            this.$refs.dialogDelete.centerDialogVisible = false
          }
