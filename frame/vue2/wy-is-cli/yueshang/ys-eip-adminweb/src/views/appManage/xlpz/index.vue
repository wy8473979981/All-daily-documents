<template>
  ...
  <div class="page-content">
    ...
    <ysNTable class="table" :tableList="tableList" :column="column1" :tableHeight="this.$store.state.tableHeight" @checkSubmit="checkSubmit" @deleteOpenFunc="deleteOpenFunc" @checkDelete="checkDelete" @table-change="handleTableChange"></ysNTable>
...
export default {
  ...
  methods: {
    ...
    handleTableChange (key, scope) {
      if (key == 'appId' && scope) {
        let { $index, row } = scope
        let newRow = { ...row, businessId: null }
        this.tableList.splice($index, 1, newRow)
      }
    },
    async getList (data) {
      try {
        ...
        await this.$axios.appManageServe.xlpzList(params).then(res => {
          if (res.code == 200) {
            let { data } = res
    ...
    async deleteConfirmFunc (row) {
      ...
      try {
        ...
        await this.$axios.appManageServe.xlpzDelete(params).then(res => {
          if (res.code == 200) {
            this.getList(this.searchParams)
            this.$refs.dialogDelete.centerDialogVisible = false
          }
    ...
    addRowFunc () {
      try {
        ...
        let row = { showInput: true, appId: null, businessId: null, timeInterval: null, maximum: null }
    ...
    async checkSubmit (row) {
      try {
        let res = await this.$axios.appManageServe.xlpzAdd(row)
        if (res.code == 200) {
