<template>
	<div class="tool-bar" type="flex" justify="space-between" align="middle">
		<div>
			<ysNSearch :searchList="searchList" :params="searchParams" @getList="getList"></ysNSearch>
			<ysNTable :tableHeight="this.$store.state.tableHeight" :tableList="tableList" :column="column" class="table"
				@echartMap="echartMap">
			</ysNTable>
...
<script>
	...
	export default {
		...
		data() {
			return {
				options: optionsList.countTotal,
				title: '',
				...
				column: columnList.countTotal,
				tableList: [],
				searchList: searchList.countTotal
			}
		},
		mounted() {
			setTimeout(() => {
		...
		methods: {
			async getList(searchParams) {
				...
				let res = await this.$axios.systemServe.getCountList({...this.pageInfo,searchParams:this.searchParams})
				if (res.code == 200) {
			...
			async echartMap(item) {
				...
				let res = await this.$axios.systemServe.getLineAppEchart(req)
				if (res.code == 200) {
					this.centerDialogVisible = true
					this.title = '调用次数统计图-应用'
					let xList = []
					let y0 = []
					let y1= []
					let y2 = []
