<template>
  ...
  <div class="page-content">
    ...
    <ysNDialog ref="dialogDetail" title="请求" :width="'576px'" :buttonList="buttonList" @cancelFunc="cancelFunc">
  ...
  methods: {
    async viewFunc (row) {
      try {
        ...
        await this.$axios.logManageServe.qqLogDetail({ id: row.id }).then(res => {
          if (res.code == 200) {
    ...
    async getList (data) {
      try {
        ...
        await this.$axios.logManageServe.qqLogList(params).then(res => {
          if (res.code == 200) {
            let { data } = res
