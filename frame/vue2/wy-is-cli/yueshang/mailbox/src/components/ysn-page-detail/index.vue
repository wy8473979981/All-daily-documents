          ...
          <van-row class="app-pl-box" @click="rowClick(item)">
            <van-col span="24">
              <van-row type="flex" style="flex-direction: column;">
                <van-col class="app-pl-box-flex" :class="{'app-pl-box-point': (item.receiverCount && item.receiverCount.length > 0 && 0 === pageType)}" style="width: 100%;position: relative;">
...
<!--                <van-row class="cut-line1" v-if="list && list.length > 0 && '0' != item.isForwardType" style="width: 81%;">-->
<!--                  <van-col class="cut-line1">-->
<!--                    <span v-if="pageType === 2" class="app-pl-box-f-cg cut-line1">{{item.content || '无内容描述'}}</span>-->
<!--                    <span v-else class="app-pl-box-f-cg">{{userType === 1?'收件人':'发件人'}}：{{item.receiverName}}</span>-->
<!--                  </van-col>-->
<!--                  <van-col v-if="item.positionName && userType === 2" style="margin-left: 40px;">-->
<!--                    <span class="app-pl-box-f-cg">岗位：</span>{{item.positionName}}-->
<!--                  </van-col>-->
<!--                </van-row>-->
<!--                <van-row class="cut-line1" v-if="item.isForwardType === '0'" style="width: 81%;">-->
<!--                  <van-col class="cut-line1">-->
<!--                    <span v-if="pageType === 1 && userType === 2" class="app-pl-box-f-cg cut-line1">转发：{{item.forwardNames || ''}}</span>-->
<!--                  </van-col>-->
<!--                </van-row>-->
                <template>
                  <van-col v-if="item.createTime" style="position: relative;">
                    <span class="app-pl-box-f-cg">时间：</span>{{item.createTime | ftime}}
                    <span class="app-pl-box-right">{{item.functionCode | getClass(depart) }}</span>
                  </van-col>
                  ...
                  <van-col v-if="userType === 2 && (!contact || contact.length === 0)">
                  ...
                  <van-col v-if="item.positionName && userType === 2 && pageType === 0">
                    <span class="app-pl-box-f-cg">岗位：</span>{{item.positionName}}
                  </van-col>
                  <van-col v-if="item.mobilePhone && userType === 2 && pageType === 0">
                ...
                <van-col v-if="contact.length > 0 && userType === 2 && (pageType === 1)" style="padding-right: 25px;display: flex;position: relative;padding-bottom: 8px;"
                  ...
                  <span class="app-pl-box-f-cg" style="display: block;height: 2em;float: left;">收件人：</span>
...
<!--&lt;!&ndash;                  <span v-if="item.functionCode" class="app-pl-box-t" :style="{'background-color': (colors[getClass3(item.functionCode,depart)]?colors[getClass3(item.functionCode,depart)] : '#5D76BD')}">{{item.functionCode | getClass(depart,1) }}</span>&ndash;&gt;-->
...
export default {
  ...
  methods: {
    ...
    async setTop(item) {
      ...
      const query = {createrId: uiid, createrName: userName, mainId: item.mainId,source: item.source}
      if(this.methodTop) {
