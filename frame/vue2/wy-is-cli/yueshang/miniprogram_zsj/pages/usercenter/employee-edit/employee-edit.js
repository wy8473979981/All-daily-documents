...
Page({
  ...
  checkParams(params) {
    ...
    if (params.idNo && !reg.idNo.test(params.idNo)) {
      this.data.warningComp.showWarning({
  ...
  addEmployee() {
    ...
    ysRequest({
      url: 'merchantOpen_web/merClerkInfo/updateMerClerkInfo.htm',
      method: 'post',
      params,
      succ: (res) => {
        if (res.code === '0') {
    ...
    // wx.request({
    //   method: 'POST',
    //   url: `${main_config.baseUrl}merchantOpen_web/merClerkInfo/updateMerClerkInfo.htm`,
    //   data: {
    //     "version": "1.0",
