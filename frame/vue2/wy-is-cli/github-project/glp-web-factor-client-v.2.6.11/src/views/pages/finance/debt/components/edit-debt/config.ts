...
import { d, date, datetime, uploadOptions, remote, a, amountOptions, requiredRule, textOptions, multiEmailOptions, textareaOptions, emailSep } from '@/config/form'
...
function loadContactInfo (id: any) {
  return http().get('/api/queryrContactInformation', { params: { id } })
}

const onSellerChange = ({ form, value, items }: any) => {
  if (!value) {
    store.clearSellerContact()
  } else {
    const target = items.find((v: any) => v.__value === value)
    store.sellerName = target?.__label || ''
...
const onBuyerChange = ({ form, value, items }: any) => {
  if (!value) {
    store.clearBuyerContact()
  } else {
    const target = items.find((v: any) => v.__value === value)
    store.buyerName = target?.__label || ''
    loadContactInfo(value).then(data => {
      if (!data) store.clearSellerContact()
      else {
        store.sellerContact = {
...
export const transFormDefs: Array<FieldDef | GridFieldDef> = [
  { type: 'select', model: 'projectId', label: '授信项目', options: { ...remote('project-name') }, rules: [requiredRule('number')] },
  { type: 'select', model: 'sellerCompanyId', label: '卖方（债权方）', options: { options: a('seller-by-project', { store }), onValueChange: onSellerChange, disabled: true }, rules: [requiredRule('number')] },
  // { type: 'select', model: 'sellerArea', label: '卖方所在区域', options: { options: d('_province') }, rules: [{ required: true }] },
  { type: 'select', model: 'buyerCompanyId', label: '买方（债务方）', options: { options: a('buyer-by-project', { store }), onValueChange: onBuyerChange }, rules: [requiredRule('number')] }
  ...
  controls: [
    {
      ...
      columns: [
        ...
        { span: 8, xl: 5, controls: [{ type: 'date', model: 'startTime', label: '交易合同起始日', options: { format: datetime }, rules: [requiredRule(), { validator: validator('date'), format: datetime, end: 'expireTime', range: '1D|100Y' }] }] },
        { span: 8, xl: 5, controls: [{ type: 'date', model: 'expireTime', label: '交易合同到期日', dependOn: 'rowForm.fixDate === "0"', options: { format: datetime }, rules: [requiredRule(), { validator: validator('date'), format: datetime, start: 'startTime', range: '1D|100Y' }] }] },
...
export const baseFormDefs: Array<FieldDef | GridFieldDef> = [
  ...
  { type: 'date', model: 'transferStartTime', label: '转让起始时间', options: { type: 'date', format: datetime }, rules: [requiredRule(), { validator: validator('date'), format: datetime, end: 'transferEndTime', range: '1D|100Y' }] },
  { type: 'date', model: 'transferEndTime', label: '转让截止时间', options: { type: 'date', format: datetime }, rules: [requiredRule(), { validator: validator('date'), format: datetime, start: 'transferStartTime', range: '1D|100Y' }] },
  { type: 'select', model: 'transferType', label: '应收账款转让类型', options: { options: d('project_yszkzrlx'), disabled: true }, rules: [requiredRule()] },
  { span: 24, def: { type: 'textarea', model: 'contentDescription', label: '应收账款描述', options: { ...textareaOptions }, rules: [requiredRule()] } },
