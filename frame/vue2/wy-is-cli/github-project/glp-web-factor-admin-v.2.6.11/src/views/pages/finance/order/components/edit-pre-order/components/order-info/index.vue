  ...
  <edit-form :def="actualOrderFormDef" :form="orderForm" label-width="120px" label-position="right" :span="8" :xl="8">
    <template v-slot:edit="{proxy}">
      <span>
        ...
        <span v-if="hasBuyer">
          <el-button type="text" size="small" class="ml-3" icon="el-icon-edit" @click="onEditContact" v-if="atStep12 && type === 'audit'">编辑</el-button>
          <!-- <el-button type="text" size="small" class="ml-3" icon="el-icon-view" :disabled="false" @click="onViewContact" v-else>查看</el-button> -->
          <el-link :underline="false" type="primary" size="small" class="ml-3" icon="el-icon-view" @click="onViewContact" style="line-height: 30px;" v-else>查看</el-link>
        </span>
      </span>
    </template>
  </edit-form>
...
<glp-card title="融资关联信息" class="mt-4">
  ...
  <glp-section title="客户上传应收账款资料审核">
    <edit-form class="mt-2" :def="contractFormDef" :form="contractForm" :disabled="!atStep12 || type === 'view'" :span="6" :xl="6" label-width="120px"></edit-form>
  </glp-section>
