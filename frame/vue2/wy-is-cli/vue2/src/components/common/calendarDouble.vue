...
<script>
    ...
    export default {
        props: {
            calendarData: {
                beforeNowFlag: '',//是否可选，true不可选
                afterNowFlag: '',//默认今天以后的可以选，改为false不能选
                occupyDays: [],//不可选的集合
                startDate: '',
                endDate: '',
                minDays: ''
        ...
        methods: {
            ...
            render(y, m) {
                ...
                for(let i = 1; i <= lastDateOfMonth; i++) {
                    let day = {};
                    day.name = i;
                    day.nameCopy = i;//已有开始，结束，再选开始或者结束，需要把上一次的开始结束的name重置，用nameCopy的值去重置
                    day.ymd = y + '-' + this.zero(m) + '-' + this.zero(i);
                    day.beforeNowFlag = false; //默认进来都是可选的
                    day.showDateFlag = true;//是否可以选，true可选
                     //
                    if(this.startDate != this.endDate) {//开始日期不等于结束日期
            ...
            //已有开始，结束，再选开始或者结束，需要把上一次的开始结束的name重置，用nameCopy的值去重置
            clearName(){
                debugger
