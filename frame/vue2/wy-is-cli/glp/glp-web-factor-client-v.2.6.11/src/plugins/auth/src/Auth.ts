...
export default class Auth {
  ...
  toLogin (data: string, logout = false) {
    ...
    if (this.single) {
      const backUrl = logout ? window.location.origin : normalizeBackUrl(window.location.href)
      const url = new URL(data)
      url.searchParams.append('appId', process.env.VUE_APP_APP_ID)
      // there is some problem, because backend doesn't actually exit
      if (!logout) url.searchParams.append('backUrl', backUrl)
      const { origin, pathname, hash, search } = url
      const actualUrl = origin + pathname + hash + search
    ...
    function normalizeBackUrl (path: string) {
