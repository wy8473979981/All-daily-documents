...
export const formDefs: FieldDef[] = [
  ...
  { type: 'select', model: 'senceCode', label: '场景名称', options: { options: a('sceneName-by-appId') }, rules: [{ required: true, message: '必填' }] },
  { type: 'text', model: 'name', label: '凭证名称', rules: [{ required: true, message: '必填' }] },
  { type: 'select', model: 'jdeType', label: '凭证类型编码', options: { options: a('jdeType-name-by-appId') }, rules: [{ required: true, message: '必填' }] }
]
