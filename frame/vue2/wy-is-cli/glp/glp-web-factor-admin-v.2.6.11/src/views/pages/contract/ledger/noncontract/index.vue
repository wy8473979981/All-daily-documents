...
export default class Noncontract extends Vue {
 ...
 submit (params: any) {
   ...
   const req = { contractName, contractTypeId, coreContractId, email, fileName: name, ids, signMethod, signerInfos, url }
   this.$http.post<any>(
     'backend/contractWorkbench/addNonStandardContract',
     { ...req }
   ).then(data => {
     this.$message({
       type: 'success',
     ...
     this.$router.push(`/contract/audit/${data}?status=pending`)
   }).finally(() => {
     this.isSubmit = true
   })
 }
