...
export default class QueryTable extends Vue {
  @Prop({ type: String, default: '结果列表' }) title!: string
  ...
  render (h: CreateElement) {
    const data = {
      props: { title: this.title, subtitle: this.subtitle }
    }
    const tag = this.wrap ? 'glp-card' : 'div'
  ...
  private renderHeader (h: CreateElement) {
    ...
    function genCheckboxWrap (extra: any) {
      ...
      if (!vm.checkedRows || !vm.showCheckedInfo) return extra ? h('el-alert', { props: { type: 'info', closable: false }, style }, [h('div', { slot: 'title' }, [extra])]) : h('div', {}, extra)
      ...
      return h('el-alert', { props: { type: 'info', closable: false }, style }, [content])
    }
  }

  private renderDataTable (h: CreateElement) {
    const columns = !this.columns ? undefined : this.columns.map(v => {
      const old = v.formatter
      const formatter = (row: any, column: any, cellValue: string, index: number) => {
        if (old) return old(row, column, cellValue, index) || '-'
        return cellValue || '-'
      }
      return Object.assign({}, v, { formatter })
    })
