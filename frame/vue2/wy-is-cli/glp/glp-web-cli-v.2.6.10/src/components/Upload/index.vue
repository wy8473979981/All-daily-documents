<template>
    <div class="upload-section" v-loading="isLoading">
        ...
        <el-upload
            class="upload-ele"
            :class="hasLimit?'hidden-picture-card':''"
            ref="upload"
              action="#"
        ...
        <template v-if="!showFileList">
            ...
            <!-- 展示框 -->
            <UploadListTpl :dialog-visible="dialogVisible"
                :disabled="onlyView"
                :list-data="filesList"
                @eventDelete="onRemove"
...
<script type="text/javascript">
  ...
  export default {
    ...
    methods:{
      ...
      beforeUpload (file){          //监听文件变化
      },
      onChange (file,filesList){       //监听文件变化

      },
      uploadFn (obj){           //覆盖默认上传功能
        ...
        putFile (obj){                         //开始上传文件到oss
            let {file,filename}=obj;
            // let uuid = Utils.GUID();                         //避免可能重复的uuid
            let suffix = Utils.getSuffix(file.name);
            let key=this.stsToken.path+'/fsp-'+file.uid+'.'+suffix;
            let self=this;
      ...
      getStsToken (callback){                   //请求stsToken
        if(this.isRequestStsToken==true){return}
        this.isRequestStsToken=true;
        CommonApi.getStsToken().then((res) => {
      ...
      onError (data,res){                 //上传失败
        ...
        handelDeleteFile (item){                             //监听弹出列表删除文件
            this.filesList.findIndex()
        },
        handleClose (){                             //监听弹出列表关闭
