...
export function setSession(path, fromData, postMessage = true) {
  if (window.location.hash.includes('type=pd') && postMessage) {
...
/**
 * 根据年月 获取 当前年月的第一天，和最后一天
 * @param {string} 当前年月
 * @return {array} 当前年月的第一天，和最后一天
*/
export function getFullMonthRange(time) {
  const yearMonth = time.substring(0, 7);
  if (yearMonth.length > 5) {
    const startDate = parseTime(new Date(yearMonth), '{y}-{m}-{d}')

    const yearMonthEnd = new Date(new Date(yearMonth).setMonth(new Date(yearMonth).getMonth() + 1)).getTime() - 24 * 3600 * 1000;
    const endDate = parseTime(new Date(yearMonthEnd), '{y}-{m}-{d}')
